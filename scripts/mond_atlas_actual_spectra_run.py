...
def run():
    started=time.monotonic();out=PACKAGE/'run001';out.mkdir(exist_ok=False);private=PRIVATE/'run001';private.mkdir(parents=True,exist_ok=True)
    caches=json.loads((PACKAGE/'cache001/summary.json').read_text(encoding='utf-8'));assets={a['label']:a for a in caches['assets']}
    deps=[Path(__file__),ROOT/'scripts/mond_atlas_actual_spectra.py',PACKAGE/'PREFLIGHT.md',PACKAGE/'CORNER_REFINEMENT_ADDENDUM.md',PACKAGE/'cache001/summary.json']+[ROOT/a['cache'] for a in caches['assets']];save(out/'bindings.json',{str(p.relative_to(ROOT)):digest(p) for p in deps});spectra={};cases=[];statuses=[];corners=[]
    try:
        for height,gravity,pressure,branch,spin in itertools.product(['h0p1','h0p4'],['newton','newton_plus_log'],[5.,10.,15.],['boxcar_independent','boxcar_hanning_full','boxcar_hanning_decimated'],[-1,1]):
            ...
            for name,asset in assets.items():
                ...
                for systemic,sigma in nuisance:
                    tag=f'{label}_sys{int(systemic)}_sig{int(sigma)}';prediction=model.predict([systemic,sigma,1]);assert prediction.shape==(15,42) and np.isfinite(prediction).all();spectra[f'{name}__{tag}']=prediction
                ...
                for systemic,sigma in itertools.product([-30.,30.],[3.,20.]):
                    q=model.predict([systemic,sigma,1]);assert np.isfinite(q).all()
