...
def main():
    dest=ROOT/'work/gravity-first-principles/balanced-cube-gas-001';dest.mkdir(exist_ok=False)
    shutil.copy2(__file__,dest/'runner.py')
    reg=dict(baseline='constrained-cube-balanced-001',descriptor_predecessor='cube-gas-coverage-002',
        ...
        grid=np.linspace(-.3,.3,13).tolist(),gate='All five selected fits converge; at least 30 covered calibration/test pixels each.',
    ...
    result=read('work/gravity-first-principles/constrained-cube-balanced-001/result.json')
    ...
    for obj in result['objects']:
        ...
        if min(nt,nv)<30 or not all(f['optimizer_success'] for f in obj['fits']):
            skipped.append(dict(name=name,train_pixels=nt,test_pixels=nv,reason='Coverage or selected-fit convergence gate'));continue
