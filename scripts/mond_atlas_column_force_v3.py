...
def controls():
    ...
    return checks
def main():
    out=P/'run003';out.mkdir(exist_ok=False);manifest=R/'work/gravity-first-principles/mond-atlas-spatial-program-001/source-bindings.json';cfg=json.loads(manifest.read_text());cases=[c for c in cfg['source_cases'] if c['id'].startswith('f4')];paths=[Path(__file__),P/'PREFLIGHT.md',P/'SUPPORT_ADDENDUM.md',P/'RADIAL_ADDENDUM.md',manifest,R/'work/gravity-first-principles/mond-atlas-observation-admission-001/first-score-protocol.json'];bindings={str(p.relative_to(R)):hashlib.sha256(p.read_bytes()).hexdigest() for p in paths};comps={c['path']:c for ca in cases for c in ca['components']}
    ...
    hi=next(c for c in comps.values() if c['id']=='atomic_helium');hp=np.load(R/hi['path']);nodes=hp['latent_axis'];h=nodes[1]-nodes[0];surface=hp['intrinsic_effective_surface']/1.36;radii=np.arange(8,961)*.00625;theta=np.arange(2048)*2*np.pi/2048;x=radii[:,None]*np.cos(theta);y=radii[:,None]*np.sin(theta);coords=np.array([(x-nodes[0])/h,(y-nodes[0])/h]);weight=map_coordinates(surface,coords,order=1,mode='constant',cval=0,prefilter=False);assert np.all(weight.mean(axis=1)>0);hp.close()
    configs=[('nf96',96.,.03125,'newton',0),('l32fine',32.,.015625,'log',128)]
