...
for row in rows:
 label=row['case'];height,rest=label.split('_',1);gravity,rest=rest.rsplit('_p',1);pressure,rest=rest.split('_',1);branch,spin=rest.rsplit('_s',1);c=np.load(assets[row['source']]);v2=c['radius_kpc']*(c[f'force_{height}_{gravity}'].sum(0)+int(pressure)**2*c['pressure_gradient']/c[f'den_{height}']);bad=~(c['force_supported']&np.isfinite(v2)&(v2>=0));flux=c['grouped_flux'][:,bad].sum(1)
