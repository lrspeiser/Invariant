...
def rebin_tracer(image, header, good, geometry, grid, conversion=1., transform=None,
    ...
    rings, taper, inside_cutoff = integer_geometry(grid)
    nr = int(rings.max())+1
    ...
    report = dict(native_supported_pixels=len(ix), pixel_subdivisions=subdivisions,
                  ...
                  observed_area_fraction_inside_cutoff=float(np.mean(np.minimum(coverage,1)[inside_cutoff])),
...
def integer_geometry(grid):
    layout=grid.get('dimensionless_layout',{})
    names=('half_width_cells','annulus_width_cells','taper_start_cells','cutoff_cells')
    if set(layout)!=set(names):raise ValueError('Explicit integer layout required')
    if any(isinstance(layout[k],bool) or not isinstance(layout[k],(int,np.integer)) or layout[k]<=0 for k in names):raise ValueError('Positive integer index lengths required')
    h=float(grid['spacing_kpc'])
    physical=('half_width_kpc','annulus_width_kpc','taper_start_kpc','cutoff_kpc')
    for a,b in zip(names,physical):
        if not np.isclose(layout[a]*h,grid[b],rtol=1e-12,atol=0):raise ValueError('Physical and index lengths disagree')
    half,A,start,cut=[int(layout[k]) for k in names]
    if not 0<start<cut<=half:raise ValueError('Invalid index support')
    index=np.arange(-half,half+1,dtype=np.int64)
    squared=index[:,None]**2+index[None,:]**2
    rings=squared_annuli(squared,A)
    taper=np.clip((cut-np.sqrt(squared))/(cut-start),0,1)
    return rings,taper,squared<cut*cut


def squared_annuli(squared,width):
    a=np.asarray(squared)
    if not isinstance(width,(int,np.integer)) or isinstance(width,bool) or width<=0 or np.any(a<0) or not np.isfinite(a).all():raise ValueError('Nonnegative squared radii and positive integer width required')
    count=int(np.sqrt(float(a.max()))/width)+2
    edges=(np.arange(count,dtype=np.int64)*int(width))**2
    return np.searchsorted(edges,a,side='right')-1
