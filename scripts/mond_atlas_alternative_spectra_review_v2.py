...
def run():
 out=P/'scalar-review';out.mkdir(exist_ok=False);r=P/'run001';s=json.loads((r/'summary.json').read_text());packet=ROOT/s['private_packet'];assert digest(packet)==s['private_sha256'];z=np.load(packet);v=load_instrument()['velocity_km_s'];pairs={'planar':('p0p0625_z24','p0p03125_z24'),'vertical':('p0p0625_z24','p0p0625_z48'),'joint':('p0p0625_z24','p0p03125_z48'),'fine_planar_vertical':('p0p03125_z24','p0p03125_z48'),'fine_vertical_planar':('p0p0625_z48','p0p03125_z48')};rows=list(csv.DictReader((r/'refinement.csv').open()));error=0
 for row in rows:
