...
PROTOCOL=ROOT/"configs/mond_atlas_noise_v2.json"
...
def masks(east,north,config):
    ...
    if config.get("split_mode")=="balanced_geometry":
        candidate=background&interior
        stride=config["validation_stride_pixels"]
        sparse=candidate&(xx%stride==0)&(yy%stride==0)
        quadrant=(east>=0).astype(int)+2*(north>=0).astype(int)
        block=(yy//side)*1000+xx//side
        ids=np.unique(block[candidate])
        counts=np.array([np.bincount(quadrant[sparse&(block==i)],minlength=4) for i in ids])
        fullcounts=np.array([np.sum(candidate&(block==i)) for i in ids])
        rng=np.random.default_rng(config["split_seed"])
        labels=rng.integers(0,2,(config["split_candidates"],len(ids)))
        validation_counts=labels@counts;calibration_counts=counts.sum(axis=0)-validation_counts
        validation_total=validation_counts.sum(axis=1)
        calibration_total=(1-labels)@fullcounts
        required=config["diagnostic_gates"]["minimum_quadrant_pixels"]
        acceptable=(validation_counts>=required).all(axis=1)&(calibration_counts>=required).all(axis=1)
        acceptable &= (validation_total>=config["minimum_validation_pixels"]) & (calibration_total>=config["minimum_calibration_pixels"])
        score=np.sum(((validation_counts-counts.sum(axis=0)/2)/(counts.sum(axis=0)+1))**2,axis=1)
        score[~acceptable]=np.inf
        if not np.isfinite(score).any():raise ValueError("no geometry-only split covers every quadrant")
        chosen=labels[int(np.argmin(score))].astype(bool)
        test_blocks=np.isin(block,ids[chosen])
        return candidate&~test_blocks,sparse&test_blocks
...
def check_packet(packet,config):
    ...
    result=dict(training_background_pixels=int(sum(train.ravel())),validation_background_pixels=int(sum(test.ravel())),
        validation_quadrant_counts=np.bincount(quadrant,minlength=4).tolist(),
...
def run(output,private,protocol_path=PROTOCOL):
    ...
    summary=dict(status="EXECUTED_BACKGROUND_DIAGNOSTIC" if not failures else "INCOMPLETE_BACKGROUND_DIAGNOSTIC",
        ...
        input_hashes={p.resolve().relative_to(ROOT).as_posix():digest(p) for p in [protocol_path,source]},
...
if __name__=="__main__":
    ...
    args=p.parse_args();result=run(args.output,args.private,args.protocol)
