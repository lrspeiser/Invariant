...
def run():
    start=time.monotonic();out=P/'emitter001';out.mkdir(exist_ok=False)
    ...
    for case in sorted(set(row['case'] for row in rows)):
        ...
        den=np.interp(unique_r,r,sigma);assert np.all(den>0)
        ...
        for model,key in [('newton','newton_gbar'),('newton_plus_log','newton_plus_log_gbar')]:
            base=np.array([np.interp(unique_r,r,[float(row[key]) for row in c]) for c in component])
