...
def main():
    out=P/'run004';out.mkdir(exist_ok=False)
    ...
    for j,y in enumerate(range(12,h['NAXIS2'],24)):
        for i,x in enumerate(range(12,h['NAXIS1'],24)):
            ...
            xx,yy=np.meshgrid(x-6-.5+np.arange(13),y-6-.5+np.arange(13));sky=w.pixel_to_world(xx,yy);offset=sky.transform_to(origin.skyoffset_frame());e=offset.lon.rad*D;n=offset.lat.rad*D
