...
registration=dict(names=names,geometry=geom,scenarios=scenarios,
    ...
    local_stellar_input='Squared stellar-disk force-component speed from published source decomposition; no new SPARC velocity targets opened for scoring. Complete 12-object surface-brightness supplement is unavailable, so no placeholder brightness is inserted.',
...
def source_fields(name,g,shape,header,ml,di,dp):
    ...
    radius=np.hypot(major,minor/np.cos(inc))*float(raw[name]['distance_mpc'])*1000*np.pi/(180*3600)
    ...
    sb=ml[0]*vd**2  # Stellar force-component scale, explicitly not measured surface brightness.
...
def build_all():
    ...
    for name in names:
        ...
        for resolution in ('NA','RO'):
            ...
            assert fits.getheader(paths[1])['BUNIT'].strip()=='METR/SEC'
            ...
            for scenario,(ml,di,dp) in scenarios.items():
                ...
                X=np.column_stack([np.log10(np.maximum(y,1e-20)),np.log10(np.maximum(rr,1e-20)),np.log10(1+sb),fg,
                    ...
                    np.full(len(rr),np.sin(fields['inc'])),np.full(len(rr),np.log10(float(raw[name]['distance_mpc'])))])
