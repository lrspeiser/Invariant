...
D=ROOT/'work/gravity-first-principles/ngc2903-matter-002';D.mkdir(exist_ok=False)
P=ROOT/'work/private/ngc2903-matter-002';P.mkdir(exist_ok=False)
...
registration=dict(name='NGC2903',source_selection='Only object passing stellar-gaia-alignment-001 strict holdout gate.',
 ...
 relative_registration='Prior source-only diagnostic found P5->P1 shift (-3,-1) P1 pixels. Freeze this correction; fit flux scale/background on alternating 80-pixel blocks and evaluate the other blocks. Source-only development validation, not a pristine holdout.',
...
shift=np.array([-3.,-1.])
sw.wcs.crpix-=np.linalg.solve(sw.pixel_scale_matrix,w1.pixel_scale_matrix@shift)
...
combined=star+dust
calibration=bright&((xx//80+yy//80)%2==0);validation=bright&~calibration
design=np.column_stack([observed[calibration],np.ones(calibration.sum())]);coefs=np.linalg.lstsq(design,combined[calibration],rcond=None)[0]
prediction=observed*coefs[0]+coefs[1]
relative=float(np.sqrt(np.mean((combined[validation]-prediction[validation])**2))/np.sqrt(np.mean(combined[validation]**2)))
correlation=float(np.corrcoef(combined[validation],observed[validation])[0,1])
...
save(D/'p1-p5-registration.json',dict(relative_flux_rms=relative,correlation=correlation,pass_gate=alignment_pass,
 p1_pixel_translation=shift.tolist(),comparison_flux_scale=float(coefs[0]),comparison_background=float(coefs[1]),
