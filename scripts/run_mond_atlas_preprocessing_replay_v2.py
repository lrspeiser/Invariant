...
def run(config_path,output):
    ...
    paths=[config_path,ROOT/config['native_protocol'],native_path,ROOT/config['cached_packet_bindings'],Path(__file__),
        ...
        ROOT/'tests/test_mond_atlas_preprocessing.py',ROOT/'scripts/mond_atlas_channel_order.py',
        ROOT/'tests/test_mond_atlas_channel_order.py',ROOT/'scripts/run_gravity_cube_pilot.py']
    ...
    suite=unittest.TestSuite([unittest.defaultTestLoader.discover(str(ROOT/'tests'),pattern=pattern) for pattern in ['test_mond_atlas_preprocessing.py','test_mond_atlas_channel_order.py']])
    ...
    for item in sorted(read_json(ROOT/native_config['cube_receipt'])['files'],key=lambda r:r['name']):
        ...
        try:
            ...
            direction=increasing_optical_velocity_direction(h,audit['spectral_input'])
            ...
            for channel in selected:
                ...
                stored=cube.shape[0]-1-channel if direction<0 else channel;reference=cached[stored]
...
if __name__=='__main__':
    ...
    parser.add_argument('--config',type=Path,default=ROOT/'configs/mond_atlas_preprocessing_replay_v2.json')
