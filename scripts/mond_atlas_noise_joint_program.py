...
def run():
    out=P/'run002';out.mkdir(parents=True,exist_ok=False)
    try:
        ...
        for alpha,K in covs.items():
            ...
            for side in [1,2,4,8,12,24]:
                ...
                apertures.append(dict(alpha=alpha,selected=alpha==selected,side=side,q=float(aq.mean()),trace_ratio=float(tr.mean()),q_pass=bool(.8<=aq.mean()<=1.2)))
