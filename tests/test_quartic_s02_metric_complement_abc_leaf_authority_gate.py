...
from sigma_theory_compiler.quartic_s02_metric_complement_abc_leaf_authority_gate import (
    OUTPUT_PATH,
    S02MetricComplementLeafAuthorityError,
    _content_sha,
    build_campaign,
    validate_campaign,
)
...
def test_nine_column_complement_completes_s02_family() -> None:
    counts = _load()["gate_counts"]
    assert counts["previous_s02_metric_columns"] == 1
    assert counts["new_s02_metric_coordinate_columns"] == 9
    assert counts["complete_s02_metric_family_columns"] == 10
...
def test_existing_s02_atom_is_not_recounted() -> None:
    checked = _load()
    assert checked["exact_controls"]["previous_s02_atom_not_recounted"] == "s02[2]"
    for candidate in checked["candidate_manifests"]:
        assert len(candidate["direction_packets"]) == 180
        assert all(row["derivative_atom"] != "s02[2]" for row in candidate["direction_packets"])


def test_projection_family_is_source_bound_with_exact_zeros() -> None:
    packets = _load()["projection_packets"]
    assert [row["coordinate_atom"] for row in packets] == [f"s02[{field}]" for field in range(10)]
...
def test_D2_and_global_claims_remain_closed() -> None:
    ...
    assert counts["remaining_coordinate_columns_without_A_B_C_leaf_authority"] == 95
...
def test_resealed_projection_D2_and_scope_tamper_fails() -> None:
    ...
    for mutate in mutations:
        ...
        with pytest.raises(S02MetricComplementLeafAuthorityError, match="result changed"):
