...
import sigma_theory_compiler.continuous_scientific_pipeline_formal_receipt_batch_0004 as worker
...
def test_checked_successor_advances_two_more_leaves_without_promotion() -> None:
    ...
    assert result["batch_leaf_catalog_indices"] == [13, 14]
    assert result["counts"]["processed_partition_prefix_length"] == 15
    assert result["counts"]["processed_leaf_pages"] == 15
    assert result["counts"]["cumulative_formally_checked_candidates"] == 268
    assert result["counts"]["cumulative_newly_processed_candidates"] == 266
    assert result["counts"]["cumulative_reconciled_preserved_candidates"] == 2
    assert result["counts"]["cumulative_candidate_rejects"] == 268
    assert result["counts"]["remaining_pending_formal_receipts"] == 10_981
    assert [leaf["counts"]["partition_candidates"] for leaf in leaves] == [30, 12]
    ...
    assert result["first_remaining_blocker"] == "10981_candidate_specific_formal_receipts_pending"
...
def test_selection_is_deterministic_disjoint_bounded_and_merkle_reachable() -> None:
    ...
    assert [row["catalog_index"] for row in selected] == [13, 14]
    assert [row["pending_candidate_count"] for row in selected] == [30, 12]
    assert sum(row["pending_candidate_count"] for row in selected) == 42
...
def test_predecessor_lineage_and_receipts_are_preserved_verbatim() -> None:
    ...
    assert cursor["processed_partition_summaries"][:13] == previous["processed_partition_summaries"]
    assert (
        cursor["cumulative_formal_receipt_records"][:226]
...
def test_new_receipts_are_disjoint_source_bound_and_all_rejected() -> None:
    ...
    assert len(ordinals) == len(set(ordinals)) == 42
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda value: value.__setitem__("batch_leaf_catalog_indices", [13, 15]),
