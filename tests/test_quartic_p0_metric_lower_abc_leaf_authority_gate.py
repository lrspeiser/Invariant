...
from sigma_theory_compiler.quartic_p0_metric_lower_abc_leaf_authority_gate import (
    OUTPUT_PATH,
    P0MetricLowerLeafAuthorityError,
    _content_sha,
    build_campaign,
    validate_campaign,
)
...
def test_all_ten_p0_metric_tangents_are_materialized_on_the_general_domain() -> None:
    checked = _load()
    packets = checked["coordinate_tangent_packets"]
    assert [row["coordinate_atom"] for row in packets] == [f"p0[{field}]" for field in range(10)]
    assert [row["coordinate_column"] for row in packets] == list(range(10, 20))
...
def test_exact_negative_controls_forbid_affine_or_flat_substitutions() -> None:
    controls = _load()["exact_controls"]
    assert controls["off_diagonal_seed_normalization"]["exact_value"] == "sqrt(2)/2"
    assert controls["off_diagonal_seed_normalization"]["replace_by_one_rejected"] is True
...
def test_d2_and_remaining_lower_boundaries_stay_fail_closed() -> None:
    ...
    assert counts["previous_missing_coordinate_columns"] == 48
    assert counts["new_p0_metric_coordinate_columns"] == 10
    assert counts["remaining_coordinate_columns_without_A_B_C_leaf_authority"] == 38
...
def test_resealed_tangent_domain_root_or_d2_tamper_fails_closed() -> None:
    mutations = (
        lambda value: value["coordinate_tangent_packets"][0]["delta_H"].update({"H_00": "0"}),
        lambda value: value["coordinate_tangent_packets"][1].update({"domain": "flat only"}),
    ...
    for mutate in mutations:
        ...
        mutate(corrupted)
        _reseal(corrupted)
        with pytest.raises(P0MetricLowerLeafAuthorityError, match="checked result changed"):
