...
import sigma_theory_compiler.continuous_scientific_pipeline_formal_receipt_batch_0003 as worker
...
def test_checked_successor_advances_prefix_by_two_leaves_without_promotion() -> None:
    ...
    assert result["batch_leaf_catalog_indices"] == [11, 12]
    assert [leaf["counts"]["partition_candidates"] for leaf in leaves] == [22, 24]
    assert sum(leaf["counts"]["newly_processed_candidates"] for leaf in leaves) == 46
    assert sum(leaf["counts"]["candidate_rejects"] for leaf in leaves) == 46
    assert result["counts"] == {
        "processed_partition_prefix_length": 13,
        "processed_leaf_pages": 13,
        "cumulative_formally_checked_candidates": 226,
        "cumulative_newly_processed_candidates": 224,
        "cumulative_reconciled_preserved_candidates": 2,
        "cumulative_candidate_rejects": 226,
        ...
        "remaining_pending_formal_receipts": 11_023,
        "rank_assignments": 0,
    ...
    assert result["first_remaining_blocker"] == "11023_candidate_specific_formal_receipts_pending"
...
def test_selection_is_deterministic_bounded_and_merkle_reachable() -> None:
    ...
    assert [row["catalog_index"] for row in selected] == [11, 12]
    assert [row["pending_candidate_count"] for row in selected] == [22, 24]
    assert sum(row["pending_candidate_count"] for row in selected) == 46
    assert [row["leaf_queue_root_sha256"] for row in selected] == [
        "291064f52553919e9c83ec27937d05d9140ca65f2347926da0c48c746032e9a4",
        "ea1b33f47824eb39202d4e1345641ba7eedbeb496ed32169c5c5d3f4aa4237e3",
    ]
    assert [len(row["hierarchy_path"]) for row in selected] == [9, 9]
...
def test_predecessor_lineage_and_receipts_are_preserved_verbatim() -> None:
    ...
    assert cursor["processed_partition_summaries"][:11] == previous["processed_partition_summaries"]
    assert (
        cursor["cumulative_formal_receipt_records"][:180]
...
def test_prefix_rejects_overlap_and_gap(mutation: str) -> None:
    ...
    if mutation == "overlap":
        summaries[12]["leaf_catalog_index"] = 11
        summaries[12]["leaf_binding"] = summaries[11]["leaf_binding"]
    else:
        summaries[12]["leaf_catalog_index"] = 13
        summaries[12]["leaf_binding"] = catalog[13]["leaf_binding"]
    partitions = [
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda value: value.__setitem__("batch_leaf_catalog_indices", [11, 13]),
