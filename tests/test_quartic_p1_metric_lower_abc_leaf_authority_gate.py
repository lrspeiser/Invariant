...
from sigma_theory_compiler.quartic_p1_metric_lower_abc_leaf_authority_gate import (
    OUTPUT_PATH,
    P1MetricLowerLeafAuthorityError,
    _content_sha,
    build_campaign,
    validate_campaign,
)
...
def test_all_ten_p1_metric_tangents_are_materialized_on_the_pointwise_frame_domain() -> None:
    checked = _load()
    packets = checked["coordinate_tangent_packets"]
    assert [row["coordinate_atom"] for row in packets] == [f"p1[{field}]" for field in range(10)]
    assert [row["coordinate_column"] for row in packets] == list(range(21, 31))
    assert all(row["seed"]["dP_derivative"] == 1 for row in packets)
    ...
    assert checked["coordinate_tangent_program"]["no_flat_reference_specialization"] is True
    assert checked["gate_counts"]["materialized_coordinate_tangent_scalar_values"] == 200
...
def test_every_candidate_has_200_live_target_pairs_and_26400_roots() -> None:
    ...
    for manifest in manifests:
        assert manifest["derivative_coordinate_columns"] == list(range(21, 31))
...
def test_exact_controls_distinguish_p1_from_p0_and_forbid_flat_substitution() -> None:
    ...
    assert controls["connection_dependence"]["infer_zero_rejected"] is True
    assert controls["inverse_first_jet_product_rule"]["omit_term_rejected"] is True
...
def test_d2_and_remaining_lower_boundaries_stay_fail_closed() -> None:
    ...
    assert counts["previous_missing_coordinate_columns"] == 38
    assert counts["new_p1_metric_coordinate_columns"] == 10
    assert counts["remaining_coordinate_columns_without_A_B_C_leaf_authority"] == 28
...
def test_resealed_tangent_index_root_or_d2_tamper_fails_closed() -> None:
    mutations = (
        lambda value: value["coordinate_tangent_packets"][0]["seed"].update({"dP_derivative": 0}),
        lambda value: value["coordinate_tangent_packets"][1]["delta_H"].update({"H_11": "0"}),
    ...
    for mutate in mutations:
        ...
        mutate(corrupted)
        _reseal(corrupted)
        with pytest.raises(P1MetricLowerLeafAuthorityError, match="checked result changed"):
