...
from sigma_theory_compiler.quartic_p3_metric_lower_abc_leaf_authority_gate import (
    OUTPUT_PATH,
    P3MetricLowerLeafAuthorityError,
    _content_sha,
    build_campaign,
    validate_campaign,
)
...
def test_all_ten_p3_metric_tangents_are_materialized_by_exact_spatial_permutation() -> None:
    checked = _load()
    packets = checked["coordinate_tangent_packets"]
    assert [row["coordinate_atom"] for row in packets] == [f"p3[{field}]" for field in range(10)]
    assert [row["coordinate_column"] for row in packets] == list(range(43, 53))
    assert all(row["seed"]["dP_derivative"] == 3 for row in packets)
    ...
    assert program["exact_spatial_permutation"] == "swap_indices_1_and_3_from_p1"
...
def test_every_candidate_has_200_live_target_pairs_and_26400_roots() -> None:
    ...
    for manifest in manifests:
        assert manifest["derivative_coordinate_columns"] == list(range(43, 53))
...
def test_exact_controls_distinguish_p3_and_reject_nonspatial_or_flat_substitution() -> None:
...
def test_d2_and_remaining_lower_boundaries_stay_fail_closed() -> None:
    ...
    assert counts["previous_missing_coordinate_columns"] == 18
    assert counts["new_p3_metric_coordinate_columns"] == 10
    assert counts["remaining_coordinate_columns_without_A_B_C_leaf_authority"] == 8
...
def test_resealed_nonzero_tangent_index_root_or_d2_tamper_fails_closed() -> None:
    ...
    for mutate in mutations:
        ...
        mutate(corrupted)
        _reseal(corrupted)
        with pytest.raises(P3MetricLowerLeafAuthorityError, match="checked result changed"):
