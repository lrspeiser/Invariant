...
from sigma_theory_compiler.quartic_s11_metric_complement_abc_leaf_authority_gate import (
    OUTPUT_PATH,
    S11MetricComplementLeafAuthorityError,
    _content_sha,
    build_campaign,
    validate_campaign,
)
...
def test_six_column_complement_completes_s11_family() -> None:
    counts = _load()["gate_counts"]
    assert counts["previous_s11_metric_columns"] == 4
    assert counts["new_s11_metric_coordinate_columns"] == 6
    assert counts["complete_s11_metric_family_columns"] == 10
    assert counts["new_leaf_derivative_roots_all_candidates"] == 190080
    assert (
...
def test_existing_s11_atoms_are_not_recounted() -> None:
    checked = _load()
    expected = ["s11[0]", "s11[4]", "s11[7]", "s11[9]"]
    assert checked["exact_controls"]["previous_s11_atoms_not_recounted"] == expected
    for candidate in checked["candidate_manifests"]:
        assert len(candidate["direction_packets"]) == 120
        assert all(
            row["derivative_atom"] not in set(expected) for row in candidate["direction_packets"]
        )


def test_projection_family_is_complete_and_source_bound() -> None:
    packets = _load()["projection_packets"]
    assert [row["coordinate_atom"] for row in packets] == [f"s11[{field}]" for field in range(10)]
    assert sum(row["already_registered_leaf_authority"] for row in packets) == 4
    assert all(row["delta_H"] == "0" and row["delta_v"] == "0" for row in packets)


def test_D2_and_larger_lower_families_remain_closed() -> None:
    checked = _load()
    ...
    assert counts["remaining_coordinate_columns_without_A_B_C_leaf_authority"] == 54
    assert seals["larger_lower_families_registered"] is False
...
def test_every_candidate_packet_has_exact_census() -> None:
    for candidate in _load()["candidate_manifests"]:
        assert candidate["leaf_derivative_roots"] == 15840
        assert (
...
def test_resealed_projection_D2_and_scope_tamper_fails() -> None:
    mutations = (
        ...
        lambda value: value["claim_seals"].update({"larger_lower_families_registered": True}),
    )
    for mutate in mutations:
        corrupted = copy.deepcopy(_load())
        mutate(corrupted)
        corrupted["content_sha256"] = _content_sha(corrupted)
        with pytest.raises(S11MetricComplementLeafAuthorityError, match="result changed"):
