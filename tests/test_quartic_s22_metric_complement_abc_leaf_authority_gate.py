...
from sigma_theory_compiler.quartic_s22_metric_complement_abc_leaf_authority_gate import (
    OUTPUT_PATH,
    S22MetricComplementLeafAuthorityError,
    _content_sha,
    build_campaign,
    validate_campaign,
)
...
def test_six_column_complement_completes_s22_family() -> None:
    counts = _load()["gate_counts"]
    assert counts["previous_s22_metric_columns"] == 4
    assert counts["new_s22_metric_coordinate_columns"] == 6
    assert counts["complete_s22_metric_family_columns"] == 10
...
def test_existing_s22_atoms_are_not_recounted() -> None:
    checked = _load()
    expected = ["s22[0]", "s22[4]", "s22[7]", "s22[9]"]
    assert checked["exact_controls"]["previous_s22_atoms_not_recounted"] == expected
...
def test_projection_family_is_complete_and_source_bound() -> None:
    ...
    assert [row["coordinate_atom"] for row in packets] == [f"s22[{field}]" for field in range(10)]
...
def test_D2_and_lower_families_remain_closed() -> None:
    checked = _load()
    ...
    assert counts["remaining_coordinate_columns_without_A_B_C_leaf_authority"] == 48
    assert seals["remaining_lower_families_registered"] is False
...
def test_resealed_projection_D2_and_scope_tamper_fails() -> None:
    mutations = (
        ...
        lambda value: value["claim_seals"].update({"remaining_lower_families_registered": True}),
    )
    for mutate in mutations:
        corrupted = copy.deepcopy(_load())
        mutate(corrupted)
        corrupted["content_sha256"] = _content_sha(corrupted)
        with pytest.raises(S22MetricComplementLeafAuthorityError, match="result changed"):
