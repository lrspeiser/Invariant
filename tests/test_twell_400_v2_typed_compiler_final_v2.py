...
from sigma_theory_compiler.twell_400_v2_typed_compiler_final_v2 import (
    CARDS_PATH,
    CONFIG_PATH,
...
def test_every_live_card_is_schema_valid_registry_admissible_and_versioned(
    packet: tuple[list[dict], bytes, dict],
) -> None:
    ...
    for row in rows:
        ...
        assert card["parents"][0]["relation"] == "REPAIRS"
        assert card["parents"][0]["card_sha256"] == card["version_change"]["previous_card_sha256"]
...
def test_stored_final_packet_matches_exact_rebuild() -> None:
    assert check_packet(ROOT, ROOT / CARDS_PATH, ROOT / RECEIPT_PATH) == build_packet(ROOT)[2]
