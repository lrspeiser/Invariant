...
from sigma_theory_compiler.quartic_s33_metric_complement_abc_leaf_authority_gate import (
    OUTPUT_PATH,
    S33MetricComplementLeafAuthorityError,
    _content_sha,
    build_campaign,
    validate_campaign,
)
...
def test_eight_column_complement_completes_s33_family() -> None:
    counts = _load()["gate_counts"]
    assert counts["previous_s33_metric_columns"] == 2
    assert counts["new_s33_metric_coordinate_columns"] == 8
    assert counts["complete_s33_metric_family_columns"] == 10
    assert counts["new_leaf_derivative_roots_all_candidates"] == 253440
    assert (
...
def test_existing_s33_atoms_are_not_recounted() -> None:
    checked = _load()
    assert checked["exact_controls"]["previous_s33_atoms_not_recounted"] == ["s33[4]", "s33[7]"]
    for candidate in checked["candidate_manifests"]:
        assert len(candidate["direction_packets"]) == 160
        assert all(
            row["derivative_atom"] not in {"s33[4]", "s33[7]"}
            for row in candidate["direction_packets"]
        )


def test_projection_family_is_complete_and_source_bound() -> None:
    packets = _load()["projection_packets"]
    assert [row["coordinate_atom"] for row in packets] == [f"s33[{field}]" for field in range(10)]
    assert sum(row["already_registered_leaf_authority"] for row in packets) == 2
...
def test_D2_and_global_claims_remain_closed() -> None:
    ...
    assert counts["remaining_coordinate_columns_without_A_B_C_leaf_authority"] == 60
...
def test_every_candidate_packet_has_exact_census() -> None:
    for candidate in _load()["candidate_manifests"]:
        assert candidate["leaf_derivative_roots"] == 21120
        assert (
...
def test_resealed_projection_D2_and_scope_tamper_fails() -> None:
    ...
    for mutate in mutations:
        ...
        with pytest.raises(S33MetricComplementLeafAuthorityError, match="result changed"):
