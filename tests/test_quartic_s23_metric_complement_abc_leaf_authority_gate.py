...
from sigma_theory_compiler.quartic_s23_metric_complement_abc_leaf_authority_gate import (
    OUTPUT_PATH,
    S23MetricComplementLeafAuthorityError,
    _content_sha,
    build_campaign,
    validate_campaign,
)
...
def test_nine_column_complement_completes_s23_family() -> None:
    counts = _load()["gate_counts"]
    assert counts["previous_s23_metric_columns"] == 1
    assert counts["new_s23_metric_coordinate_columns"] == 9
    assert counts["complete_s23_metric_family_columns"] == 10
...
def test_existing_s23_atom_is_not_recounted() -> None:
    checked = _load()
    assert checked["exact_controls"]["previous_s23_atom_not_recounted"] == "s23[8]"
    for candidate in checked["candidate_manifests"]:
        assert len(candidate["direction_packets"]) == 180
        assert all(row["derivative_atom"] != "s23[8]" for row in candidate["direction_packets"])
        assert candidate["candidate_decision"].startswith("pass_s23_metric_complement")


def test_projection_family_is_complete_and_source_bound() -> None:
    packets = _load()["projection_packets"]
    assert [row["coordinate_atom"] for row in packets] == [f"s23[{field}]" for field in range(10)]
...
def test_D2_and_global_claims_remain_closed() -> None:
    ...
    assert counts["remaining_coordinate_columns_without_A_B_C_leaf_authority"] == 68
...
def test_resealed_projection_D2_and_scope_tamper_fails() -> None:
    ...
    for mutate in mutations:
        ...
        with pytest.raises(S23MetricComplementLeafAuthorityError, match="result changed"):
