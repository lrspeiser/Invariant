...
def radial_tail_jet(disk, k, weights, transform, radius, cutoff, *, log_nodes=128, batch_size=32):
...
def complete_leading_tail(near, disks, components, k, weights, transforms, radius, height, G, cutoff, *, log_nodes=128):
    ...
    for name, S in zip(components, transforms, strict=True):
        d = disks[name]
        a = radial_tail_jet(d, k, weights, S, radius, cutoff, log_nodes=log_nodes)
