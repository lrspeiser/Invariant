"""Complete s23 metric A/B/C leaf authority with its nine-column complement."""
...
from . import quartic_s13_metric_complement_abc_leaf_authority_gate as prior
...
CONFIG_SCHEMA = "sigma-quartic-s23-metric-complement-abc-leaf-authority-config-1.0"
RESULT_SCHEMA = "sigma-quartic-s23-metric-complement-abc-leaf-authority-gate-1.0"
CAMPAIGN_ID = "quartic-s23-metric-complement-abc-leaf-authority-001"
STEM = "quartic_s23_metric_complement_abc_leaf_authority_gate"
SLUG = "quartic-s23-metric-complement-abc-leaf-authority-gate"
...
CONFIG_PRODUCTION_SHA256 = "304b5fc2db23110a606bb40732798bed31d556230324cbfe36620ce9b8f56f78"
S23_FULL = tuple((f"s23[{field}]", 131 + field) for field in range(10))
S23_EXISTING = "s23[8]"
S23_NEW = tuple(row for row in S23_FULL if row[0] != S23_EXISTING)
CONTRACT = {
    "candidate_count": 12,
    "previous_missing_coordinate_columns": 77,
    "new_s23_metric_columns": 9,
    "previous_s23_metric_columns": 1,
...
POLICIES = {
    "family_selection": "complete_s23_metric_family_by_exact_nine_column_complement",
...
class S23MetricComplementLeafAuthorityError(ValueError):
    """An s23 projection, root, or authority binding changed."""


def _inside(root: Path, relative: str) -> Path:
    if not relative or "\\" in relative:
        raise S23MetricComplementLeafAuthorityError("s23 path is not portable")
    path = (root / relative).resolve()
    if path != root and root not in path.parents:
        raise S23MetricComplementLeafAuthorityError("s23 path escapes root")
    return path


def _validate_config(value: Mapping[str, Any], path: Path) -> None:
    if _production_sha(path) != CONFIG_PRODUCTION_SHA256:
        raise S23MetricComplementLeafAuthorityError("config production bytes changed")
    if (
        ...
        raise S23MetricComplementLeafAuthorityError("config contract changed")
...
def _load_predecessor(root: Path, bundle: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for key, relative in {
        ...
        if not _matches_text_authority(_inside(root, relative), str(bundle[key])):
            raise S23MetricComplementLeafAuthorityError("predecessor text changed")
    value = json.loads(
    ...
    if value.get("content_sha256") != bundle["content_sha256"] or value.get(
        ...
        raise S23MetricComplementLeafAuthorityError("predecessor receipt changed")
    if (
        value.get("decision")
        != "pass_s13_metric_nine_column_complement_285120_exact_leaf_roots_D2_blocked"
        or value.get("gate_counts", {}).get(
            "remaining_coordinate_columns_without_A_B_C_leaf_authority"
        )
        != 77
    ):
        raise S23MetricComplementLeafAuthorityError("predecessor boundary changed")
    return value
...
def _projection_packets(
    ...
    packets, tangents = [], {}
    for atom, column in S23_FULL:
        row = by_atom[atom]
        if (
            row.get("coordinate_column") != column
            or row.get("derivative_pair") != [2, 3]
            or row.get("theorem")
            != "arbitrary_inverse_metric_Einstein_principal_second_jet_formula"
        ):
            raise S23MetricComplementLeafAuthorityError("s23 projection changed")
        tangent = {
        ...
        body = {
            ...
            "already_registered_leaf_authority": atom == S23_EXISTING,
...
def _base_body(
    ...
    old_full, old_existing, old_new = prior.S13_FULL, prior.S13_EXISTING, prior.S13_NEW
    old_projection = prior._projection_packets
    prior.S13_FULL, prior.S13_EXISTING, prior.S13_NEW = S23_FULL, S23_EXISTING, S23_NEW
    prior._projection_packets = _projection_packets
    try:
        return prior._expected_body(root, config_path, config, predecessor)
    finally:
        prior.S13_FULL, prior.S13_EXISTING, prior.S13_NEW = old_full, old_existing, old_new
...
def _expected_body(
    ...
    if _unspecialized_principal_blocks()["content_sha256"] != prior.prior.prior.prior.BLOCK_SHA256:
        raise S23MetricComplementLeafAuthorityError("live A/B/C block changed")
    body = _base_body(root, config_path, config, predecessor)
    body["schema_version"] = RESULT_SCHEMA
    body["campaign_id"] = CAMPAIGN_ID
    body["decision"] = "pass_s23_metric_nine_column_complement_285120_exact_leaf_roots_D2_blocked"
    body["exact_controls"] = {
        "previous_s23_atom_not_recounted": S23_EXISTING,
    ...
    for key in (
        "previous_s13_metric_columns",
        "new_s13_metric_coordinate_columns",
        "complete_s13_metric_family_columns",
    ):
        counts.pop(key)
    counts.update(
        {
            "previous_registered_coordinate_columns": 76,
            "previous_missing_coordinate_columns": 77,
            "previous_s23_metric_columns": 1,
            "new_s23_metric_coordinate_columns": 9,
            "complete_s23_metric_family_columns": 10,
            "registered_coordinate_columns_after": 85,
            "remaining_coordinate_columns_without_A_B_C_leaf_authority": 68,
        }
    )
    seals = body["claim_seals"]
    for key in (
        "complete_s13_metric_family_registered",
        "previous_s13_atom_not_recounted",
        "remaining_77_coordinate_columns_registered",
    ):
        seals.pop(key)
    seals.update(
        {
            "complete_s23_metric_family_registered": True,
            "previous_s23_atom_not_recounted": True,
            "remaining_68_coordinate_columns_registered": False,
        }
    )
    for manifest in body["candidate_manifests"]:
        manifest["candidate_decision"] = (
            "pass_s23_metric_complement_leaf_authority_D2_replay_blocked"
        )
    ...
    body["scope"] = (
        "exact s23 metric nine-column complement leaf authority; no inferred zero, D2 "
...
def validate_campaign(value: Mapping[str, Any], *, root: Path | str | None = None) -> None:
    ...
    if value.get("content_sha256") != _content_sha(value) or value != expected:
        raise S23MetricComplementLeafAuthorityError("checked result changed")
