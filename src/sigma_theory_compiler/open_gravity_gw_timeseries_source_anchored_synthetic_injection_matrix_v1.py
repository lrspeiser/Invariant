"""Response-blind GW/time-series source-anchored synthetic injection matrix."""
...
CONFIG_PATH = Path(
    "configs/open_gravity_gw_timeseries_source_anchored_synthetic_injection_matrix_v1.json"
)
PARAMETER_SCHEMA_PATH = Path(
    "configs/open_gravity_gw_timeseries_source_anchored_synthetic_injection_matrix_v1.parameters.schema.json"
)
TEST_PATH = Path(
    "tests/test_open_gravity_gw_timeseries_source_anchored_synthetic_injection_matrix_v1.py"
)
OUTPUT_DIR = Path(
    "runs/gravity/open-gravity-gw-timeseries-source-anchored-synthetic-injection-matrix-v1"
)
...
_EXPECTED_CONFIG_CONTENT_SHA256 = "4b5821b74fc2fdfb58d9251e25441a69f84959f77a68bc04562ce56900485bf2"
_EXPECTED_CONFIG_RAW_SHA256 = "2857f4ca3e76337146f2bcc38323a413b1a0633b082744e0613b7cd8c573954b"
...
def validate_config(config: Mapping[str, Any], *, verify_hashes: bool = True) -> None:
    ...
    _require(
        ...
        and access["infrastructure_files_hashed"] == 10
        and access["infrastructure_bytes_hashed"] == 109_039,
...
def _catalogue(config: Mapping[str, Any]):
    ...
    return catalogue_from_elements("open-gravity-gw-timeseries-source-anchored", "v1.0.0", elements)
...
def _bindings(config: Mapping[str, Any]) -> tuple[FormulaExecutionBinding, ...]:
    ...
    for formula_id in sorted((*_ENTRYPOINTS, *_BLOCK_STATUS)):
        ...
        rows.append(
            FormulaExecutionBinding(
                binding_id=f"binding.gw-timeseries.{formula_id.lower().replace('_', '-')}.v1",
                formula_id=formula_id,
                formula_version="v1.0.0-source-anchored-synthetic",
                ...
                entrypoint=(
                    ...
                    "open_gravity_gw_timeseries_source_anchored_synthetic_injection_matrix_v1:"
                ...
                approximation_ceiling=(
                    "source-shaped synthetic dual-domain network; no real strain, PSD payload, calibration archive, or likelihood"
                    if executable
                    else block_reason[formula_id]
                ),
                health_gates=(
                    "determinism",
                    "dual-domain-consistency",
                    "finite-output",
                    "limit",
                    "source-hash",
                    "typed-output",
...
def _finish_prediction(
    ...
    spectrum = np.asarray(frequency_positive, dtype=np.complex128) * _calibration_complex(features)
...
def _noise_response(
    ...
    time_sigma = np.asarray(values["source.vector.time-noise-sigma"], dtype=np.float64)
    conditional_sigma = np.asarray(
    ...
    for output_id in _OUTPUTS:
        value = np.asarray(truth[output_id], dtype=np.float64)
        if "frequency-" in output_id:
            sigma = frequency_sigma
            mask = active[:, None]
        elif output_id == "prediction.matrix.time-strain":
            sigma = np.repeat(time_sigma[:, None], value.shape[1], axis=1)
            mask = active[:, None]
        else:
            sigma = np.repeat(conditional_sigma[:, None], value.shape[1], axis=1)
            mask = active[:, None]
        if family == "zero-noise" or output_id == "prediction.matrix.conditional-variance":
            response = np.array(value, copy=True)
        else:
            response = value + rng.normal(size=value.shape) * sigma * mask
        responses[output_id] = np.asarray(response, dtype=np.float64)
        variances[output_id] = np.square(sigma, dtype=np.float64)
...
def _diagnostics(
    ...
    for slot in slots:
        for formula_id in config["truth_mechanisms"]:
            ...
            reconstructed = np.fft.irfft(
                np.concatenate([np.zeros((3, 1), dtype=np.complex128), spectrum], axis=1),
                n=256,
                axis=1,
            )
            fft_error = max(
                fft_error,
                float(np.max(np.abs(reconstructed - prediction["prediction.matrix.time-strain"]))),
            )
    ...
    return {
        "schema": "open-gravity-gw-timeseries-invariance-degeneracy-failure-ledger-1.0",
...
def derive_release() -> tuple[dict[str, Any], bytes, bytes, bytes, bytes, bytes, bytes]:
    ...
    registrations = tuple(
        AdapterRegistration.create(f"adapter.gw-timeseries.{row.formula_id.lower()}.v1", row)
    ...
    for slot in slots:
        for truth_index, truth_formula_id in enumerate(config["truth_mechanisms"]):
            ...
            scenario_id = f"gw.{slot['slot_id']}.{truth_formula_id.lower()}.v1"
            lineage = SeedLineage(
                config["suite_seed"],
                scenario_id,
                slot["slot_id"],
            ...
            response_runtime = {
                _RESPONSE_FOR[output_id]: responses[output_id] for output_id in _OUTPUTS
            }
            uncertainty_runtime = {
    ...
    result = run_discovery_matrix_v2(
        ...
        ledger_id="gravity.synthetic.gw-timeseries-source-anchored-matrix.v1",
    )
    ...
    diagnostics = _diagnostics(config, inventory, slots, predictions, numerical_invalid_count)
    _require(diagnostics["pass"], "GW invariant/limit gates failed")
    confusion = {
        "schema": "open-gravity-gw-timeseries-confusion-matrix-1.0",
    ...
    receipt_body = {
        "schema": "open-gravity-gw-timeseries-source-anchored-synthetic-injection-matrix-receipt-1.0",
