...
def _validate_config(config: dict[str, Any]) -> None:
    if (
        ...
        != "project_first_K55_failure_onto_every_equal_eigenspace_before_any_correction"
        or set(config.get("upstreams", {}))
        != {"K55_frontier", "flat_P55", "flat_action_metric", "projector_recipes"}
...
def build_campaign(project_root: Path, config_path: Path) -> dict[str, Any]:
    ...
    for eigenvalue, projector in projectors.items():
        ...
        if block:
            packet = poly._packet(f"Pi_{eigenvalue}^T_R3_Pi_{eigenvalue}", block, [55, 55])
        else:
            empty_body = {
        ...
        diagonal_packets.append(
            {
                "eigenvalue": str(eigenvalue),
                "nonzero_polynomial_entries": len(block),
                "packet": packet,
            }
        )
    correction: Sparse = {}
    ...
    for left, left_projector in projectors.items():
        for right, right_projector in projectors.items():
            if left == right:
                continue
            block = poly._multiply(
                poly._multiply(poly._transpose(left_projector), residual), right_projector
            )
    ...
    body = {
        ...
        "counts": {
            ...
            "equal_plus_one_projection_nonzero_polynomial_entries": len(
                obstruction_blocks[Fraction(1)]
            ),
            "equal_minus_one_projection_nonzero_polynomial_entries": len(
                obstruction_blocks[Fraction(-1)]
            ),
        ...
        "negative_controls": {
            "treat_equal_eigenspace_block_as_divisible_by_zero_gap": {"rejected": True},
            "drop_physical_projection": {"rejected": True},
            "count_zero_unequal_gap_correction_as_solution": {"rejected": True},
            "advance_manifest_after_obstruction": {"rejected": True},
        },
        "source_bindings": {
