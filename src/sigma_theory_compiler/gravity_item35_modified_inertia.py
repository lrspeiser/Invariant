"""Frozen Item 35 action-level modified-inertia search."""
...
from sigma_theory_compiler.gravity_item34_condensate_superfluid import (
    _fresh_pool as _item34_fresh_pool,
)
from sigma_theory_compiler.gravity_item34_condensate_superfluid import (
    _robust_comparison,
)
from sigma_theory_compiler.gravity_item34_condensate_superfluid import (
    load_config as load_item34_config,
)

CONFIG_PATH = Path("configs/gravity_item35_modified_inertia_v1.json")
MODULE_PATH = Path("src/sigma_theory_compiler/gravity_item35_modified_inertia.py")
GOAL_PATH = Path("docs/GRAVITY_HIDDEN_VARIABLE_AND_THEORY_SEARCH_GOALS.md")
TEST_PATH = Path("tests/test_gravity_item35_modified_inertia.py")
_ADMISSIBLE_CACHE: dict[str, tuple[dict[str, np.ndarray], dict[str, Any]]] = {}


class GravityItem35Error(RuntimeError):
    """Raised when an Item 35 freeze, action, leakage, or replay invariant fails."""


def load_config(root: Path) -> dict[str, Any]:
    config = _read_json(root / CONFIG_PATH)
    expected = "invariant-gravity-item35-modified-inertia-config-1.0"
    if config.get("schema_version") != expected or int(config.get("item", -1)) != 35:
        raise GravityItem35Error("unexpected Item 35 config")
    if _sha256_file(root / GOAL_PATH) != str(config["stable_goal_sha256"]):
        raise GravityItem35Error("stable gravity goal changed")
    generator = config["candidate_generator"]
    if int(generator["raw_candidate_cells"]) != 262144:
        raise GravityItem35Error("raw candidate boundary changed")
    if int(generator["post_response_cells"]) != 0:
        raise GravityItem35Error("post-response candidates entered Item 35")
    if bool(config["scope"]["confirmation_opening_authorized"]):
        raise GravityItem35Error("confirmation opening is not authorized")
    if bool(config["scope"]["paid_api_calls_authorized"]):
        raise GravityItem35Error("paid calls are outside Item 35")
    if not bool(config["discovery_policy"]["equal_initial_viability"]):
        raise GravityItem35Error("equal-viability policy changed")
    if not bool(
        config["discovery_policy"]["single_empirical_counterexample_is_not_a_formula_family_veto"]
    ):
        raise GravityItem35Error("counterexample policy changed")
    if bool(config["gates"]["single_empirical_counterexample_is_veto"]):
        raise GravityItem35Error("empirical single-counterexample veto entered Item 35")
    if sum(bool(row["action_track_eligible"]) for row in generator["niches"]) != 3:
        raise GravityItem35Error("action eligibility allocation changed")
    for relative, expected_hash in config["scientific_dependencies"].items():
        if _sha256_file(root / str(relative)) != str(expected_hash):
            raise GravityItem35Error(f"scientific dependency changed: {relative}")
    return config
...
def verify_science_freeze(root: Path, config: Mapping[str, Any]) -> None:
    ...
    if _contract_digest(frozen) != _contract_digest(config):
        raise GravityItem35Error("scientific contract differs from frozen commit")
    ...
    if not isinstance(frozen_module, bytes) or _sha256_bytes(frozen_module) != _sha256_file(
        ...
        raise GravityItem35Error("Item 35 module differs from scientific freeze")
...
def verify_sample_freeze(root: Path, config: Mapping[str, Any]) -> None:
    ...
    for key in ("predictors", "predictor_source_manifest", "sample_manifest", "candidate_manifest"):
        ...
        if not isinstance(frozen, bytes) or _sha256_bytes(frozen) != _sha256_file(paths[key]):
            raise GravityItem35Error(f"{key} differs from sample freeze")
...
def verify_source_feature_freeze(root: Path, config: Mapping[str, Any]) -> None:
    ...
    for key in ("source_features", "source_feature_manifest"):
        ...
        frozen = _git(root, "show", f"{commit}:{relative}", text_mode=False)
        if not isinstance(frozen, bytes) or _sha256_bytes(frozen) != _sha256_file(paths[key]):
            raise GravityItem35Error(f"{key} differs from source feature freeze")
...
def generate_raw_candidates(config: Mapping[str, Any]) -> dict[str, np.ndarray]:
    ...
    if int(np.prod(list(radices.values()))) != per_niche:
        raise GravityItem35Error("mixed-radix grammar does not fill each niche exactly")
    ...
    for niche in range(4):
        ...
        for key, radix in reversed(list(radices.items())):
            ...
            working //= radix
        if np.any(working != 0):
            raise GravityItem35Error("candidate decoder overflow")
...
def _action_kernel(
    ...
    omega = xp.asarray(predictors["omega_Gyr_inverse"])[None, :]
    ...
    age_gyr = xp.asarray(predictors["age_gyr_proxy"])[None, :]
    vertical_ratio = xp.asarray(predictors["vertical_to_orbital_frequency"])[None, :]
    ...
    algebraic = xp.clip(common * (1.0 + coupling * source_eta) / (1.0 + coupling), 0.0, 1.0)
    algebraic *= xp.power(1.0 - 0.25 * latent * xp.abs(axis_ratio - 0.6), power)

    spectral_circular = 1.0 / (1.0 + xp.power(xp.maximum(omega * scale, 1e-12), power))
    spectral_mode = 1.0 / (
        1.0
        + xp.power(
            xp.maximum(omega * scale * (1.0 + coupling * (mode_frequency - 1.0)), 1e-12),
            power,
        )
    )
    spectral_circular = xp.power(spectral_circular, 1.0 / (1.0 + latent))
    spectral_mode = xp.power(spectral_mode, 1.0 / (1.0 + latent))

    maturity = 1.0 - xp.exp(-xp.power(xp.maximum(age_gyr / xp.maximum(scale, 1e-12), 1e-12), power))
    history_circular = common * maturity * spectral_circular
    history_mode = common * maturity * spectral_mode

    tensor_coordinate = xp.log10(xp.maximum(vertical_ratio / scale, 1e-12)) / width
    tensor_window = xp.exp(-xp.power(xp.abs(tensor_coordinate), power))
    tensor_circular = common * tensor_window * xp.power(1.0 - 0.25 * latent * source_eta, power)
    tensor_mode = xp.clip(
        common
        * tensor_window
        * (1.0 + coupling * source_eta * (mode_frequency - 1.0))
        / (1.0 + 2.0 * coupling),
        0.0,
        1.0,
    )
    ...
    circular_activation = xp.where(
        ...
        xp.where(
            niche == 1,
            common * spectral_circular,
            xp.where(niche == 2, history_circular, tensor_circular),
        ),
    )
    mode_activation = xp.where(
        ...
        xp.where(
            niche == 1,
            common * spectral_mode,
            xp.where(niche == 2, history_mode, tensor_mode),
        ),
    )
    kinetic_circular = 1.0 + polarity * amplitude * circular_activation
    kinetic_mode = 1.0 + polarity * amplitude * mode_activation
    return kinetic_circular, kinetic_mode, circular_activation, mode_activation
...
def _candidate_deltas(
    ...
    kinetic_circular, kinetic_mode, _, _ = _action_kernel(
        config, arrays, predictors, begin, end, xp
    )
    source_eta = xp.asarray(predictors["source_nonaxisymmetry"])[None, :]
    circular = -0.5 * xp.log10(kinetic_circular)
    ratio = kinetic_circular / kinetic_mode
...
def _adversarial_predictors() -> dict[str, np.ndarray]:
    ...
    omega = np.geomspace(0.1, 300.0, 64)
    source = 0.01 + 0.8 * ((index * 17) % 64) / 63.0
    mode = 1.0 + 2.0 * ((index * 29) % 64) / 63.0
    age = 0.1 + 9.9 * ((index * 37) % 64) / 63.0
    vertical = 0.1 + 5.0 * ((index * 43) % 64) / 63.0
    return {
        "log_acceleration": log_acceleration,
        "omega_Gyr_inverse": omega,
        "source_nonaxisymmetry": source,
        "mode_frequency_ratio": mode,
        "age_gyr_proxy": age,
        "vertical_to_orbital_frequency": vertical,
        "axis_ratio": 0.3 + 0.55 * ((index * 47) % 64) / 63.0,
    }
...
def _local_predictors(config: Mapping[str, Any]) -> dict[str, np.ndarray]:
    return {
        ...
        "omega_Gyr_inverse": np.asarray([2.0e8]),
        "source_nonaxisymmetry": np.asarray([0.01]),
        "mode_frequency_ratio": np.asarray([2.0]),
        "age_gyr_proxy": np.asarray([4.5]),
        "vertical_to_orbital_frequency": np.asarray([1.0]),
...
def _admissible_candidates(
    ...
    minimum_kinetic = np.full(count, np.nan)
    maximum_kinetic = np.full(count, np.nan)
    ...
    for begin in range(0, count, batch):
        ...
        kinetic_circular, kinetic_mode, _, _ = _action_kernel(config, raw, domain, begin, end, np)
        ...
        minimum_kinetic[begin:end] = np.minimum(
            np.min(kinetic_circular, axis=1), np.min(kinetic_mode, axis=1)
        )
        maximum_kinetic[begin:end] = np.maximum(
            np.max(kinetic_circular, axis=1), np.max(kinetic_mode, axis=1)
        )
        ...
        keep[begin:end] = (
            ...
            & (minimum_kinetic[begin:end] >= float(gates["minimum_kinetic_eigenvalue"]))
            & (maximum_kinetic[begin:end] <= float(gates["maximum_kinetic_eigenvalue"]))
            & (
                material[begin:end]
            ...
            & (
                ...
                <= float(gates["maximum_local_fractional_inertia_response"])
            )
            & (
    ...
    audit = {
        ...
        "minimum_admitted_kinetic_eigenvalue": float(np.min(minimum_kinetic[keep])),
        "maximum_admitted_kinetic_eigenvalue": float(np.max(maximum_kinetic[keep])),
        "minimum_admitted_material_response": float(np.min(material[keep])),
        "maximum_admitted_local_fractional_inertia_response": float(np.max(local_response[keep])),
    ...
    for expected_key, observed_key in checks:
        ...
        if expected not in (None, "TO_BE_MEASURED", -1) and audit[observed_key] != expected:
            raise GravityItem35Error(f"candidate invariant changed: {expected_key}")
    ...
    if (
        ...
        raise GravityItem35Error("admissible niche counts changed")
...
def _candidate_manifest(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return _content_hashed(
        {
            "schema_version": "invariant-gravity-item35-candidate-manifest-1.0",
...
def _fresh_pool(
    ...
    item34_config = load_item34_config(root)
    inherited, inherited_audit = _item34_fresh_pool(root, item34_config)
    independence = config["independence"]
    if len(inherited) != int(independence["expected_item34_fresh_pool"]):
        raise GravityItem35Error("Item 34 fresh pool changed")
    item34_sample = _read_json(root / str(config["sources"]["item34_sample_manifest"]))
    _verify_content_hash(item34_sample, "Item 34 sample manifest")
    roles = item34_sample["objects"]
    if len(roles) != int(independence["expected_item34_roles"]):
        raise GravityItem35Error("Item 34 role count changed")
    ...
    if len(post_identity) != int(independence["expected_post_identity_pool"]):
        raise GravityItem35Error("Item 35 identity exclusion count changed")
    ...
    if excluded_coordinates != int(independence["expected_additional_coordinate_exclusions"]):
        raise GravityItem35Error("Item 35 coordinate exclusion count changed")
    fresh = []
    for source, separation in zip(post_identity, separations, strict=True):
        if separation <= veto:
            continue
        row = dict(source)
        row["minimum_item34_role_separation_arcsec"] = float(separation)
        fresh.append(row)
    if len(fresh) != int(independence["expected_fresh_before_disk_quality"]):
        raise GravityItem35Error("Item 35 fresh pre-quality count changed")
    sample = config["sample"]
    pool = [
        row
        for row in fresh
        if float(row["snr_med_g"]) >= float(sample["predictor_minimum_snr_med_g"])
    ...
    if len(pool) != int(sample["expected_fresh_disk_pool"]):
        raise GravityItem35Error("Item 35 disk predictor pool changed")
    audit = {
        "item34_fresh_pool": len(inherited),
        "item34_predecessor_audit": inherited_audit,
        "item34_roles": len(roles),
        "post_identity_pool": len(post_identity),
        "additional_coordinate_exclusions": excluded_coordinates,
        "fresh_before_disk_quality": len(fresh),
...
def _sample_manifest(
    ...
    if capacities != expected:
        raise GravityItem35Error("Item 35 response-blind cell capacities changed")
    objects = []
    cell_counts = {}
    selected_count = int(sample["selected_per_mass_sersic_cell"])
    ...
    return _content_hashed(
        {
            "schema_version": "invariant-gravity-item35-sample-manifest-1.0",
            "scientific_freeze_commit": config["scientific_freeze_commit"],
...
def prepare_predictors(root: Path) -> dict[str, Path]:
    ...
    if sample_manifest["counts"] != expected_counts:
        raise GravityItem35Error("frozen Item 35 sample counts changed")
    columns = [
        *list(pool[0]),
        "mass_bin",
        "sersic_bin",
    ...
    predictor_manifest = _content_hashed(
        {
            "schema_version": "invariant-gravity-item35-predictor-source-manifest-1.0",
            "scientific_freeze_commit": config["scientific_freeze_commit"],
            "item34_sample": {
                "path": config["sources"]["item34_sample_manifest"],
                "sha256": _sha256_file(root / str(config["sources"]["item34_sample_manifest"])),
            },
    ...
    try:
        ...
        for key, expected in source["required_primary_headers"].items():
            if str(primary.get(key, "")).strip() != str(expected):
                hdus.close()
                raise GravityItem35Error(f"MaNGA MAPS header {key} changed")
        if str(primary.get("PLATEIFU", "")).strip() != str(sample_row["plateifu"]):
            hdus.close()
            raise GravityItem35Error("MaNGA MAPS plateifu changed")
        if str(primary.get("MANGAID", "")).strip().upper() != str(sample_row["mangaid"]).upper():
            hdus.close()
            raise GravityItem35Error("MaNGA MAPS mangaid changed")
        ...
        if bool(source["fits_checksum_required"]):
            for hdu in [hdus[0], *required]:
                if hdu.verify_checksum() != 1 or hdu.verify_datasum() != 1:
                    hdus.close()
                    raise GravityItem35Error("MaNGA MAPS FITS checksum failed")
        return hdus, primary
    except GravityItem35Error:
        raise
    except (OSError, ValueError, TypeError, IndexError, KeyError, AttributeError) as exc:
        raise GravityItem35Error("invalid MaNGA MAPS FITS") from exc
...
def _source_feature_rows(
    ...
    if not np.any(domain):
        raise GravityItem35Error("no valid continuum source domain")
    ...
    try:
        ...
        raise GravityItem35Error("Item 35 requires scipy for the frozen Sersic source") from exc
    ...
    for annulus_index, (label, bounds) in enumerate(
        ...
        if count < int(source["minimum_source_pixels_per_annulus"]):
            raise GravityItem35Error(f"insufficient continuum source pixels in {label}")
...
def _source_control_spec(
    ...
    probabilities = [float(value) for value in config["evaluation"]["frequency_control_quantiles"]]
    output = {}
    for label in labels:
...
def acquire_source_maps(root: Path) -> dict[str, Path]:
    ...
    _verify_content_hash(sample_manifest, "Item 35 sample manifest")
    exploration = sorted(
    ...
    for sample_row in exploration:
        ...
        if identity in confirmations:
            raise GravityItem35Error("confirmation identity entered source-map access")
        try:
            ...
            rows = _source_feature_rows(payload, sample_row, config)
        except (GravityItem32Error, GravityItem35Error) as exc:
            failures.append({"plateifu": identity, "reason": str(exc)})
            continue
        source_rows.extend(rows)
        files.append({**file_record, "source_extension_checksums_verified": True})
    if not source_rows:
        raise GravityItem35Error("no Item 35 source-only MAPS features were extracted")
    touched = {str(row["plateifu"]) for row in [*files, *failures]}
    if confirmations & touched:
        raise GravityItem35Error("confirmation MAPS entered Item 35 source phase")
    ...
    manifest = _content_hashed(
        {
            "schema_version": "invariant-gravity-item35-source-feature-manifest-1.0",
            "scientific_freeze_commit": config["scientific_freeze_commit"],
            ...
            "ordinary_frequency_control_spec": _source_control_spec(source_rows, config),
            ...
            "claims": {
                ...
                "frequency_thresholds_frozen_before_response": True,
...
def _harmonic_fit_order_three(
    ...
    if count < minimum_count:
        raise GravityItem35Error(f"insufficient {channel} measurements in {annulus}")
    azimuth = azimuth_degrees[selected]
    quadrants = _quadrant_count(azimuth)
    if quadrants < minimum_quadrants:
        raise GravityItem35Error(f"insufficient {channel} azimuth coverage in {annulus}")
    ...
    if not math.isfinite(condition) or condition > maximum_condition:
        raise GravityItem35Error(f"ill-conditioned {channel} harmonic fit in {annulus}")
    ...
    if circular_los < minimum_circular_amplitude:
        raise GravityItem35Error(f"weak {channel} circular amplitude in {annulus}")
    ...
    if not math.isfinite(ratio) or ratio > maximum_noncircular_ratio:
        raise GravityItem35Error(f"extreme {channel} noncircular ratio in {annulus}")
...
def _response_annuli(
    ...
    except GravityItem35Error as exc:
...
def acquire_responses(root: Path) -> dict[str, Path]:
    ...
    _verify_content_hash(source_manifest, "Item 35 source feature manifest")
    if int(source_manifest["counts"]["velocity_arrays_read"]) != 0:
        raise GravityItem35Error("velocity response entered source-feature freeze")
    sample_manifest = _read_json(paths["sample_manifest"])
    _verify_content_hash(sample_manifest, "Item 35 sample manifest")
    sample_by_id = {
    ...
    for file_record in source_manifest["files"]:
        ...
        if identity in confirmation or identity not in sample_by_id:
            raise GravityItem35Error("nonexploration identity entered response access")
        path = cache / str(file_record["file_name"])
        if not path.is_file():
            raise GravityItem35Error(f"source-frozen MAPS payload missing: {identity}")
        payload = path.read_bytes()
        digest = hashlib.sha256(payload).hexdigest()
        if digest != str(file_record["file_sha256"]):
            raise GravityItem35Error(f"source-frozen MAPS checksum changed: {identity}")
        try:
    ...
    responses = _content_hashed(
        {
            "schema_version": "invariant-gravity-item35-exploration-responses-1.0",
            "scientific_freeze_commit": config["scientific_freeze_commit"],
            "sample_freeze_commit": config["sample_freeze_commit"],
    ...
    response_manifest = _content_hashed(
        {
            "schema_version": "invariant-gravity-item35-response-source-manifest-1.0",
            "scientific_freeze_commit": config["scientific_freeze_commit"],
            "sample_freeze_commit": config["sample_freeze_commit"],
            "source_feature_freeze_commit": config["source_feature_freeze_commit"],
...
def _load_experiment_data(
    ...
    for key in (
        ...
        _verify_content_hash(value, f"Item 35 {key}")
    source_manifest = _read_json(paths["source_feature_manifest"])
    responses = _read_json(paths["exploration_responses"])
    if int(source_manifest["counts"]["velocity_arrays_read"]) != 0:
        raise GravityItem35Error("response leaked into source feature manifest")
    if int(responses["counts"]["confirmation_response_rows"]) != 0:
        raise GravityItem35Error("confirmation response entered Item 35")
    ...
    for identity in sorted(response_by_id):
        ...
        if bool(response["Halpha_quality_pass"]):
            for annulus_index, annulus in enumerate(response["Halpha_annuli"]):
                ...
                if key not in feature_by_key:
                    raise GravityItem35Error("Halpha response lacks frozen source feature")
                primary_rows.append(
                    {
                        **feature_by_key[key],
        ...
        if bool(response["Halpha_quality_pass"]) and bool(response["stellar_quality_pass"]):
            for annulus_index, annulus in enumerate(response["stellar_annuli"]):
                key = (identity, annulus_index)
                if key not in feature_by_key:
                    raise GravityItem35Error("stellar response lacks frozen source feature")
...
def _screen_joint_candidates(
    ...
    if {int(value) for value in folds} != set(range(outer_folds)):
        raise GravityItem35Error("response-complete folds are incomplete")
...
def _baseline_designs(
    ...
    spec = source_manifest["ordinary_frequency_control_spec"]
    smooth = []
    hinges = []
    for label in spec:
        values = np.asarray([float(row[label]) for row in rows], dtype=np.float64)
    ...
    frequency = np.column_stack([flexible, *smooth, *hinges])
    return {"structural": structural, "flexible": flexible, "ordinary_frequency": frequency}
...
def _baseline_predictions(
    ...
    alpha = {
        ...
        "ordinary_frequency": float(config["evaluation"]["ridge_alpha_frequency"]),
    }
...
def _candidate_record(
    ...
    return {
        ...
        "shared_acceleration_frequency_scale": float(values["scale"][0]),
...
def _transfer_selected_candidates(
    ...
    for fold in range(int(config["sample"]["outer_folds"])):
        ...
        if not len(train) or not len(held):
            raise GravityItem35Error("stellar transfer folds are incomplete")
...
def _slice_results(
    ...
    definitions = {
        ...
        "sersic_bin": [str(row["sersic_bin"]) for row in rows],
    }
    output = []
            ...
            output.append(
                {
                    ...
                    "improvement_vs_ordinary_frequency": _improvement(
                        reference_mse["ordinary_frequency"], candidate_mse
                    ),
                }
            )
    return output
...
def _synthetic_controls(
    ...
    for niche in range(4):
        ...
        if not len(indices):
            raise GravityItem35Error(f"missing synthetic niche {niche}")
    ...
    return {
        ...
        "Newtonian_candidate_mse": newtonian_candidate_mse,
        "Newtonian_baseline_mse": newtonian_baseline_mse,
        "Newtonian_control_candidate_improves": (
...
def _evaluate(
    ...
    if len(primary_rows) < 2 * int(config["sample"]["outer_folds"]):
        raise GravityItem35Error("too few Halpha response-complete observations")
    ...
    observed_statistic = improvements["ordinary_frequency"]
    ...
    for _ in range(int(config["evaluation"]["permutation_trials"])):
        null_target = _permuted_joint_target(
            target, baselines["ordinary_frequency"], primary_rows, random
        )
        ...
        null_improvements.append(
            _improvement(
                _joint_mse(null_target, null_baselines["ordinary_frequency"]),
    ...
    robust_observations = _robust_comparison(
        ...
        baselines["ordinary_frequency"].reshape(-1),
        flat_rows,
        config,
    )
    robust_galaxies = _robust_joint_by_galaxy(
        target,
        observed["prediction"],
        baselines["ordinary_frequency"],
        primary_rows,
        config,
    )
    transfer_matrix_target = transfer["target"][:, None]
    transfer_matrix_candidate = transfer["prediction"][:, None]
    transfer_matrix_reference = transfer_baselines["ordinary_frequency"][:, None]
    ...
    universal_checks = {
        ...
        "improvement_vs_ordinary_frequency": improvements["ordinary_frequency"]
        >= float(gates["minimum_improvement_vs_ordinary_frequency"]),
        "all_broad_slices_vs_baryonic": min(broad_baryonic)
        >= float(gates["minimum_each_broad_slice_improvement_vs_baryonic_source"]),
        "stellar_transfer_vs_ordinary_frequency": transfer_improvements["ordinary_frequency"]
        >= float(gates["minimum_stellar_transfer_improvement_vs_ordinary_frequency"]),
    ...
    phenomenon_checks = {
        "quality": quality["all_pass"],
        "improvement_vs_ordinary_frequency": improvements["ordinary_frequency"]
        >= float(gates["phenomenon_minimum_improvement_vs_ordinary_frequency"]),
        "stellar_transfer": transfer_improvements["ordinary_frequency"]
    ...
    partial_slices = [
        row
        for row in slices
        if float(row["improvement_vs_ordinary_frequency"])
        >= float(gates["partial_minimum_slice_improvement_vs_ordinary_frequency"])
    ]
    ...
    if not quality["all_pass"]:
        decision = "INCONCLUSIVE_ITEM35_RESPONSE_QUALITY"
    elif universal_pass:
        decision = "ADVANCE_ITEM35_MODIFIED_INERTIA_CANDIDATE"
    elif phenomenon_pass:
        decision = "RETAIN_ITEM35_NONCIRCULAR_PHENOMENON_SIGNAL"
    elif partial_slices:
        decision = "RETAIN_ITEM35_PARTIAL_SLICE_SIGNAL"
    else:
        decision = "NO_ITEM35_MODIFIED_INERTIA_LEAD"
    result = _content_hashed(
        {
            "schema_version": "invariant-gravity-item35-modified-inertia-result-1.0",
            "item": 35,
            "title": config["title"],
            "decision": decision,
            ...
            "action_and_ontology": {
                ...
                "positive_kinetic_spectrum_all_admitted": (
                    candidate_audit["minimum_admitted_kinetic_eigenvalue"]
                    >= float(config["admissibility"]["minimum_kinetic_eigenvalue"])
                ),
                "maximum_local_fractional_inertia_response": candidate_audit[
                    "maximum_admitted_local_fractional_inertia_response"
                ],
                "real_even_frequency_kernel": True,
                "mass_and_composition_independent_kernel": True,
                "quadratic_conservation_proxy_only": True,
                "relativistic_completion_proved": False,
                "causal_initial_value_formulation_proved": False,
                "historical_novelty_claimed": False,
            },
            ...
            "next_step": (
                ...
                "Advance the ordered roadmap to Item 36 after recording this exploration."
            ),
        }
    )
    compute_manifest = _content_hashed(
        {
            "schema_version": "invariant-gravity-item35-compute-manifest-1.0",
...
def validate_checked(root: Path) -> None:
    ...
    for key in (
        ...
        _verify_content_hash(manifest, f"Item 35 {key}")
    result_path = root / str(config["paths"]["result"])
    result = _read_json(result_path)
    _verify_content_hash(result, "Item 35 result")
    if result.get("schema_version") != "invariant-gravity-item35-modified-inertia-result-1.0":
        raise GravityItem35Error("unexpected Item 35 result schema")
    if int(result.get("item", -1)) != 35:
        raise GravityItem35Error("unexpected Item 35 result item")
    allowed = {
        "INCONCLUSIVE_ITEM35_RESPONSE_QUALITY",
        "ADVANCE_ITEM35_MODIFIED_INERTIA_CANDIDATE",
        "RETAIN_ITEM35_NONCIRCULAR_PHENOMENON_SIGNAL",
        "RETAIN_ITEM35_PARTIAL_SLICE_SIGNAL",
        "NO_ITEM35_MODIFIED_INERTIA_LEAD",
    }
    if result.get("decision") not in allowed:
        raise GravityItem35Error("unexpected Item 35 decision")
    if int(result["sample"]["confirmation_values_read"]) != 0:
        raise GravityItem35Error("confirmation value entered Item 35 result")
    if bool(result["counterexample_policy"]["single_empirical_counterexample_is_veto"]):
        raise GravityItem35Error("one-object empirical veto entered Item 35 result")
    if bool(result["counterexample_policy"]["formula_family_rejected_from_one_empirical_object"]):
        raise GravityItem35Error("formula family rejected from one empirical object")
    if bool(result["paper_claim_authorized"]):
        raise GravityItem35Error("unreplicated paper claim entered Item 35")
    if int(result["compute"]["paid_model_calls"]) != 0:
        raise GravityItem35Error("paid model call entered Item 35")
    if int(result["compute"]["confirmation_calls"]) != 0:
        raise GravityItem35Error("confirmation computation entered Item 35")
    if float(result["compute"]["cpu_gpu_max_absolute_difference"]) > float(
        result["compute"]["cpu_gpu_tolerance"]
    ):
        raise GravityItem35Error("Item 35 CPU/GPU crosscheck failed")
    sample_manifest = _read_json(paths["sample_manifest"])
    if sample_manifest["counts"] != {
        ...
        raise GravityItem35Error("Item 35 sample counts changed")
    source_manifest = _read_json(paths["source_feature_manifest"])
    if int(source_manifest["counts"]["response_extensions_read"]) != 0:
        raise GravityItem35Error("response extension entered source freeze")
    if int(source_manifest["counts"]["confirmation_maps_downloaded"]) != 0:
        raise GravityItem35Error("confirmation source map entered Item 35")
    responses = _read_json(paths["exploration_responses"])
    if int(responses["counts"]["confirmation_response_rows"]) != 0:
        raise GravityItem35Error("confirmation response entered Item 35")
    compute = _read_json(paths["compute_manifest"])
    if str(compute["result_content_sha256"]) != str(result["content_sha256"]):
        raise GravityItem35Error("compute manifest does not bind Item 35 result")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if arguments.command == "prepare-predictors":
        ...
        raise GravityItem35Error("unknown Item 35 command")
    return 0
