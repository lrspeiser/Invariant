"""Complete s01 metric A/B/C leaf authority with its nine-column complement."""
...
from . import quartic_s03_metric_abc_leaf_authority_gate as engine
...
CONFIG_SCHEMA = "sigma-quartic-s01-metric-complement-abc-leaf-authority-config-1.0"
RESULT_SCHEMA = "sigma-quartic-s01-metric-complement-abc-leaf-authority-gate-1.0"
CAMPAIGN_ID = "quartic-s01-metric-complement-abc-leaf-authority-001"
STEM = "quartic_s01_metric_complement_abc_leaf_authority_gate"
SLUG = "quartic-s01-metric-complement-abc-leaf-authority-gate"
...
CONFIG_PRODUCTION_SHA256 = "6975d30fcda32815327166cf2a4f66319bc5ee557666a8f59df184a6dc4e2e98"
BLOCK_SHA256 = engine.BLOCK_SHA256
S01_FULL = tuple((f"s01[{field}]", 54 + field) for field in range(10))
S01_EXISTING = "s01[1]"
S01_NEW = tuple(row for row in S01_FULL if row[0] != S01_EXISTING)
CONTRACT = {
    "candidate_count": 12,
    "previous_missing_coordinate_columns": 113,
    "new_s01_metric_columns": 9,
    "previous_s01_metric_columns": 1,
...
POLICIES = {
    "family_selection": "complete_s01_metric_family_by_exact_nine_column_complement",
...
class S01MetricComplementLeafAuthorityError(ValueError):
    """An s01 complement projection, root, or binding changed."""


def _inside(root: Path, relative: str) -> Path:
    if not relative or "\\" in relative:
        raise S01MetricComplementLeafAuthorityError("s01 path is not portable")
    path = (root / relative).resolve()
    if path != root and root not in path.parents:
        raise S01MetricComplementLeafAuthorityError("s01 path escapes root")
    return path


def _validate_config(value: Mapping[str, Any], path: Path) -> None:
    if _production_sha(path) != CONFIG_PRODUCTION_SHA256:
        raise S01MetricComplementLeafAuthorityError("config production bytes changed")
    ...
    if (
        ...
        or set(predecessor)
        != {"stem", "slug", "source_sha256", "config_sha256", "test_sha256", "content_sha256"}
        or any(
            not re.fullmatch(r"[0-9a-f]{64}", predecessor[key])
            for key in predecessor
            if key.endswith("sha256")
        )
    ):
        raise S01MetricComplementLeafAuthorityError("config contract changed")
...
def _load_predecessor(root: Path, bundle: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for key, relative in {
        ...
        if not _matches_text_authority(_inside(root, relative), str(bundle[key])):
            raise S01MetricComplementLeafAuthorityError("predecessor text changed")
    value = json.loads(
    ...
    if value.get("content_sha256") != bundle["content_sha256"] or value.get(
        ...
        raise S01MetricComplementLeafAuthorityError("predecessor receipt changed")
    if (
        value.get("decision")
        != "pass_complete_s03_metric_family_316800_exact_leaf_roots_D2_blocked"
        or value.get("gate_counts", {}).get(
            "remaining_coordinate_columns_without_A_B_C_leaf_authority"
        )
        != 113
    ):
        raise S01MetricComplementLeafAuthorityError("predecessor boundary changed")
    return value
...
def _projection_packets(
    ...
    packets, tangents = [], {}
    for atom, column in S01_FULL:
        row = by_atom[atom]
        if (
            row.get("coordinate_column") != column
            or row.get("derivative_pair") != [0, 1]
            or row.get("theorem")
            != "arbitrary_inverse_metric_Einstein_principal_second_jet_formula"
        ):
            raise S01MetricComplementLeafAuthorityError("s01 projection changed")
        tangent = {
        ...
        body = {
            ...
            "already_registered_leaf_authority": atom == S01_EXISTING,
...
def _filtered_manifests(
    ...
    original = engine.S03_METRIC
    engine.S03_METRIC = S01_FULL
    engine._generic_packets.cache_clear()
    try:
        manifests, dag = engine._candidate_manifests(coefficients, targets, tangents, inverse)
    finally:
        engine.S03_METRIC = original
        engine._generic_packets.cache_clear()
    result = []
    for manifest in manifests:
        packets = [
            row for row in manifest["direction_packets"] if row["derivative_atom"] != S01_EXISTING
        ]
        nonzero = sum(row["nonzero_leaf_derivative_roots"] for row in packets)
        body = {
            **{key: manifest[key] for key in ("candidate_id", "coefficients", "target_atoms")},
            "derivative_coordinate_columns": [column for _, column in S01_NEW],
            "target_direction_pairs": 180,
            "leaf_derivative_roots": 23760,
            "nonzero_leaf_derivative_roots": nonzero,
            "exact_zero_leaf_derivative_roots": 23760 - nonzero,
            "direction_packets": packets,
            "candidate_decision": "pass_s01_metric_complement_leaf_authority_D2_replay_blocked",
...
def _expected_body(
    root: Path, config_path: Path, config: Mapping[str, Any], predecessor: Mapping[str, Any]
) -> dict[str, Any]:
    projection_bundle = predecessor["source_bindings"]["evidence"]["principal_projection"]
    projection = engine._load_bundle(root, projection_bundle)
    data = _unspecialized_principal_blocks()["data"]
    if _unspecialized_principal_blocks()["content_sha256"] != BLOCK_SHA256:
        raise S01MetricComplementLeafAuthorityError("live block changed")
    ...
    if S01_EXISTING not in targets:
        raise S01MetricComplementLeafAuthorityError("existing s01 authority missing")
    ...
    body = {
        ...
        "decision": "pass_s01_metric_nine_column_complement_285120_exact_leaf_roots_D2_blocked",
        ...
        "exact_controls": {
            "previous_s01_atom_not_recounted": S01_EXISTING,
        ...
        "gate_counts": {
            ...
            "previous_registered_coordinate_columns": 40,
            "previous_missing_coordinate_columns": 113,
            "previous_s01_metric_columns": 1,
            "new_s01_metric_coordinate_columns": 9,
            "complete_s01_metric_family_columns": 10,
            "registered_coordinate_columns_after": 49,
            "remaining_coordinate_columns_without_A_B_C_leaf_authority": 104,
        ...
        "claim_seals": {
            "complete_s01_metric_family_registered": True,
            "previous_s01_atom_not_recounted": True,
            "no_tensor_component_inferred_zero": True,
            "D2_entry_count_advanced": False,
            "remaining_104_coordinate_columns_registered": False,
            "complete_D2F": False,
            "global_H7": False,
        },
        ...
        "scope": "exact s01 metric nine-column complement leaf authority; no inferred zero, D2 advance, complete tensor, H7, rejection, or observation",
    }
    return body
...
def build_campaign(
    ...
    body = _expected_body(
        project_root, path, config, _load_predecessor(project_root, config["predecessor"])
    )
...
def validate_campaign(value: Mapping[str, Any], *, root: Path | str | None = None) -> None:
    ...
    if value.get("content_sha256") != _content_sha(value) or value != expected:
        raise S01MetricComplementLeafAuthorityError("checked result changed")
