"""Mutation-freeze-only Lane-9 CF4 x VAST executor contract v2.
...
CONFIG_PATH = REPO_ROOT / "configs/open_gravity_void_correlation_executor_contract_v2.json"
MODULE_PATH = REPO_ROOT / "src/sigma_theory_compiler/open_gravity_void_correlation_executor_contract_v2.py"
TEST_PATH = REPO_ROOT / "tests/test_open_gravity_void_correlation_executor_contract_v2.py"
OUTPUT_PATH = REPO_ROOT / "runs/gravity/open-gravity-void-correlation-executor-contract-v2/receipt.json"
_CONFIG_RAW_SHA256 = "2c90b8e3700e3b148da047de314f93b8a2991514a9541dd1b9b1bb415428e654"
_CONFIG_CONTENT_SHA256 = "2031c728692e20f3263beec900e18f1681504c537276d0cac114fdcbfc07bcbf"
_MODULE_SEMANTIC_SHA256 = "3a6b4628512dc60f14c850d603632638cbb58a2ffe851b1376fb89999c5a4d23"
_TEST_RAW_SHA256 = "302cd055b8748e431a5fc36a5e0267ff98af733dcb63dd4377b14a094de1e9e4"
...
class VoidExecutorV2Error(RuntimeError):
    """Raised when a frozen v2 invariant changes or a sealed action is attempted."""


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise VoidExecutorV2Error(message)
...
def canonical_bound_path(relative: str) -> Path:
    ...
    try:
        ...
        raise VoidExecutorV2Error("bound path escapes repository") from error
...
def load_config() -> dict[str, Any]:
    ...
    _require(value["status"] == "DRAFT_MUTATION_FREEZE_ROWS_UNOPENED_AWAIT_INDEPENDENT_AUDIT", "status drift")
    _require(value["decision"] == "MUTATION_FREEZE_REQUESTED_AWAIT_SEPARATE_AGENT_AUDIT_NO_ROW_ACCESS", "decision drift")
...
def shear_quadratic_columns(direction: Sequence[float]) -> tuple[float, float, float, float, float]:
    nx, ny, nz = normalize_direction(direction)
    return (
...
def nuisance_velocity_design(distance_mpc: float, direction: Sequence[float]) -> tuple[float, ...]:
    ...
    nx, ny, nz = normalize_direction(direction)
    q = shear_quadratic_columns((nx, ny, nz))
...
def validate_stage_counts(
    ...
    if stage in {"VALIDATION_OPEN", "CONFIRMATION_OPEN", "PANTHEON_OPEN"}:
        raise VoidExecutorV2Error(f"{stage} is not authorized by v2")
    raise VoidExecutorV2Error("unknown access stage")
...
def _ordered_inputs(
    ...
    direction_values = [normalize_direction(directions[index]) for index in order]
...
def profile_at_delta(
    ...
    delta, c_value = float(delta_h), float(c_km_s)
    _require(math.isfinite(delta) and math.isfinite(c_value) and c_value > 0.0, "invalid law parameter or c")
    ...
    rhs = [0.0] * 9
    for row in range(9):
        rhs[row] = math.fsum(weights[index] * design[index][row] * residual_without_nuisance[index] for index in range(len(ids)))
        for column in range(row + 1):
...
def profile_grid(
    ...
    profiles = [profile_at_delta(y, sigma, path_distances, directions, l_void, identifiers, delta) for delta in delta_h_grid()]
...
def _synthetic_profile_fixture() -> tuple[list[float], list[float], list[float], list[float], list[tuple[float, float, float]], list[float], list[int]]:
    ...
    directions = [normalize_direction((1.0, 0.1 * (index + 1), 0.05 * (index % 3 + 1))) for index in range(12)]
...
def build_receipt() -> dict[str, Any]:
    config = load_config()
    predecessor = _validate_bound_receipt(config["predecessor"], "PASS_FIXED_CONTRACT_ROWS_UNOPENED_AWAIT_INDEPENDENT_REAUDIT")
    ...
    receipt: dict[str, Any] = {
        "schema": "invariant-open-gravity-void-correlation-executor-contract-receipt-2.0",
        "package_id": config["package_id"],
        "status": "PASS_MUTATION_FREEZE_SYNTHETIC_CONFORMANCE_ROWS_UNOPENED_AWAIT_INDEPENDENT_AUDIT",
        ...
        "next_gate": "A_DIFFERENT_AGENT_MUST_AUDIT_THIS_EXACT_RECEIPT_AND_HASHES_BEFORE_IDS_PARTITIONED",
        "content_sha256": "",
    }
