"""Complete s02 metric A/B/C leaf authority with its nine-column complement."""
...
from . import quartic_s01_metric_complement_abc_leaf_authority_gate as prior
...
CONFIG_SCHEMA = "sigma-quartic-s02-metric-complement-abc-leaf-authority-config-1.0"
RESULT_SCHEMA = "sigma-quartic-s02-metric-complement-abc-leaf-authority-gate-1.0"
CAMPAIGN_ID = "quartic-s02-metric-complement-abc-leaf-authority-001"
STEM = "quartic_s02_metric_complement_abc_leaf_authority_gate"
SLUG = "quartic-s02-metric-complement-abc-leaf-authority-gate"
...
CONFIG_PRODUCTION_SHA256 = "eedceb4802c7fea7d57709d25bed486c066c66c64bf93201576353599acc7eb9"
S02_FULL = tuple((f"s02[{field}]", 65 + field) for field in range(10))
S02_EXISTING = "s02[2]"
S02_NEW = tuple(row for row in S02_FULL if row[0] != S02_EXISTING)
CONTRACT = {
    "candidate_count": 12,
    "previous_missing_coordinate_columns": 104,
    "new_s02_metric_columns": 9,
    "previous_s02_metric_columns": 1,
...
POLICIES = {
    "family_selection": "complete_s02_metric_family_by_exact_nine_column_complement",
...
class S02MetricComplementLeafAuthorityError(ValueError):
    """An s02 projection, exact root, or authority binding changed."""


def _inside(root: Path, relative: str) -> Path:
    if not relative or "\\" in relative:
        raise S02MetricComplementLeafAuthorityError("s02 path is not portable")
    path = (root / relative).resolve()
    if path != root and root not in path.parents:
        raise S02MetricComplementLeafAuthorityError("s02 path escapes root")
    return path


def _validate_config(value: Mapping[str, Any], path: Path) -> None:
    if _production_sha(path) != CONFIG_PRODUCTION_SHA256:
        raise S02MetricComplementLeafAuthorityError("config production bytes changed")
    if (
        ...
        raise S02MetricComplementLeafAuthorityError("config contract changed")
...
def _load_predecessor(root: Path, bundle: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for key, relative in {
        ...
        if not _matches_text_authority(_inside(root, relative), str(bundle[key])):
            raise S02MetricComplementLeafAuthorityError("predecessor text changed")
    value = json.loads(
    ...
    if value.get("content_sha256") != bundle["content_sha256"] or value.get(
        ...
        raise S02MetricComplementLeafAuthorityError("predecessor receipt changed")
    if (
        value.get("decision")
        != "pass_s01_metric_nine_column_complement_285120_exact_leaf_roots_D2_blocked"
        or value.get("gate_counts", {}).get(
            "remaining_coordinate_columns_without_A_B_C_leaf_authority"
        )
        != 104
    ):
        raise S02MetricComplementLeafAuthorityError("predecessor boundary changed")
    return value
...
def _projection_packets(
    ...
    packets, tangents = [], {}
    for atom, column in S02_FULL:
        row = by_atom[atom]
        if (
            row.get("coordinate_column") != column
            or row.get("derivative_pair") != [0, 2]
            or row.get("theorem")
            != "arbitrary_inverse_metric_Einstein_principal_second_jet_formula"
        ):
            raise S02MetricComplementLeafAuthorityError("s02 projection changed")
        tangent = {
        ...
        body = {
            ...
            "already_registered_leaf_authority": atom == S02_EXISTING,
            "projection_content_sha256": row["content_sha256"],
        }
        packets.append({**body, "content_sha256": _sha(body)})
    if sum(row["exact_zero_projection"] for row in packets) != 2:
        raise S02MetricComplementLeafAuthorityError("s02 zero projection census changed")
...
def _filtered_manifests(
    ...
    old_full, old_existing, old_new = prior.S01_FULL, prior.S01_EXISTING, prior.S01_NEW
    prior.S01_FULL, prior.S01_EXISTING, prior.S01_NEW = S02_FULL, S02_EXISTING, S02_NEW
    try:
        return prior._filtered_manifests(coefficients, targets, tangents, inverse)
    finally:
        prior.S01_FULL, prior.S01_EXISTING, prior.S01_NEW = old_full, old_existing, old_new
...
def _expected_body(
    ...
    projection = prior.engine._load_bundle(root, projection_bundle)
    blocks = _unspecialized_principal_blocks()
    if blocks["content_sha256"] != prior.BLOCK_SHA256:
        raise S02MetricComplementLeafAuthorityError("live A/B/C block changed")
    ...
    if S02_EXISTING not in targets:
        raise S02MetricComplementLeafAuthorityError("existing s02 authority missing")
    ...
    body = {
        ...
        "decision": "pass_s02_metric_nine_column_complement_285120_exact_leaf_roots_D2_blocked",
        ...
        "exact_controls": {
            "previous_s02_atom_not_recounted": S02_EXISTING,
            "exact_zero_projection_atoms": ["s02[0]", "s02[7]"],
        ...
        "gate_counts": {
            ...
            "previous_registered_coordinate_columns": 49,
            "previous_missing_coordinate_columns": 104,
            "previous_s02_metric_columns": 1,
            "new_s02_metric_coordinate_columns": 9,
            "complete_s02_metric_family_columns": 10,
            "registered_coordinate_columns_after": 58,
            "remaining_coordinate_columns_without_A_B_C_leaf_authority": 95,
        ...
        "claim_seals": {
            "complete_s02_metric_family_registered": True,
            "previous_s02_atom_not_recounted": True,
            "no_tensor_component_inferred_zero": True,
            "D2_entry_count_advanced": False,
            "remaining_95_coordinate_columns_registered": False,
            "complete_D2F": False,
            "global_H7": False,
        },
        ...
        "source_bindings": {
            ...
            "live_A_B_C_block_sha256": prior.BLOCK_SHA256,
        },
        "scope": "exact s02 metric nine-column complement leaf authority; no inferred zero, D2 advance, complete tensor, H7, rejection, or observation",
    }
    return body
...
def validate_campaign(value: Mapping[str, Any], *, root: Path | str | None = None) -> None:
    ...
    if value.get("content_sha256") != _content_sha(value) or value != expected:
        raise S02MetricComplementLeafAuthorityError("checked result changed")
