"""Advance the sealed Epoch 003 formal-receipt cursor beyond prefix thirteen."""
...
from . import continuous_scientific_pipeline_formal_receipt_batch_0003 as predecessor_worker
from . import continuous_scientific_pipeline_formal_receipt_batch_worker as engine

CONFIG_REL = "configs/continuous_scientific_pipeline_epoch_003_formal_receipt_batch_0004.json"
SOURCE_REL = "src/sigma_theory_compiler/continuous_scientific_pipeline_formal_receipt_batch_0004.py"
TEST_REL = "tests/test_continuous_scientific_pipeline_formal_receipt_batch_0004.py"
RESULT_REL = (
    "runs/engine/continuous-scientific-pipeline-epoch-003-formal-receipt-batch-0004/result.json"
)
CAMPAIGN_ID = "continuous-scientific-pipeline-epoch-003-formal-receipt-batch-0004"
...
_REGISTERED_HISTORICAL_RESULT_CONTENT_SHA256 = (
    "e478e89c6ca5f7b9db5d4b1416aa8bfcbeedfb246e6444baffb30b833b3413b4"
)
...
def load_config(root: Path, path: Path) -> dict[str, Any]:
    ...
    if set(config) != {
        ...
        raise ValueError("formal receipt batch 0004 config keys mismatch")
    if (
        ...
        raise ValueError("formal receipt batch 0004 config contract mismatch")
    artifact_directory = engine._resolve(root, config["artifact_directory"])
    if artifact_directory.name != config["campaign_id"]:
        raise ValueError("formal receipt batch 0004 artifact directory mismatch")
    ...
    if set(validator_binding) != {"path", "file_sha256"}:
        raise ValueError("batch 0003 validator config binding mismatch")
    ...
    if (
        ...
        raise ValueError("batch 0003 validator config hash mismatch")
...
def _validate_admission_sample(value: Mapping[str, Any], config: Mapping[str, Any]) -> None:
    engine._validate_sealed(value, "formal receipt batch 0004 admission sample")
    gate = config["resource_gate"]
    admitted = (
    ...
    if (
        ...
        raise ValueError("formal receipt batch 0004 admission sample contract mismatch")
...
def _validate_blocked_result(
    ...
    engine._validate_sealed(value, "formal receipt batch 0004 blocked result")
    ...
    if (
        ...
        raise ValueError("formal receipt batch 0004 blocked result contract mismatch")


def validate_result(value: Mapping[str, Any], root: Path, config_path: Path) -> None:
    """Replay batch 0004 exclusively from sealed predecessor/catalog artifacts."""
    if value.get("schema_version") == BLOCK_RESULT_SCHEMA:
        config = load_config(root, config_path)
        _validate_blocked_result(value, root, config)
        return
    engine._validate_sealed(value, "formal receipt batch 0004 result")
    ...
    if not isinstance(bindings, list) or len(bindings) != len(entries):
        raise ValueError("formal receipt batch 0004 executed leaf binding count mismatch")
    ...
    if (
        ...
        raise ValueError("formal receipt batch 0004 result contract mismatch")
