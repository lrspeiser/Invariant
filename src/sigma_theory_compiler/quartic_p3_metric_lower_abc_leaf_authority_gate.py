"""Register exact lower p3 metric A/B/C leaves by spatially permuting p1."""
...
from . import quartic_p2_metric_lower_abc_leaf_authority_gate as prior
...
CONFIG_SCHEMA = "sigma-quartic-p3-metric-lower-abc-leaf-authority-config-1.0"
RESULT_SCHEMA = "sigma-quartic-p3-metric-lower-abc-leaf-authority-gate-1.0"
CAMPAIGN_ID = "quartic-p3-metric-lower-abc-leaf-authority-001"
STEM = "quartic_p3_metric_lower_abc_leaf_authority_gate"
SLUG = "quartic-p3-metric-lower-abc-leaf-authority-gate"
...
CONTRACT = {
    "candidate_count": 12,
    "previous_registered_coordinate_columns": 135,
    "previous_missing_coordinate_columns": 18,
    "new_p3_metric_columns": 10,
    "d1_target_atoms": 20,
...
POLICIES = {
    "family_selection": (
        "complete_p3_metric_first_jet_family_by_exact_spatial_permutation_of_p1_tangent"
    ),
...
class P3MetricLowerLeafAuthorityError(ValueError):
    """A lower p3 tangent, spatial permutation, or sealed boundary changed."""


def _inside(root: Path, relative: str) -> Path:
    if not relative or "\\" in relative:
        raise P3MetricLowerLeafAuthorityError("p3 lower path is not portable")
    path = (root / relative).resolve()
    if path != root and root not in path.parents:
        raise P3MetricLowerLeafAuthorityError("p3 lower path escapes root")
    return path
...
def _validate_config(value: Mapping[str, Any], path: Path) -> None:
    if _production_sha(path) != CONFIG_PRODUCTION_SHA256:
        raise P3MetricLowerLeafAuthorityError("p3 lower config production bytes changed")
    if (
        ...
        raise P3MetricLowerLeafAuthorityError("p3 lower config contract changed")
...
def _load_inputs(root: Path, config: Mapping[str, Any]) -> dict[str, dict[str, Any]]:
    try:
        ...
        raise P3MetricLowerLeafAuthorityError(str(error)) from error
    if (
        ...
        raise P3MetricLowerLeafAuthorityError("p3 lower predecessor boundary changed")
...
def _validate_projection(projection: Mapping[str, Any]) -> None:
    records = {
        ...
        if row.get("family") == "p_metric" and row.get("derivative_index") == 3
    }
    if set(records) != {row[0] for row in P3_METRIC}:
        raise P3MetricLowerLeafAuthorityError("p3 lower projection family changed")
    for atom, column, pair in P3_METRIC:
        row = records[atom]
        seed = row.get("tangent_seed", {})
        if (
            row.get("coordinate_column") != column
            or seed.get("dP_derivative") != 3
            ...
            raise P3MetricLowerLeafAuthorityError("p3 lower projection seed changed")


def _permute_index(index: int) -> int:
    return 3 if index == 1 else 1 if index == 3 else index
...
def _primitive_permutation() -> tuple[dict[sp.Symbol, sp.Symbol], dict[str, sp.Symbol]]:
    ...
    for symbol in primitives:
        ...
        if gradient:
            ...
            raise P3MetricLowerLeafAuthorityError("p3 primitive symbol family changed")
        substitution[symbol] = by_name[target]
    if len(substitution) != 44 or set(substitution.values()) != set(primitives):
        raise P3MetricLowerLeafAuthorityError("p3 spatial permutation is not bijective")
    return substitution, by_name


_P1_MODULE = prior.prior
_P1_METRIC_ORIGINAL = prior._P1_METRIC_ORIGINAL
_P1_TANGENTS_ORIGINAL = prior._P1_TANGENTS_ORIGINAL


@cache
def _exact_p3_tangents() -> tuple[dict[str, dict[str, Any]], dict[str, Any]]:
    active_metric = _P1_MODULE.P1_METRIC
    active_tangents = _P1_MODULE._exact_p1_tangents
    _P1_MODULE.P1_METRIC = _P1_METRIC_ORIGINAL
    _P1_MODULE._exact_p1_tangents = _P1_TANGENTS_ORIGINAL
    try:
        p1_packets, _ = _P1_TANGENTS_ORIGINAL()
    finally:
        _P1_MODULE.P1_METRIC = active_metric
        _P1_MODULE._exact_p1_tangents = active_tangents
    ...
    for atom, column, pair in P3_METRIC:
        ...
        delta_h: dict[str, str] = {}
        delta_g: dict[str, str] = {}
        ...
        body = {
            ...
            "spatial_permutation": "swap_indices_1_and_3",
            "all_20_covariant_tangent_components_materialized": True,
        }
        packets[atom] = {**body, "content_sha256": _sha(body)}
    for field in range(10):
        packets[f"p0[{field}]"] = packets[f"p3[{field}]"]
    program_body = {
        "schema_version": "sigma-p3-metric-nonlinear-coordinate-tangent-program-1.0",
        "primitive_symbols": sorted(locals_map),
        "frame_constraint": "g_ab=u^ab=diag(-1,1,1,1) at the evaluation point",
        "exact_spatial_permutation": "swap_indices_1_and_3_from_p1",
        "permutation_domain": "eta_11=eta_33=1; no time-space permutation used",
        "formulas": [
            "P1_kab maps to P3_pi(k)pi(a)pi(b) under pi=(1 3)",
            "v_k maps to v_pi(k)",
            "deltaH_ab maps to deltaH_pi(a)pi(b)",
            "deltaG^ab maps to deltaG^pi(a)pi(b)",
            (
                "all p1 connection, differentiated-connection, Ricci, Hessian, and "
                "Einstein formulas commute with this spatial permutation"
            ),
        ],
...
def _prior_body(
    ...
    old = {name: getattr(prior, name) for name in names}
    updates = {
        "P2_METRIC": P3_METRIC,
        "_exact_p2_tangents": _exact_p3_tangents,
...
def _expected_body_uncached(
    ...
    body = _prior_body(root, config_path, config, values)
    ...
    body["first_blocker"] = (
        "independently_replay_the_D1_arithmetic_DAG_for_p3_metric_columns_before_"
        "admitting_ordered_D2_roots_then_materialize_the_remaining_8_columns"
    )
    body["leaf_authority_theorem"].update(
        {
            "name": "pointwise_frame_arbitrary_first_jet_p3_metric_to_live_A_B_C_chain",
            "exact_result": (
                "Spatial permutation covariance maps the complete p1 tangent family to all "
                "ten p3 metric first-jet directions and composes them through every live "
                "A/B/C leaf used by the 20 registered D1 targets."
            ),
        }
    )
    for manifest in body["candidate_manifests"]:
        manifest["derivative_coordinate_columns"] = list(range(43, 53))
        manifest["candidate_decision"] = "pass_p3_metric_lower_leaf_authority_D2_replay_blocked"
    ...
    counts.pop("new_p2_metric_coordinate_columns")
    counts.update(
        {
            "previous_registered_coordinate_columns": 135,
            "previous_missing_coordinate_columns": 18,
            "new_p3_metric_coordinate_columns": 10,
            "registered_coordinate_columns_after": 145,
            "remaining_coordinate_columns_without_A_B_C_leaf_authority": 8,
        }
    )
    seals = body["claim_seals"]
    seals.pop("complete_p2_metric_lower_family_registered")
    seals.pop("remaining_18_coordinate_columns_registered")
    seals.update(
        {
            "complete_p3_metric_lower_family_registered": True,
            "remaining_8_coordinate_columns_registered": False,
        }
    )
    body["scope"] = (
        "exact p3 metric lower-coordinate nonlinear tangent and live leaf authority; "
...
def validate_campaign(value: Mapping[str, Any], *, root: Path | str | None = None) -> None:
    ...
    if value.get("content_sha256") != _content_sha(value) or value != expected:
        raise P3MetricLowerLeafAuthorityError("checked result changed")
