...
CONFIG_PATH = Path(
    "configs/open_gravity_lens_ray_clock_source_shaped_synthetic_injection_matrix_v1.json"
)
TEST_PATH = Path(
    "tests/test_open_gravity_lens_ray_clock_source_shaped_synthetic_injection_matrix_v1.py"
)
OUTPUT_DIR = Path(
    "runs/gravity/open-gravity-lens-ray-clock-source-shaped-synthetic-injection-matrix-v1"
)
...
def validate_config(config: Mapping[str, Any], *, verify_hashes: bool = True) -> None:
    ...
    _require(
        config["schema"]
        == "open-gravity-lens-ray-clock-source-shaped-synthetic-injection-matrix-1.0",
        "lens synthetic schema changed",
    )
    _require(
        config["package_id"]
        == "open-gravity-lens-ray-clock-source-shaped-synthetic-injection-matrix-v1"
    ...
    for formula_id in sorted((*_ENTRYPOINTS, *_BLOCK_STATUS)):
        ...
        rows.append(
            FormulaExecutionBinding(
                ...
                entrypoint=(
                    f"sigma_theory_compiler.open_gravity_lens_ray_clock_source_shaped_synthetic_injection_matrix_v1:{_ENTRYPOINTS[formula_id]}"
                    if executable
                    else None
                ),
...
def derive_release() -> tuple[dict[str, Any], bytes, bytes, bytes, bytes, bytes]:
    ...
    release = SyntheticSuiteRelease(
        ...
        changed_feature_ids=tuple((*_OUTPUTS, *(_RESPONSE_FOR[value] for value in _OUTPUTS), "truth.scalar.injection-id")),
        change_level="MAJOR",
    ...
    ledger = SyntheticReplayLedger("gravity.synthetic.lens-ray-clock-source-shaped-matrix.v1", ())
    scenario_rows = []
    confusion_counts = {
    ...
    receipt_body = {
        "schema": "open-gravity-lens-ray-clock-source-shaped-synthetic-injection-matrix-receipt-1.0",
