...
CONFIG_SECTION_SHA256: dict[str, str] = {
    ...
    "authorization_gate": "5a6628096a8df69cd5d7b65af97e538399a3b1d6763124fe3e67465edab0ab8f",
    "candidate_registry": "577281c3f36d11d9ac75dadebf241bb984b05df4e75157660d8fa0fce549abec",
    "claim_ceiling": "f24e16aa092d309edb7b83f6f4a20dc008e0ae81cef71584f60500aedc8017ee",
    "compute_ceiling": "5bb866833aff5f02c45518f5441b69ef8cdef6bbd20f5f854ce1e617f1e1dee8",
    ...
    "interrupted_run_contract": "59ab78d42d4755270ae8cd5391680f25340bc80c985d08f11ea10a515533aea9",
    "output_paths": "5513bebf72d23ae5eab3f6b696d4bffdd02667302db1b295775142ba2a83b87b",
    "result_validation_contract": "b2f0ea3ea9357a55be80703db834dc276d86fd3062a3fe56d308f31eb7839b64",
    "runtime_environment_contract": "c1c98e4759eaad8c6a78b40c150b3ceb494b08c5c439e6345a7e729518dd1851",
    "runtime_preflight": "b56fca76c13fba701fc5fe6a133e12f6e20a15e2aeff9b6ae38a2e9623a43d7e",
    "selection_contract": "4aa7155350acac1e204a9424ac6a0b5a3893976e307194b3a092acaf9f527f1b",
    ...
    "zero_access_chronology": "9c540e5abe5171feb7ecadcb2f78930df65496758ac68dcb56914d31bb33b576",
}
...
ACKNOWLEDGEMENTS = {
    ...
    "exact_runtime_required_for_strict_binary64_selection": True,
...
def validate_config(config: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        or environment["policy_id"] != "ben-development-strict-binary64-runtime-v1"
        or environment["exact_match_required_before_access_intent"] is not True
        or environment["comparison_operator"] != "raw_binary64_strict_less_than"
        or environment["numeric_improvement_tolerance"] is not None
        ...
        or interrupted["post_access_intent_failure_receipt_schema"] != ACCESS_FAILURE_SCHEMA
        ...
        is not True
        or interrupted["write_atomic_no_clobber_failure_receipt_on_any_post_intent_exception"]
        is not True
...
def _distribution_identity(distribution_name: str) -> dict[str, str]:
    try:
...
def _static_runtime_receipt(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    packages = {name: _distribution_identity(name) for name in contract["packages"]}
    ...
    if len(pools) != 1:
        raise BENDevelopmentExecutorV4Error("frozen NumPy BLAS/SVD backend is not unique")
    backend = {
        key: pools[0].get(key)
        for key in (
            "user_api",
    ...
    if (
        ...
        raise BENDevelopmentExecutorV4Error("exact frozen host runtime changed")
    return {
...
def _activate_frozen_svd_runtime(config: Mapping[str, Any]) -> tuple[Any, dict[str, Any]]:
    ...
    if (
        ...
        raise BENDevelopmentExecutorV4Error("single-thread frozen BLAS/SVD activation failed")
    active = {
    ...
    if (
        ...
        limiter.restore_original_limits()
        raise BENDevelopmentExecutorV4Error("frozen BLAS library identity changed")
...
def load_sparc_development(
    ...
    accounting["payload_file_opens"] = accounting.get("payload_file_opens", 0) + 1
    accounting["sparc_files_opened"] = accounting.get("sparc_files_opened", 0) + 1
...
def load_xcop_development(
    ...
    for record in inventory:
        ...
        accounting["payload_file_opens"] = accounting.get("payload_file_opens", 0) + 1
        accounting["xcop_files_opened"] = accounting.get("xcop_files_opened", 0) + 1
    ...
    return {
        "objects": clusters,
        "payload_files_opened": len(inventory),
...
def _build_candidate_decisions(
    ...
    """Compare only raw binary64 losses after every named comparator has passed."""
    ...
    for full in registered["full"]:
        ...
        checks = {
            ...
            and s_full["domain_loss"] is not None
            and all(
                s_full["domain_loss"] < sparc_results[item]["domain_loss"] for item in ablation_ids
            ),
            "xcop_beats_each_valid_ablation": ablations_valid
            and x_full["domain_loss"] is not None
            and all(
                x_full["domain_loss"] < xcop_results[item]["domain_loss"] for item in ablation_ids
            ),
        }
        decisions.append(
            {
...
def _execute_scoring(
    ...
    decisions = _build_candidate_decisions(registered, sparc_results, xcop_results)
...
def _preflight_gpu(config: Mapping[str, Any]) -> tuple[Any, dict[str, Any]]:
    """Verify the frozen production GPU in FP64 without opening scientific payloads."""

    runtime = config["runtime_preflight"]
    frozen_cuda = config["runtime_environment_contract"]["cuda"]
    try:
        import cupy as cp
    ...
    try:
        ...
        if device_count != frozen_cuda["device_count"]:
        ...
        if (
            device_name != frozen_cuda["device_name"]
            or cuda_runtime != frozen_cuda["runtime_version_integer"]
            or cuda_driver != frozen_cuda["driver_version_integer"]
            or compute_major != frozen_cuda["compute_capability_major"]
            or compute_minor != frozen_cuda["compute_capability_minor"]
        ):
...
def _render_result_floats(value: Any) -> Any:
    """Render binary64 only after every strict scientific comparison is complete."""
...
def _validate_recorded_runtime_environment(
    ...
    if (
        ...
        or receipt["numeric_improvement_tolerance"] is not None
        or receipt["exact_match_pass"] is not True
    ):
...
def validate_result_document(
    ...
    _strict_keys(
        access_intent,
        {
            ...
            "payload_scope",
            "forbidden_scope",
            "payload_files_opened_before_this_record",
    ...
    if (
        ...
        or access_intent["payload_files_opened_before_this_record"] != 0
    ...
    decisions = _build_candidate_decisions(registered, sparc_adjudicated, xcop_adjudicated)
    eligible = [row["class_id"] for row in decisions if row["eligible"]]
    selected = eligible[0] if eligible else None
    if (
    ...
    expected_accounting = {
        ...
        * populations["xcop"]["predictor_rows_parsed_and_scored"],
        ...
        "paid_calls": 0,
        "api_spend_usd": 0.0,
        "payload_file_opens": populations["sparc"]["payload_files_opened"]
        + populations["xcop"]["payload_files_opened"],
        "result_validation_events": 1,
    }
...
def _write_access_failure_receipt(
    ...
    body: dict[str, Any] = {
        ...
        "status": "POST_ACCESS_INTENT_FAILURE_SUCCESSOR_REQUIRED",
        "authorization_id": authorization["authorization_id"],
        "authorization_file_sha256": file_sha256(confined(authorization_path)),
        "preflight_receipt_content_sha256": preflight["content_sha256"],
        "access_intent_content_sha256": access_intent["content_sha256"],
...
def execute(authorization_path: Path) -> Path:
    ...
    static_runtime = _static_runtime_receipt(config)
    limiter, svd_runtime = _activate_frozen_svd_runtime(config)
    try:
        ...
        try:
            ...
            record_phase()
            payload_file_opens = access_accounting["payload_file_opens"]
            if payload_file_opens > config["compute_ceiling"]["maximum_payload_file_opens"]:
                raise BENDevelopmentExecutorV4Error("payload-file-open ceiling exceeded")
            scores["actual_compute_accounting"]["payload_file_opens"] = payload_file_opens
