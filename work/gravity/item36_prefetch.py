...
CACHE = (ROOT / "work/gravity/item-36-manga-maps-cache").resolve()
MANIFEST = ROOT / "runs/gravity/roadmap/item-36-extra-dimensions-v1-source/sample-manifest.json"
BASE = (
...
def fetch(identity: str) -> str:
    ...
    for _ in range(3):
        try:
            request = urllib.request.Request(url, headers={"User-Agent": "Invariant-Item36/1.0"})
...
def main() -> None:
    ...
    if len(identities) != 140 or len(set(identities)) != 140:
    ...
    if len(completed) != 140:
        raise RuntimeError("incomplete Item 36 prefetch")
