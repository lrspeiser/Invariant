...
CACHE = (ROOT / "work/gravity/item-37-manga-maps-cache").resolve()
MANIFEST = ROOT / "runs/gravity/roadmap/item-37-alternative-geometry-v1-source/sample-manifest.json"
BASE = (
...
def fetch(identity: str) -> str:
    ...
    for _ in range(3):
        try:
            request = urllib.request.Request(url, headers={"User-Agent": "Invariant-Item37/1.0"})
...
def main() -> None:
    ...
    if len(completed) != 120:
        raise RuntimeError("incomplete Item 37 prefetch")
