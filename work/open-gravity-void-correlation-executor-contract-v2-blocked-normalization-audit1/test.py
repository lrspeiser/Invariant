...
from sigma_theory_compiler import open_gravity_void_correlation_executor_contract_v2 as contract
...
def profile_fixture(sign: float = 1.0) -> tuple[list[float], list[float], list[float], list[float], list[tuple[float, float, float]], list[float], list[int]]:
    ...
    directions = [contract.normalize_direction((1.0, 0.11 * (index + 1), 0.07 * (index % 4 + 1))) for index in range(12)]
...
def test_canonical_paths_are_cwd_independent_and_fail_closed(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert contract.canonical_bound_path("configs/open_gravity_void_correlation_executor_contract_v2.json") == expected
    for invalid in (
        "",
        "../configs/open_gravity_void_correlation_executor_contract_v2.json",
        "configs\\open_gravity_void_correlation_executor_contract_v2.json",
        "C:/Windows/win.ini",
        "/etc/passwd",
    ):
        with pytest.raises(contract.VoidExecutorV2Error):
...
def test_arbitrary_path_is_rejected_before_any_filesystem_read(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(contract.VoidExecutorV2Error, match="absolute"):
...
def test_fixed_width_schemas_are_exact_and_nonoverlapping() -> None:
    ...
    bad[1]["start"] = 7
    with pytest.raises(contract.VoidExecutorV2Error, match="overlap"):
...
def test_strict_record_and_numeric_grammar_accepts_only_frozen_forms() -> None:
    ...
    blank[21:26] = b"     "
    with pytest.raises(contract.VoidExecutorV2Error, match="required field missing"):
...
def test_velocity_design_maps_to_log_redshift_by_dividing_each_column_once() -> None:
    ...
    with pytest.raises(contract.VoidExecutorV2Error, match="domain"):
...
def test_equal_count_distance_strata_use_identifier_to_break_all_ties() -> None:
    ...
    assert min(sizes) == 2
    assert max(sizes) == 3
    with pytest.raises(contract.VoidExecutorV2Error, match="duplicate"):
...
def test_duplicate_semantics_are_table_specific_and_unmatched_spheres_reject() -> None:
    ...
    with pytest.raises(contract.VoidExecutorV2Error, match="duplicate CF4"):
    ...
    with pytest.raises(contract.VoidExecutorV2Error, match="sphere"):
        contract.validate_vast_duplicate_keys(table1, table2 + [table2[0]])
    with pytest.raises(contract.VoidExecutorV2Error, match="unmatched"):
...
def test_minimum_counts_are_staged_without_peeking_at_holdout_eligibility() -> None:
    ...
    with pytest.raises(contract.VoidExecutorV2Error, match="raw-ID minimum"):
        contract.validate_stage_counts("IDS_PARTITIONED", {"development": 499, "validation": 150, "confirmation": 150})
    with pytest.raises(contract.VoidExecutorV2Error, match="eligibility forbidden"):
    ...
    for stage in ("VALIDATION_OPEN", "CONFIRMATION_OPEN", "PANTHEON_OPEN"):
        with pytest.raises(contract.VoidExecutorV2Error, match="not authorized"):
...
def test_receipt_is_deterministic_hash_only_and_rows_unopened() -> None:
    ...
    assert first["status"] == "PASS_MUTATION_FREEZE_SYNTHETIC_CONFORMANCE_ROWS_UNOPENED_AWAIT_INDEPENDENT_AUDIT"
...
def test_top_level_config_mutation_fails_even_if_attacker_rehashes(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    with pytest.raises(contract.VoidExecutorV2Error, match="status drift"):
...
def test_top_level_receipt_mutation_fails_even_with_new_self_hash(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    with pytest.raises(contract.VoidExecutorV2Error, match="receipt drift"):
...
def test_nonfinite_profile_inputs_fail_closed() -> None:
    ...
    bad_y = list(y)
    bad_y[0] = math.nan
    with pytest.raises(contract.VoidExecutorV2Error, match="nonfinite response"):
        contract.profile_at_delta(bad_y, sigma, path_distances, directions, l_void, identifiers, 0.0)
    bad_sigma = list(sigma)
    bad_sigma[0] = 0.0
    with pytest.raises(contract.VoidExecutorV2Error, match="uncertainty"):
    ...
    with pytest.raises(contract.VoidExecutorV2Error, match="void path"):
