...
def _all_final_gates_bound(config: Mapping[str, Any]) -> bool:
    return all(
        gate.get("status") == FINAL_GATE_STATUS
...
def validate_config(config: Mapping[str, Any]) -> None:
    ...
    _require(set(config.get("section_sha256", {})) == set(STRICT_SECTIONS), "section seal set changed")
    ...
    _require(
        committed["GP01_FOUNDATION"]["commit"]
    ...
    for gate in gates.values():
        ...
        if gate["status"] == FINAL_GATE_STATUS:
            _require(gate.get("replacement_required") is False, "final gate still needs replacement")
            _require(
                gate.get("independent_audit_status") == gate.get("required_independent_audit_status"),
                "final gate lacks required independent audit PASS",
            )
            _require(COMMIT_RE.fullmatch(str(gate.get("commit", ""))) is not None, "final gate commit malformed")
    ...
    _require(len(disposition["static_ready_architectures"]) == 15, "static ready architectures changed")
    _require(
    ...
    _require(disposition["source_ready_concept_counts"] == {"SPARC_TWELL": 60, "XCOP_TWELL": 126}, "TWELL concept counts changed")
    _require(disposition["source_ready_parameter_cell_counts"] == {"SPARC_TWELL": 176, "XCOP_TWELL": 370}, "TWELL cell counts changed")
    _require(set(config["gp01_dispositions"]) == {
        "GP01-L", "GP01-AQUAL", "GP01-T1", "GP01-T2", "GP01-ELLIPTIC",
        "GP01-TELEGRAPH", "GP01-ACTION_PLACEHOLDER",
    ...
    _require(matrix["stream_only"] is True and matrix["rows_materialized_in_receipt"] == 0, "matrix materialized")
    ...
    _require(projection["exact_parameter_cell_sources"]["total"] == 2486, "cell source count changed")
    _require(projection["exact_parameter_cell_sources"]["domain_slot_count"] == 9944, "cell domain slots changed")
    _require(projection["source_ready_registry_projection"]["eligible"] is False, "eligibility claimed")
    _require(config["claim_boundary"]["DATA_ELIGIBLE_claimed"] is False, "DATA_ELIGIBLE claimed")
    _require(config["claim_boundary"]["campaign_execution_authorized"] is False, "campaign authorized")


def load_config(root: Path) -> dict[str, Any]:
    value = _read_json(root / CONFIG_PATH, "v2 source contract")
    validate_config(value)
    return value


def load_predecessor(root: Path, config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _require(content_sha256(value) == binding["semantic_content_sha256"], "predecessor semantics changed")
    for section, expected in config["inherited_sections"].items():
        _require(content_sha256(value[section]) == expected, f"inherited section changed: {section}")
    return value


def load_inputs(root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    config = load_config(root)
    return config, load_predecessor(root, config)
...
def _gp01_reason(mechanism_id: str, domain: str) -> str:
    reasons = {
        ...
        "GP01-T1": "ALL_EIGHT_XCOP_CLUSTERS_LACK_UNIQUE_Y100_ANCHOR" if domain == "XCOP" else "FULL_SPARC_TRANSPORT_SOURCE_BLOCKED",
        "GP01-T2": "ALL_EIGHT_XCOP_CLUSTERS_LACK_UNIQUE_Y100_ANCHOR" if domain == "XCOP" else "FULL_SPARC_TRANSPORT_SOURCE_BLOCKED",
        "GP01-ELLIPTIC": "SPHERICAL_XCOP_SOURCE_READY_PENDING_EXACT_ADAPTER" if domain == "XCOP" else "FULL_SPARC_3D_SOURCE_BLOCKED",
...
def _representability(
    config: Mapping[str, Any], mechanism: Mapping[str, Any]
) -> tuple[bool, str]:
    family = mechanism["mechanism_family"]
    if family.startswith("TWELL_"):
        if _gate_bound(config, "TWELL_SUCCESSOR"):
...
def _gp01_elliptic_cell_ids() -> Iterable[str]:
    ...
    for n, a_max, rho, tide, q, power, length in itertools.product(*grid):
        yield (
            f"GP01E-n{n}-A{a_max:g}-rho{rho:g}-T{tide:g}"
            f"-q{q}-p{power}-L{length:g}"
        )
...
def build_receipt(root: Path) -> dict[str, Any]:
    config, predecessor = load_inputs(root)
    ...
    receipt: dict[str, Any] = {
        ...
        "objects": {
            ...
            "XCOP_stellar_profile_available": predecessor["objects"]["XCOP_stellar_profile_available"],
...
def write_receipt(root: Path) -> str:
    ...
    payload = json.dumps(receipt, sort_keys=True, indent=2, ensure_ascii=False).encode("utf-8") + b"\n"
    return _atomic_no_clobber(root / OUTPUT_PATH, payload)


def validate_receipt(root: Path) -> None:
    stored = _read_json(root / OUTPUT_PATH, "v2 zero-response receipt")
