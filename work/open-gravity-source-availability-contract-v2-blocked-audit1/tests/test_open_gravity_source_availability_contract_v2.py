...
def test_committed_registry_and_gp01_hashes_are_exact(inputs: tuple[dict, dict]) -> None:
    ...
    assert bindings["GP01_FOUNDATION"]["commit"] == (
...
def test_every_strict_section_rejects_mutation(section: str) -> None:
    ...
    with pytest.raises(source.SourceAvailabilityV2Error, match=f"strict section hash changed: {section}"):
...
def test_twell_source_partition_is_exact_at_concept_and_cell_levels(
    ...
    for architecture in ("A15_RETARDED", "A16_MEMORY", "A17_RESONANCE", "A18_STOCHASTIC"):
        ...
        assert rows
        for domain in ("SPARC", "XCOP"):
            assert all(source._mechanism_disposition(config, row, domain)[0] == "SOURCE_BLOCKED" for row in rows)

    compounds = {row["mechanism_id"]: row for row in catalog if row["mechanism_family"] == "TWELL_COMPOUND"}
    xcop_ready = {
        mechanism_id
    ...
    assert not any(source._mechanism_disposition(config, row, "SPARC")[0].startswith("SOURCE_READY") for row in compounds.values())
...
def test_gp01_and_ontology_dispositions_are_not_overclaimed(inputs: tuple[dict, dict]) -> None:
    ...
    for key in (("GP01-T1", "A85", "PRESSURE_PROFILE"), ("GP01-T2", "A3266", "TEMPERATURE_PROFILE")):
        assert rows[key]["concept_readiness"] == "SOURCE_BLOCKED"
        assert "Y100_ANCHOR" in rows[key]["disposition_reason"]
    assert rows[("GP01-ELLIPTIC", "CamB", "ROTATION_CURVE")]["concept_readiness"] == "SOURCE_BLOCKED"
    ...
    assert rows[("GP01-TELEGRAPH", "A85", "PRESSURE_PROFILE")]["concept_readiness"] == "SOURCE_BLOCKED"
    assert rows[("GP01-ACTION_PLACEHOLDER", "A85", "PRESSURE_PROFILE")]["concept_readiness"] == "QUARANTINED"
...
def test_xcop_stellar_split_and_shared_xray_ancestry_are_explicit(
    ...
    rows = _matrix_rows_by_key(config, predecessor, wanted)
    assert rows[("GP01-L", "A85", "PRESSURE_PROFILE")]["stellar_source_status"] == "SOURCE_AVAILABLE"
    assert rows[("GP01-L", "A3266", "PRESSURE_PROFILE")]["stellar_source_status"] == "SOURCE_MISSING"
...
def test_partitions_comparators_legacy_floor_and_discriminator_are_retained(
    ...
    assert predecessor["legacy_multiplicity_lower_bound"]["minimum_known_candidate_galaxy_evaluations"] == 450_000_000
...
def test_manifest_concept_domain_projection_has_stable_hash_for_every_slot(
    ...
    assert all(row["domain_execution"]["source_contract_sha256"] == row["source_contract_sha256"] for row in rows)
    ...
    assert not any(row["domain_execution"]["execution_disposition"] == "SEALED_UNOPENED_FOR_SCORING" for row in rows)
...
def test_unrepresentable_cards_and_lanes_are_reported(inputs: tuple[dict, dict]) -> None:
    ...
    assert not any(row["manifest_candidate_representable"] for row in grouped["GRAVITY_LIGHT_ONTOLOGY"])
...
def test_generator_does_not_open_any_upstream_receipt_payload(
    ...
    assert source.manifest_projection_summary(config, predecessor)["concept_domain_slots"]["slot_count"] == 1680
...
def test_final_seal_is_impossible_while_any_agent_hash_gate_is_pending(
    ...
    if source._all_final_gates_bound(config):
        ...
        with pytest.raises(source.SourceAvailabilityV2Error, match="final bind gates remain pending"):
            source.build_receipt(ROOT)
...
def test_sealed_receipt_is_zero_response_and_contains_no_stream_rows(
    ...
    assert receipt["manifest_projection"]["concept_domain_slots"]["rows_materialized_in_receipt"] == 0
    assert receipt["manifest_projection"]["parameter_cell_domain_slots"]["rows_materialized_in_receipt"] == 0
