...
CONFIG_SECTION_SHA256: dict[str, str] = {
    ...
    "authorization_gate": "91e4edefbc58f7172d24e376223be56016421b0511e8034e9f9aae2cffb6ecf3",
    "candidate_registry": "577281c3f36d11d9ac75dadebf241bb984b05df4e75157660d8fa0fce549abec",
    "claim_ceiling": "6b9297941312df5f298a54896301cd3299aa4416a337235f8b0830bc367263a1",
    "compute_ceiling": "3b72b9d462a38b5ff8b56a03cbc99e63c958522248af93d3589a0271f3feb4a3",
    ...
    "output_paths": "c9eb9544e66efffe8943379be259801a562b265445e4bdb72faa948e8e988b05",
...
def validate_config(config: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise BENDevelopmentExecutorV4Error("domain-switch risk or runtime preflight changed")
...
def build_registered_variants(registry: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "accounting": {
            ...
            "registered_variants_using_x_geometry": sum(
                row["uses_x_geometry"] for row in variants
            ),
...
def build_preflight() -> dict[str, Any]:
    ...
    body: dict[str, Any] = {
        ...
        "development_populations": {
            ...
            "sparc_split_confirmation_name_root_sha256": config[
                "development_populations"
            ]["sparc"]["split_confirmation_name_root_sha256"],
            "sparc_split_exploration_name_root_sha256": config[
                "development_populations"
            ]["sparc"]["split_exploration_before_admission_name_root_sha256"],
            "sparc_admitted_score_name_root_sha256": config["development_populations"][
                "sparc"
            ]["admitted_score_name_root_sha256"],
            "sparc_admitted_score_name_row_ledger_sha256": config[
                "development_populations"
            ]["sparc"]["admitted_score_name_row_ledger_sha256"],
        },
...
def authorization_template(
    ...
    return {
        ...
        "sparc_objects": 139,
        "sparc_rows": 2720,
        "sparc_split_confirmation_name_root_sha256": config["development_populations"][
            "sparc"
...
def validate_authorization(
    ...
    if not isinstance(supplied["approved_at"], str) or RFC3339_UTC.fullmatch(
...
def load_sparc_development(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "admitted_score_name_row_ledger_sha256": binding[
...
def _nontrivial_response_variation(h: np.ndarray) -> bool:
    ...
    return bool(
        ...
        >= math.sqrt(np.finfo(np.float64).eps) * scale
    )
...
def _build_candidate_decisions(
    ...
    for full in registered["full"]:
        ...
        checks = {
            ...
            and s_full["domain_loss"] is not None
            and all(
                s_full["domain_loss"] < sparc_results[item]["domain_loss"]
                for item in ablation_ids
            ),
            "xcop_beats_each_valid_ablation": ablations_valid
            and x_full["domain_loss"] is not None
            and all(
                x_full["domain_loss"] < xcop_results[item]["domain_loss"]
                for item in ablation_ids
            ),
        }
        decisions.append(
...
def _execute_scoring(
    ...
    actual_compute_accounting = {
        ...
        "total_formula_row_cells_per_backend": sparc_formula_row_cells
        + xcop_formula_row_cells,
        ...
        * (sparc_formula_row_cells + xcop_formula_row_cells),
        ...
        "xcop_analytic_scale_solves": sum(
            row["analytic_scale_solves"] for row in xcop_scores
        ),
        "object_score_reductions": sum(row["object_score_reductions"] for row in all_scores),
        "response_row_score_terms": sum(
            row["response_row_score_terms"] for row in all_scores
        ),
        "candidate_selection_events": 1,
        "gpu_runtime_preflight_calls": 1,
        "gpu_runtime_preflight_probe_values": config["runtime_preflight"][
            "float64_probe_values"
        ],
        "threshold_tuning_calls": 0,
        "formula_generation_calls": 0,
        "formula_repair_calls": 0,
...
def _preflight_gpu(config: Mapping[str, Any]) -> tuple[Any, dict[str, Any]]:
    ...
    try:
        ...
        if device_count < runtime["minimum_cuda_devices"]:
            raise BENDevelopmentExecutorV4Error("GPU preflight found too few CUDA devices")
        device_id = int(cp.cuda.Device().id)
        properties = cp.cuda.runtime.getDeviceProperties(device_id)
        raw_name = properties["name"]
        device_name = raw_name.decode() if isinstance(raw_name, bytes) else str(raw_name)
        if runtime["required_device_name_substring"] not in device_name:
    ...
    if host.shape != (probe_count,) or host.dtype != np.dtype(np.float64) or not np.all(
        np.isfinite(host)
    ):
...
def execute(authorization_path: Path) -> Path:
    ...
    # This hardware-only probe must succeed before the one-shot target access intent exists.
    ...
    payload_file_opens = 1 + xcop["payload_files_opened"]
    ...
    result_raw: dict[str, Any] = {
        ...
        "development_populations": {
            "sparc": {
                key: value for key, value in sparc.items() if key != "objects"
            },
