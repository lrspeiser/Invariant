...
def run():
    out=P/'finer001';out.mkdir(exist_ok=False)
    m.save(out/'pre-access-bindings.json',{p.relative_to(m.ROOT).as_posix():m.digest(p) for p in [Path(__file__),m.ROOT/'scripts/mond_atlas_refraction_program.py',m.ROOT/'scripts/mond_atlas_source_resolution.py',P/'REFINEMENT_ADDENDUM.md',P/'run001/summary.json',m.MANIFEST]})
    try:
