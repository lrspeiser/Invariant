...
def main():
 out=P/'centroid001';out.mkdir(exist_ok=False);priv=PRIVATE/'centroid001';priv.mkdir(parents=True,exist_ok=False)
 ...
 for item in old['assets']:
  ...
  interp=RegularGridInterpolator((axis,axis),S,bounds_error=False,fill_value=0);positive=interp(np.column_stack([x,y]));assert (positive>0).all()
  ...
  rows.append(dict(case=case,step_kpc=step,emitters=len(x),radius_min=float(d['radius_kpc'].min()),radius_max=float(d['radius_kpc'].max()),independent_centroid_error_kpc=float(max(errs)),independent_mass_relative_error=float(max(masserrs)),min_underlyingHI=float(positive.min()),mass_flux_arrays_unchanged=True))
 (out/'summary.json').write_text(json.dumps(dict(status='MASS_CENTROID_QUADRATURE_PASS',assets=assets,cases=rows,observed_response_access=False,script_sha256=sha(Path(__file__)),addendum_sha256=sha(P/'CENTROID_ADDENDUM.md')),indent=2),encoding='utf8');print(json.dumps(rows,indent=2))
