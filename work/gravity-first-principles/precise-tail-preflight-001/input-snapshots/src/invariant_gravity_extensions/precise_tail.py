...
The PCHIP coefficients, core coefficient, Gauss nodes and transform are the
same stored binary numbers. No physical source or high-k cutoff is changed.
"""
...
class PreciseLogGreen:
    ...
    def __init__(self, disk, nodes=128, digits=50):
        ...
        c = self.ctx
        self.disk = disk
        self.x, self.w = [[c.mpf(float(v)) for v in a] for a in roots_legendre(nodes)]
