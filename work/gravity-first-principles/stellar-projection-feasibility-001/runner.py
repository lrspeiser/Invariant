"""Source-only positive Plummer-mixture bracket feasibility; no gravity fits."""
import hashlib
import json
...
dest=root/'work/gravity-first-principles/stellar-projection-feasibility-001'
...
registration=dict(input_sha256=hashlib.sha256(path.read_bytes()).hexdigest(),
    ...
    objective='Minimize common additive bracket relaxation as fraction of each measured mass; nonnegative Plummer component masses. No pressure or gravity data.',
    limits='Finite centered Plummer mixtures imply declining spherical density. Failure is restricted to this representation and grid, not all positive sources; bracket feasibility is not a likelihood or statistical confidence claim.')
...
for p in packet['stellar_packets']:
    ...
    for grid in registration['grids']:
        scales=np.geomspace(.1*r[0],grid['outer_factor']*r[-1],grid['nodes'])
        # Exact Plummer projected cumulative mass fraction R²/(R²+a²).
        normalization=m[-1]
        design=r[:,None]**2/(r[:,None]**2+scales[None,:]**2)*normalization/m[:,None]
