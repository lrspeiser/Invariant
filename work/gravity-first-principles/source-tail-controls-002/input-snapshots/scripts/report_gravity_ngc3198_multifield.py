...
def seal(run):
    ...
    resolution = {'own_snapshot': 0, 'parent_snapshot': 0, 'hash_identical_repository_artifact': 0}
    for name, digest in started['input_hashes'].items():
        snapshot = run/'input-snapshots'/name
        parent = started.get('config', {}).get('parent')
        inherited = ROOT/parent/'input-snapshots'/name if parent else None
        if snapshot.exists():
            path, origin = snapshot, 'own_snapshot'
        elif inherited is not None and inherited.exists() and sha256(inherited.read_bytes()).hexdigest() == digest:
            path, origin = inherited, 'parent_snapshot'
        else:
            path, origin = ROOT/name, 'hash_identical_repository_artifact'
    ...
    return {'input_hashes_verified': len(started['input_hashes']), 'result_sha256': digest,
            'failure_retained': (run/'failure.json').exists(), 'input_resolution': resolution}
