...
def field_bridge(config, maps):
    ...
    for name, disks in source_variants:
        ...
        for definition in definitions:
            ...
            flux_radius = 1000.0
            measured_gm = float(source.potential.spline(np.log(flux_radius), 1)[0]*flux_radius)
            rows.append({"source_variant": name, "grid": asdict(grid), "radii_kpc": radii, "inward_force_kms2_kpc": force,
                         ...
                         "monopole_flux_relative_mass_error": measured_gm/(G*sum(mass))-1})
    ...
    map_change = max(float(np.max(abs(r["relative_force_change"]))) for r in differences if r["source_variant"] == "coarse_map")
    flux_error = max(abs(r["monopole_flux_relative_mass_error"]) for r in rows)
    return {"rows": rows, "maximum_numerical_relative_force_change": refinement, "source_sensitivities": differences,
            ...
            "maximum_map_refinement_relative_force_change": map_change,
            "map_refinement_target_met": map_change < config["independent_controls"].get("map_refinement_relative_force_max", .03),
            "maximum_monopole_flux_relative_mass_error": flux_error,
            "monopole_flux_target_met": flux_error < config["independent_controls"].get("monopole_flux_relative_mass_error_max", .003),
...
def main():
    ...
    parser.add_argument("--config", type=Path, default=ROOT/"configs/gravity_map_axisymmetric_source_v1.json")
    args = parser.parse_args()
    args.output.mkdir(parents=True, exist_ok=False)
    config_path = args.config.resolve()
    if not config_path.is_relative_to(ROOT):
        raise ValueError("configuration must be within the research checkout")
    ...
    try:
        ...
        result = {**provenance, "config": config, "controls": checks, "field_bridge": fields,
                  ...
                  "status": "SOURCE_AND_NUMERICAL_BRIDGE_RETAINED" if checks["passes"] and fields["numerical_target_met"] and fields["map_refinement_target_met"] and fields["monopole_flux_target_met"] else "NUMERICAL_BRIDGE_UNRESOLVED_RETAINED"}
