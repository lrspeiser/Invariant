...
def main():
    ...
    registration = {'source_result_sha256': digest, 'fractional_stencil_steps': [.0005, .00025],
        ...
        'maximum_high_precision_moment_third_error_bound_at_admission_radius': 1e-10,
        'admission_radius_kpc': result['config']['canonical_minimum_radius_kpc'],
    ...
    try:
        ...
        for record in result['records']:
            ...
            recombined = []
            for l in range(moments['maximum_order']+1):
                value = mp.mpf(0)
                if l % 2 == 0:
            ...
            bound = mp.fsum([abs(v-mp.mpf(moments['scaled_multipole_moments'][l]))
                *(mp.mpf(moments['scale'])/registration['admission_radius_kpc'])**l*(l+3)**6 for l, v in enumerate(recombined)])/mp.mpf(moments['compact_source_mass'])
            moment_rows.append({'variant': name, 'precision_digits': 80, 'third_tensor_rounding_bound_at_admission_radius': float(bound)})
        ...
        passed = (all(v < registration['maximum_fine_stencil_monopole_scaled_error'] for r in fine for v in r['maximum_errors'].values())
            and all(r['third_tensor_rounding_bound_at_admission_radius'] < registration['maximum_high_precision_moment_third_error_bound_at_admission_radius'] for r in moment_rows))
