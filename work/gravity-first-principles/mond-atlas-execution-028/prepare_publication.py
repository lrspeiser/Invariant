"""Verify inherited artifacts and prepare exact scoped publication."""
import hashlib,json,subprocess
from pathlib import Path
R=Path(__file__).resolve().parents[3];P=Path(__file__).resolve().parent
prior=R/'work/gravity-first-principles/mond-atlas-execution-027/publication-manifest.json'
...
for pattern in ['mond_atlas_external_joint*.py','mond_atlas_noise_dc_regularization.py','mond_atlas_foreground_metadata.py','mond_atlas_normalization.py']:
    paths.update(p.relative_to(R).as_posix() for p in (R/'scripts').glob(pattern))
for name in ['mond-atlas-external-joint-001','mond-atlas-noise-dc-regularization-001','mond-atlas-foreground-metadata-001','mond-atlas-normalization-001','mond-atlas-execution-028']:
...
paths.add(manifest.relative_to(R).as_posix());private=R/'work/private/mond-atlas-execution-028';private.mkdir(exist_ok=False)
