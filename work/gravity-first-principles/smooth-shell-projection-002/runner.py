...
def projected(R,s,a,n):
    ...
    theta=(q+1)*np.pi/4
    return float(np.sum(w*np.pi/4*np.sin(theta)*offset_fraction(R,a,s*np.sin(theta))))
...
dest=root/'work/gravity-first-principles/smooth-shell-projection-002'
dest.mkdir(exist_ok=False)
registration=dict(shell_radius=1.,widths=[.02,.1,.2],
    radii=[.001,.01,.1,.5,.9,1.,1.1,2.,10.,100.],nodes=[64,128,256],
    relative_target=1e-7,scope='Use polar angle with its sine Jacobian to resolve small projected offsets; compare orientation-averaged offset Plummer apertures with direct cylinder integration of the spherical convolution density. No cluster source or gravity admission.')
