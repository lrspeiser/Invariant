"""Small-domain interpolation pilot; no observational scoring or admission."""
...
def main():
    ...
    try:
        ...
        _, disks = regular_disks(profile, {'id': 'primary'})
        coarse_r, coarse_z = np.arange(17)*.25, np.arange(9)*.1
        fine_r, fine_z = np.arange(33)*.125, np.arange(17)*.05
        probe_r, probe_z = coarse_r[:-1]+.0625, coarse_z[:-1]+.025
