...
summary=read(P/'emitter002/summary.json');history=read(R/'work/gravity-first-principles/mond-atlas-native-spectral-001/NGC2976.json');p=history['provenance'];n=p['parent_channel_count'];cal=np.array(p['continuum_fit_parent_indices_zero_based']);keep=np.array(p['parent_channel_indices_zero_based']);header=fits.getheader(R/history['source_path']);dv=abs(header['CDELT3'])/1000
