...
def main():
    ...
    for name in registration['names']:
        try:
            ...
            full=fitsout[-1];train=[];test=[]
            with torch.no_grad():
                for beta in registration['gas_beta_grid']:
            ...
            row=dict(name=name,fits=fitsout,gas_train_loss=train,gas_test_loss=test)
    ...
    # Only fully converged nuisance fits are eligible for the transferable term.
    eligible=[r for r in rows if all(f['optimizer_success'] for f in r['fits'])]
    gas=[]
    if len(eligible)>=3:
        for row in eligible:
            other=[r for r in eligible if r['name']!=row['name']]
            normalized=np.array([np.array(r['gas_train_loss'])/r['fits'][-1]['train_loss'] for r in other])
            index=int(np.argmin(normalized.mean(axis=0)));baseline=row['fits'][-1]['test_loss']
            gas.append(dict(name=row['name'],beta=registration['gas_beta_grid'][index],baseline_test_loss=baseline,
                gas_test_loss=row['gas_test_loss'][index],fractional_improvement=1-row['gas_test_loss'][index]/baseline))
    result=dict(status='COMPLETED_DEVELOPMENT_PILOT',objects=rows,failures=failures,eligible_gas_objects=len(eligible),gas_leave_galaxy_out=gas,
    ...
    print('DONE',len(rows),'fits',len(failures),'failures',len(gas),'gas tests',flush=True)
