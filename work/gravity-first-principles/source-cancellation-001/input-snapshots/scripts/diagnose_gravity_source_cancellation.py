    ...
    for variant in variants:
        ...
        for R,z in config['points_kpc']:
            ...
            for card in cards:
                ...
                values = {str(order):np.zeros(3) if length == 0 else prototype(spec,p,H,dH2,dlap,length,order) for order in [16,32]}
                ...
                rows.append({'variant':variant,'R_kpc':R,'z_kpc':z,'card':card['id'],
                    'h_over_x_plus_epsilon_squared':float(length**2*np.sum(H*H)/(np.dot(p,p)+spec.epsilon**2)),
