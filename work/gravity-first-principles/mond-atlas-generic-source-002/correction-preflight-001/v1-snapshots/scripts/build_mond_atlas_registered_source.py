...
def reference_wcs(header):
    # Isolate the two celestial axes of the already collapsed moment image.
    # The lightweight FITS reader represents integers as floats. SIP is
    # deliberately excluded here under the recorded core-TAN calibration.
    keys = ('CTYPE1','CTYPE2','CRVAL1','CRVAL2','CRPIX1','CRPIX2',
            'CD1_1','CD1_2','CD2_1','CD2_2','CDELT1','CDELT2',
            'CUNIT1','CUNIT2','LONPOLE','LATPOLE','RADESYS','EQUINOX')
    spatial = {key:header[key] for key in keys if key in header}
    spatial.update(NAXIS=2,NAXIS1=int(header['NAXIS1']),NAXIS2=int(header['NAXIS2']))
    return WCS(spatial,naxis=2)
