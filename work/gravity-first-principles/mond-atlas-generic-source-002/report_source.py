...
def main():
    ...
    for name in ('distance_low','distance_high'):
        ...
        for r in [x for x in s['mass_cases'] if x['case']==name]:
            ...
            distance.append(dict(case=name,conversion=r['conversion'],fill=r['fill'],expected_ratio=ratio,relative_error=err,threshold=c['benchmarks']['distance_scaling_relative_max'],passed=err<=c['benchmarks']['distance_scaling_relative_max']))
    distance_pass=all(r['passed'] for r in distance)
    counterexamples=[]
    with np.load(ROOT/cases['nominal']['packet']) as nominal:
        for name in ('distance_low','distance_high'):
            with np.load(ROOT/cases[name]['packet']) as trial:
                for component in COMPONENTS:
                    a=nominal[component+'_axis'];b=trial[component+'_axis']
                    rad=np.hypot(*np.meshgrid(a,a,indexing='ij'));rt=np.hypot(*np.meshgrid(b,b,indexing='ij'))
                    normalized=rad/cases['nominal']['grid']['annulus_width_kpc'];rn=np.floor(normalized).astype(int)
                    tn=np.floor(rt/cases[name]['grid']['annulus_width_kpc']).astype(int);inside=rad<c['source_grid']['cutoff_kpc'];changed=(rn!=tn)&inside
                    cov=nominal[component+'_coverage'];covt=trial[component+'_coverage']
                    counterexamples.append(dict(case=name,component=component,ring_labels_changed_inside_cutoff=int(changed.sum()),changed_labels_max_distance_to_integer=float(np.max(abs(normalized[changed]-np.rint(normalized[changed])))),maximum_coverage_difference=float(np.max(abs(cov-covt))),trusted_mask_changed_inside_cutoff=int(np.sum(((cov>=.5)!=(covt>=.5))&inside)),observed_relative_l1=cases[name]['comparison_to_nominal_on_same_angular_index'][component]['coordinate_matched_signed_relative_l1']))
    ...
    write_json(out/'packet-replay.json',dict(packet_integrals_passed=True,component_cases=checks,mass_rows_replayed=72,distance_scaling_passed=distance_pass,distance_scaling=distance,observed_response_arrays_opened=0,raw_source_arrays_reopened=False))
    write_json(out/'distance-scaling-counterexample.json',dict(status='BENCHMARK_FAILED',gate='actual annular-fill distance scaling',threshold=c['benchmarks']['distance_scaling_relative_max'],maximum_total_mass_relative_error=max(r['relative_error'] for r in distance),cases=counterexamples,diagnosis='Dimensionful floor(radius/annulus_width) changes exact ring-boundary cells under floating point distance rescaling. Saved observed maps and coverage agree at roundoff; trusted masks do not change. Annular fill depends on changed ring membership.',failed_original_report='findings-001/failure.json',builder_or_threshold_changed=False))
    ...
    write_json(out/'summary.json',dict(status='EXECUTED_WITH_BENCHMARK_FAILURE',disposition='SOURCE_BLOCKED',numerical_disposition='BENCHMARK_FAILED',object_id='NGC3198',geometry_registration_cases=12,component_cases=36,mass_conversion_fill_rows=72,nominal_mass=nom,convergence=convergence,aperture_losses=truncation,sensitivity=sensitivity,actual_distance_scaling_passed=distance_pass,new_gravity_scores=0,observed_response_arrays_opened=0,gpu_used=False,numerical_threads=1,bindings={p.relative_to(ROOT).as_posix():digest(p) for p in [Path(__file__),source/'summary.json',P/'freeze.json']}))
    lines=['# NGC3198 executed source-only pilot','','Disposition: **SOURCE_BLOCKED**; actual annular-fill distance-scaling gate **BENCHMARK_FAILED**. Twelve registered geometry/quadrature cases, 36 component grids and 72 conversion/fill mass rows executed using the unchanged generic builder. One CPU numerical thread; no GPU, new downloads, motion/lensing response arrays or gravity scores. These are repeated conditional representations of the same observations, not independent measurements.','',
