...
def stencil_indices(R, z, rr, zz, axis, active_r, offset, step):
    """Shift only the rows used by this stencil and verify every grid lookup."""
    xr = np.where(active_r, R+step*offset, R) if axis == 0 else R
    hz = z+step*offset if axis == 1 else z
    ri, zi = np.searchsorted(rr, abs(xr)), np.searchsorted(zz, hz)
    if (np.any(ri >= len(rr)) or np.any(zi >= len(zz)) or
            not np.array_equal(rr[ri], abs(xr)) or not np.array_equal(zz[zi], hz)):
        raise ValueError('A required stencil coordinate is absent from the evaluation grid')
    return ri, zi, xr
                ...
                for axis in [0, 1]:
                    for name, (offsets, coefficients) in stencils.items():
                        if axis == 1 and name != 'central':
                            continue
                        active = (~interfaces if name == 'central' else interfaces) if axis == 0 else np.ones(len(R), bool)
                        for offset, coefficient in zip(offsets, coefficients, strict=True):
                            if coefficient:
                                stencil_indices(R, z, rr, zz, axis, active, offset, step)
                                checked += int(active.sum()*len(z))
                write(f"sampling_preflight_{variant['id']}_{step}.json", {'all_required_lookups_present': True,
                    'active_point_offset_lookups_checked': checked, 'radial_evaluations': len(rr), 'vertical_evaluations': len(zz)})
                shape = (len(rr), len(zz))
                ...
                for axis in [0, 1]:
                    for name, (offsets, coefficients) in stencils.items():
                        ...
                        for offset, coefficient in zip(offsets, coefficients, strict=True):
                            if coefficient == 0:
                                continue
                            ri, zi, xr = stencil_indices(R, z, rr, zz, axis, active_r, offset, step)
