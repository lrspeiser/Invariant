...
D=ROOT/'work/gravity-first-principles/things-observable-acquisition-003'
PRIVATE=ROOT/'work/private/things-observable-12gal-003'
...
historical=json.loads((ROOT/'configs/open_gravity_rg_things_six_external_2d_replication_preflight_v1.json').read_text())
known={a['url'] for a in historical['response_assets']}|{s['url'] for o in pre['object_source_contracts'] for s in o['hi_files']}
        ...
        for moment in (0,1,2):
            ...
            if url not in links:
                ...
                if matches:
                    ...
                    assert url in known, 'Require an exact historical source URL if missing from current index.'
            assets.append(dict(name=obj['object_id'],resolution='NA' if '_NA_' in url else 'RO',
                moment=moment,url=url,current_index_link=url in links,file=PRIVATE/Path(url).name))
