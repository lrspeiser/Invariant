...
def main():
    ...
    for name in ['NGC2976','NGC3198']:
        ...
        h=fits.getheader(path);w=WCS(h).spectral;v=w.pixel_to_world_values(np.arange(h['NAXIS3']));assert np.isfinite(v).all() and np.all(np.diff(v)<0)
    ...
    for j,y in enumerate(range(12,h['NAXIS2'],24)):
        for i,x in enumerate(range(12,h['NAXIS1'],24)):
            ...
            xx,yy=np.meshgrid(x-6-.5+np.arange(13),y-6-.5+np.arange(13));sky=w.pixel_to_world(xx,yy);east,north=origin.spherical_offsets_to(sky);e=east.rad*D;n=north.rad*D
