...
def beta_zero_density_source(fields, a0, power):
    """Independent div(s grad(psi)/a0), including matter, in cylindrical components.

    The Hessian and the vectors in its contraction must share their coordinate
    basis; a radial/theta vector cannot be contracted with a cylindrical Hessian.
    """
    if not np.isfinite(a0) or a0 <= 0 or type(power) is not int or power not in (1, 2):
        raise ValueError("positive a0 and power 1 or 2 required")
    p = np.asarray(fields['gradient'])/a0
    x = np.sum(p*p, axis=0)
    s, ds = 1/(1+x)**power, -power/(1+x)**(power+1)
    directional = np.einsum('i...,ij...,j...->...', p, fields['hessian'], p)
    return (s*fields['laplacian']+2*ds*directional)/a0
