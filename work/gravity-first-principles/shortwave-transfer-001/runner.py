...
dest=root/'work/gravity-first-principles/shortwave-transfer-001'
...
registration=dict(module_sha256=hashlib.sha256(module.read_bytes()).hexdigest(),
    ...
    wave_number_ratios=[.5,2.],gradient_perturbation_ratios=[1e-5,1e-6],
...
for shape in registration['shapes']:
    ...
    for x in registration['x_values']:
        ...
        for ratio in registration['wave_number_ratios']:
            ...
            for small in registration['gradient_perturbation_ratios']:
                amplitude=small*np.sqrt(x)/k
                phase=np.pi/4
                ...
                checks.append(dict(k_over_critical=ratio,relative_gradient_amplitude=small,
...
out=dict(registration=registration,rows=rows,
    maximum_smallest_amplitude_error=max(c['scaled_error'] for r in rows for c in r['checks'] if c['relative_gradient_amplitude']==1e-6),
