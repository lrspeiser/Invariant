...
def derivative_check(provider, R, z, step, scale, *, radial_stencil='central'):
    if radial_stencil not in {'central','left','right'}:
        raise ValueError('central, left or right radial stencil required')
    ...
    for axis in range(2):
        ...
        for offset in [-2,-1,1,2]:
            ...
            samples[offset] = f
        keys = ['potential','gradient_R_z','hessian_RR_Rz_zz_pp']
        accum = {key: (8*(samples[1][key]-samples[-1][key])-
            (samples[2][key]-samples[-2][key]))/(12*step) for key in keys}
        if axis == 0 and radial_stencil != 'central':
            interfaces = np.isin(R,provider.near.radius) & (R > 0) & (R <= provider.outer)
            if np.any(interfaces):
                direction = -1 if radial_stencil == 'left' else 1
                one_sided = {key: np.zeros_like(base[key][...,interfaces]) for key in keys}
                for offset,weight in [(1,48), (2,-36), (3,16), (4,-3)]:
                    f = provider.fields(R[interfaces]+direction*offset*step,z[interfaces])
                    for key in keys:
                        one_sided[key] += weight*(f[key]-base[key][...,interfaces])/(12*direction*step)
                for key in keys:
                    accum[key][...,interfaces] = one_sided[key]
...
def main():
    ...
    config = {'inner_kpc': 60., 'outer_kpc': 80., 'targets': {'force': 1e-4, 'hessian': .002,
        ...
        'interface_rule': 'Retain central results, but qualify both one-sided stencils at positive radial tensor boundaries; central elsewhere. C3 potentials need not have continuous fourth derivatives.',
        'cancellation_rule': 'Subtract paired field values before weighting or dividing; one-sided stencils subtract their common base first',
                ...
                if grid == 'fine':
                    ...
                    for step in config['derivative_steps_kpc']:
                        for stencil in ['central','left','right']:
                            errors = derivative_check(provider,DR,DZ,step,ds,radial_stencil=stencil)
                            derivative_rows.append({'step_kpc':step,'stencil':stencil,'errors':maxima(errors,DR,DZ)})
                            write(f'derivatives_{variant}_{step}_{stencil}.json',{'R_kpc':DR,'z_kpc':DZ,'errors':errors})
                    row['independent_derivatives'] = derivative_rows
                    row['fine_derivative_checks_pass'] = all(v['value'] < config['fine_derivative_target']
                        for record in derivative_rows if record['step_kpc']==config['derivative_steps_kpc'][-1]
                        and record['stencil'] in ['left','right'] for v in record['errors'].values())
