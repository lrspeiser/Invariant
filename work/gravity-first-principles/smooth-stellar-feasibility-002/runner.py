...
dest=base/'smooth-stellar-feasibility-002'
...
for p in packet['stellar_packets']:
    ...
    for width in registration['width_to_shell_radius']:
        previous=base/'smooth-stellar-feasibility-001'/f"case_{p['cluster']}_{width}.json"
        if previous.exists():
            row=json.loads(previous.read_bytes())
            row['reused_case_sha256']=hashlib.sha256(previous.read_bytes()).hexdigest()
            rows.append(row)
            (dest/previous.name).write_text(json.dumps(row,indent=2),encoding='utf-8')
            continue
        ...
        if answer.success:
            ...
            consistency=bool(abs(violation-answer.x[-1])<2e-8 and np.all(masses>=0))
            row.update(constraint_consistency_pass=consistency,minimum_component_mass=float(masses.min()),relaxation_discrepancy=float(abs(violation-answer.x[-1])))
            row.update(relaxation=float(answer.x[-1]),verified_bracket_violation=violation,
                prediction_change_over_mass=change,
                numerical_pass=consistency and error<1e-6 and change<1e-6,
