...
def sample(asset,ra,dec):
 ...
 try:x,y=w.all_world2pix(ra,dec,0,maxiter=100)
 except NoConvergence as error:
  best=error.best_solution.copy()
  for indices in (error.divergent,error.slow_conv):
   if indices is not None:best[indices]=np.nan
  x=best[:,0].reshape(ra.shape);y=best[:,1].reshape(ra.shape)
 return map_coordinates(a,[y,x],order=1,mode='constant',cval=np.nan)

def main():
 dest=ROOT/'work/gravity-first-principles/cube-gas-coverage-002';dest.mkdir(exist_ok=False)
