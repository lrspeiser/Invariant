...
def main():
    ...
    try:
        ...
        for name in config['refined_source_variants']:
            ...
            newtonian = ReconstructedNewtonianSource.build(name, lambda R, z: 4*np.pi*G*sum(d.density(R, z) for d in disks.values()), density_grid)
            ...
            def solve(beta):
                a = solve_isolated_auxiliary(solver, p, beta, config['power'], **c['solver'])
                field = -a.physical_flux_potential.evaluate(radii, np.zeros_like(radii))['acceleration']*a0
