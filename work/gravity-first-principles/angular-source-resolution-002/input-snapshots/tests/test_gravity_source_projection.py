...
def test_even_projection_matches_exact_cartesian_polynomial_and_radial_derivative():
    ...
    for record in metrics[1:]:
        ...
        # Roundoff leakage from the large constant density into l=4,6,8 is
        # amplified by angular differentiation / r at the smallest shell.
        # Run001 retained the 2.35e-12 residual against an overly tight 1e-12
        # bound. This tolerance is for this synthetic projection only.
        assert max(record['gradient_L1_fraction_error']) < 1e-11
        assert max(abs(record['relative_shell_mass_error'])) < 1e-12
        assert max(record['negative_density_fraction']) == 0
    exact = {**projection, 'coefficients': expected,
             'radial_derivative_coefficients': derivative}
    for record in projection_metrics({'p': PolynomialSource()}, exact, [2, 8], 64):
        assert max(record['gradient_L1_fraction_error']) < 1e-14
