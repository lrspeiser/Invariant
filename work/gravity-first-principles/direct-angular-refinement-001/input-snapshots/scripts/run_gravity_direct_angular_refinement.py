...
def main():
    ...
    parent = base/'direct-difference-001'
    parent_result = json.loads((parent/'result.json').read_bytes())
    assert sha256((parent/'result.json').read_bytes()).hexdigest() == '299f258373aa0afc58511330f4f7dcd6e78a5661a8a487bf18b05df01c945d6c'
    baseline = parent_result['config']['grids']['radial']
    grids = {'baseline':baseline,
        'angles':{**baseline,'angular_nodes':1280},
        'multipoles':{**baseline,'angular_nodes':1280,'l_max':320},
        'upper_modes':{**baseline,'angular_nodes':1280,'l_max':480}}
    variants, distances = previous['config']['variants'], previous['config']['distances']
    config = {'registration':'Refine angular quadrature 640 to 1280 and orders 160 to 320 to 480 at fixed radial grid4097; reuse the completed parent radial baseline; retain all 54 cards, three distances and both thicknesses.',
        'grids':grids,'variants':variants,'distances':distances,'relative_peak_signal_target':.01,
        'comparison_pairs':[['angles','baseline'],['multipoles','angles'],['upper_modes','multipoles']],
        ...
        'baseline_comparison':'Reuse parent radial direct-response result; no repeated old subtraction comparison.',
    ...
    for variant in variants:
        ...
        paths.add(parent/f'family_{variant}_radial.json')
    paths.add(parent/'result.json')
    ...
    try:
        for variant in variants:
            ...
            for name,definition in grids.items():
                if name == 'baseline':
                    prior=json.loads((parent/f'family_{variant}_radial.json').read_bytes())
                    assert prior['grid']==definition and prior['radii_kpc']==radii.tolist()
                    for row in prior['predictions']:
                        row['response_gradient_R_z']=np.array(row['response_gradient_R_z'])
                        row['fixed_full_force_scale']=np.array(row['fixed_full_force_scale'])
                    families[variant+'/'+name]={(r['card'],r['distance']):r for r in prior['predictions']}
                    write(f'family_{variant}_{name}.json',prior)
                    print(variant+'/baseline: reused verified completed parent radial grid',flush=True)
                    continue
        ...
        assert len(comparisons)==972 and len(subtractions)==0
