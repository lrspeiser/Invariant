...
center_path=base/'stellar-measured-centering-001/result.json'
centers=json.loads(center_path.read_bytes())
measured={r['cluster']:r['offset_r500'] for r in centers['rows']}
paths.append(center_path)
r500={p['cluster']:p['r500_kpc'] for p in old['packets']}
dest=base/'stellar-measured-offset-feasibility-001'
dest.mkdir(exist_ok=False)
registration=dict(input_sha256={str(p.relative_to(root)):hashlib.sha256(p.read_bytes()).hexdigest() for p in paths},
    offset_fractions_r500=measured, scale_fractions_r500=np.geomspace(.001,.02,17).tolist(),
    runs=['shell_only','shell_plus_offset_components'],
    scope='All projected mass bounds retained. Positive shell masses plus nonnegative offset Plummer components. Each cluster uses its coordinate-derived BCG offset; scale remains free within the preset dictionary. No gravity residuals or physical source admission.')
...
for p in packet['stellar_packets']:
    ...
    extras=[dict(kind='offset_plummer',a_kpc=a*r500[p['cluster']],d_kpc=d*r500[p['cluster']])
            for a in registration['scale_fractions_r500'] for d in [measured[p['cluster']]]]
