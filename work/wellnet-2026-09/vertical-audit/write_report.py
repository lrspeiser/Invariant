"""Emit the lane REPORT.md.  The ranking table is read from model_compare.json
so the document cannot drift from the numbers."""
import json
import os

HERE = os.path.dirname(os.path.abspath(__file__))
mc = json.load(open(os.path.join(HERE, "model_compare.json")))
rk = mc.get("item4_nuisance_ranking", {})
rows = []
for n in ("M_global", "M_local_RAR", "M_local_AQUAL", "M_local_RAR_a0",
          "M_local_pow", "M_Phi_inf", "M_Newton", "M_tensor_aniso",
          "M_Phi_ref8", "M_Phi_ref4", "M_tensor_iso"):
    v = rk.get(n)
    if not v:
        continue
    sh = ("--" if v.get("shape_p50") is None
          else f"{v['shape_p16']:+.3f} to {v['shape_p84']:+.3f}")
    rows.append(f"| {n} | {v['median_dAIC']:.2f} | {v['p16']:.2f} | "
                f"{v['p84']:.2f} | {v['wins']} | {sh} |")
RANK = "\n".join(rows)

mn = json.load(open(os.path.join(HERE, "model_null.json")))
DR, DN = mn["observed_dAIC_RAR_minus_global"], mn["observed_dAIC_Newton_minus_global"]
NT = mn["n_trials"]
UB = mn["upper_bound_zero_count"]


def nrow(truth, key, obs):
    v = mn["nulls"][truth][key]
    return (f"| {truth}-truth | {v['median']:+.1f} | "
            f"[{v['p2p5']:+.1f}, {v['p97p5']:+.1f}] | "
            f"[{v['min']:+.1f}, {v['max']:+.1f}] | {v['p_ge_observed']:.3f} |")


NULLTAB = "\n".join([
    nrow("newton", "dAIC_Newton_minus_global", DN),
    nrow("rar", "dAIC_Newton_minus_global", DN)])
NULLTAB2 = "\n".join([
    nrow("newton", "dAIC_RAR_minus_global", DR),
    nrow("rar", "dAIC_RAR_minus_global", DR)])

TEXT = f"""# Vertical-audit lane — is `d log10 B_z / d log10 Sigma_0 = -0.346` real?
...
here as much as to the tensor detector. `model_null.py` generates {NT} synthetic
...
**Statistic 1: `dAIC(Newton - global)`, observed +{DN:.2f}** — does the data
...
**Statistic 2: `dAIC(RAR - global)`, observed +{DR:.2f}** — is what it demands
*local*?
...
the median `dAIC(RAR - global)` is already +{mn['nulls']['newton']['dAIC_RAR_minus_global']['median']:.1f},
...
realisations reaches it (max +{mn['nulls']['rar']['dAIC_RAR_minus_global']['max']:.1f}).
...
rejected (`P = {mn['nulls']['newton']['dAIC_Newton_minus_global']['p_ge_observed']:.3f}`,
95% bound {UB:.3f}); RAR is rejected
(`P = {mn['nulls']['rar']['dAIC_RAR_minus_global']['p_ge_observed']:.3f}`);
...
with open(os.path.join(HERE, "REPORT.md"), "w", encoding="utf-8") as fh:
    fh.write(TEXT)
print(f"wrote REPORT.md ({len(TEXT)} chars)")
