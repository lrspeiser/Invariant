"""Emit TWO_DIRECTION_INVENTORY.md -- the Test 2 deliverable.

Test 2 asks for systems in which two nearly perpendicular directions of the
gravitational field are measured in the SAME baryonic system.  Sections (a)-(e)
follow the task's own lettering.  Counts that this lane measured itself are
pulled from the produced artefacts; counts from the literature acquisitions are
taken from the sub-inventories in raw/<topic>/ and are cited to them.
"""
import json
import os
...
OUT = os.path.join(LANE, "TWO_DIRECTION_INVENTORY.md")
...
def main():
    ...
    mis = pd.read_csv(os.path.join(CLEAN, "manga_gas_star_misalignment.csv"))
    strong = mis[(mis.grad_stellar > 0.5) & (mis.grad_gas > 0.5)]
    prof = pd.read_csv(os.path.join(CLEAN, "manga_faceon_sigma_profiles.csv"))
    g = prof.groupby("plateifu")

    sami = pd.read_csv(os.path.join(LANE, "raw", "sami",
                                    "sami_dr3_master_galaxy_inventory.tsv"),
                       sep="\t", low_memory=False)
    for c in ("PA_STELKIN", "PA_GASKIN"):
        sami[c] = pd.to_numeric(sami[c], errors="coerce")
    d = np.abs(sami.PA_STELKIN - sami.PA_GASKIN) % 360.0
    sami["mis"] = np.where(d > 180, 360 - d, d)
    sm = sami.mis.dropna()

    w(HEAD)

    # ---- headline summary, computed from the artefacts
    w("## Summary: what exists, in one table")
    w("")
    w("| category | systems with BOTH directions measured | tabulated numerically? |")
    w("|---|---|---|")
    w("| (a) polar rings | 9 with rotation measured independently in both planes; "
      "40 kinematically confirmed PRGs in total | **no** -- 0 systems have a "
      "tabulated rotation curve in both planes; 2 have one plane tabulated |")
    w("| (b) warped H I disks | 15 with i(R), PA(R) and V(R) in one table "
      "(Verheijen & Sancisi 2001), of which ~9 are credibly warped; +1 (NGC 2685) | "
      "**yes**, and 26 flat-disk controls come with them |")
    w("| (c) stellar streams | 60 distinct Milky Way streams with a measured 3-D "
      "track, 30 with full 6-D; 16 / 8 of those nearly polar | **yes**, per-track |")
    w("| (d) satellite systems | 101 SAGA hosts with orientation + 378 satellites; "
      "125/132 hosts have axis ratio and PA | **yes**, but **no in-plane rotation "
      "curve for the hosts** -- a half-measurement |")
    w("| (e) two-component galaxies | %d MaNGA near-orthogonal + %d counter-rotating "
      "measured here with cubes on disk; %d / %d from SAMI's published PAs; 47 and 22 "
      "polar systems in the two best literature catalogues | **yes** |"
      % (band(strong.misalign_deg, 60, 120), band(strong.misalign_deg, 150, 181),
         band(sm, 60, 120), band(sm, 150, 181)))
    w("| vertical dispersion | %d MaNGA face-on disks with resolved sigma_LOS(R) "
      "measured here; 2 external galaxies and the Milky Way from the literature | "
      "**yes** |" % prof.plateifu.nunique())
    w("")
    w("**The single sharpest finding.** The configuration the task identified as "
      "most powerful -- polar rings -- is the one where the numbers do not exist. "
      "Nine systems have rotation measured independently in both planes and not "
      "one of them has both curves tabulated; the physics is published, the "
      "numbers are figures. Meanwhile the category the task listed last, "
      "two-component galaxies, turned out to be the largest by two orders of "
      "magnitude and is fully in hand, because integral-field spectroscopy "
      "measures both planes of the same galaxy by construction.")
    w("")
    w("---")
    w("")
    ...
    # ---------------------------------------------------------------- (e)
    w("## (e) Galaxies with two rotating components measured in different planes")
    w("")
    w("This is where the lane's own measurements land, and it is by a wide margin "
      "the largest two-direction sample it holds. A galaxy whose ionised gas "
      "rotates in a plane strongly misaligned from its stellar disk is the "
      "integral-field analogue of a polar-ring galaxy: one baryonic system, two "
      "rotation tracers, two planes. At 90 degrees it *is* a polar-gas system.")
    w("")
    w("### MaNGA, measured here from the DAP MAPS cubes")
    w("")
    w("`clean/manga_gas_star_misalignment.csv` (+ manifest). A weighted plane fit "
      "`V = a + b x + c y` inside 1.5 R_eff to the stellar and to the H-alpha "
      "velocity field of every cube this lane holds, giving a kinematic position "
      "angle for each and their difference folded onto [0, 180] deg. "
      "%d of %d cubes yielded both angles; %d have both components rotating "
      "strongly enough for the angle to be stable."
      % (len(mis), 902, len(strong)))
    w("")
    w("| misalignment | meaning | N | fraction |")
    w("|---|---|---|---|")
    for lo, hi, nm in ((0, 30, "co-rotating, one plane"),
                       (30, 60, "mildly misaligned"),
                       (60, 120, "**near-orthogonal: two planes**"),
                       (120, 150, "strongly misaligned"),
                       (150, 181, "**counter-rotating**")):
        n = band(strong.misalign_deg, lo, hi)
        w("| %d-%d deg | %s | **%d** | %.1f%% |"
          % (lo, min(hi, 180), nm, n, 100.0 * n / len(strong)))
    w("")
    w("Every one of these galaxies already has its DAP MAPS cube on disk, so the "
      "resolved velocity field in **both** planes is in hand, not merely a "
      "citation. The stellar and ionised-gas fields are independent tracers of "
      "the same potential in two different planes of the same galaxy.")
    w("")
    w("**This is a screen, not a measurement of record.** A linear gradient "
      "recovers the global rotation axis only: it cannot see a kinematically "
      "decoupled core, a warp, or a counter-rotating inner disk, and it degrades "
      "on a disturbed field. The 12 per cent near-orthogonal rate is higher than "
      "the literature rate for strong misalignment, which is what a crude "
      "estimator applied to low-gradient galaxies should do. Anything used as a "
      "detection needs a proper kinematic position-angle fit (Krajnovic et al. "
      "2006) on the same cube first. What the screen delivers is a ranked "
      "shortlist with the data already local.")
    w("")
    w("### SAMI, from the published kinematic position angles")
    w("")
    w("SAMI DR3 publishes `PA_STELKIN` and `PA_GASKIN` per galaxy, so the same "
      "quantity comes for free and from a properly fitted kinematic PA rather "
      "than a gradient screen. %d galaxies have both." % len(sm))
    w("")
    w("| misalignment | N | fraction |")
    w("|---|---|---|")
    for lo, hi, nm in ((0, 30, "co-rotating"), (30, 60, "mild"),
                       (60, 120, "**near-orthogonal**"), (120, 150, "strong"),
                       (150, 181, "**counter-rotating**")):
        n = band(sm, lo, hi)
        w("| %d-%d deg (%s) | **%d** | %.1f%% |" % (lo, min(hi, 180), nm, n,
                                                    100.0 * n / len(sm)))
    w("")
    w("The two surveys agree that the counter-rotating population is a few per "
      "cent and the near-orthogonal population around 12-16 per cent of "
      "rotating galaxies. SAMI's higher rate is expected: its PAs are fitted "
      "properly, so genuinely misaligned systems are not diluted, but its sample "
      "also reaches lower stellar masses where misalignment is commoner.")
    w("")
    w(SEC_E_LIT)

    # ---------------------------------------------------------------- sigma_z
    w("## Vertical-field measurements: resolved dispersion profiles")
    w("")
    w("Not one of the task's five lettered categories, but the same physics and "
      "the specific upgrade the task note asked for. The programme's existing "
      "DiskMass VI/VII holding gives an exponential *fit* (a central sigma_z "
      "plus a scale length in arcsec) for 30 galaxies, with the scale height "
      "*inferred* from h_R so the two columns are correlated by construction.")
    w("")
    w("### MaNGA face-on disks, measured here")
    w("")
    w("`clean/manga_faceon_sigma_profiles.csv` (+ manifest): **%d galaxies**, "
      "%d radial points, %d-%d points each, median formal error %.2f km/s. "
      "sigma_LOS spans %.1f-%.1f km/s. Profiles decline outward in %.0f%% of "
      "galaxies with median d log sigma / d log R = %.2f."
      % (prof.plateifu.nunique(), len(prof), g.size().min(), g.size().max(),
         prof.e_sigma_los_kms.median(), prof.sigma_los_kms.min(),
         prof.sigma_los_kms.max(),
         100 * np.mean([np.polyfit(np.log10(x.r_med_Re.clip(0.05)),
                                   np.log10(x.sigma_los_kms), 1)[0] < 0
                        for _, x in g if len(x) >= 4]),
         np.median([np.polyfit(np.log10(x.r_med_Re.clip(0.05)),
                               np.log10(x.sigma_los_kms), 1)[0]
                    for _, x in g if len(x) >= 4])))
    w("")
    w("Eight times DiskMass's galaxy count with actual radial profiles rather "
      "than a fit. The honest limits: it is sigma_LOS, not sigma_z (the in-plane "
      "components leak in at order sin^2 i, under 0.25 here); no scale height and "
      "no Sigma_dyn is produced, so the DiskMass h_z problem is sidestepped "
      "rather than solved; and MaNGA's instrumental sigma is around 70 km/s, so "
      "**%d of %d galaxies have every radial point above 50 km/s and %d above "
      "70 km/s**. The %d-galaxy fully-above-70 subset is the one to trust "
      "without argument."
      % (int((g.sigma_los_kms.min() > 50).sum()), prof.plateifu.nunique(),
...
SEC_CD = r"""## (c) Stellar streams above galactic disks
...
SEC_A = r"""## (a) Polar-ring galaxies -- the strongest configuration, and a clean negative
...
SEC_B = r"""## (b) Warped H I disks with measured warp geometry and rotation
...
HEAD = r"""# TWO_DIRECTION_INVENTORY.md -- systems that measure two field directions at once
...
SEC_E_LIT = r"""### From the literature
...
LIT_SIGMA = r"""### From the literature
