...
| # | Goal | Current measured outcome | Status |
|---:|---|---|---|
| 27 | Solar one-shot trial | Two candidates BLOCKED; zero primary reads, held-out reads, or real evaluations. A current closure audits 10 bound metadata artifacts and finds zero authorized observational artifacts. Each candidate still lacks source-branch instantiation, split commitment, selected primary roots, and opening authorization: eight missing commitments total. The future opening-packet preflight is consume-once and target-free, but the post-authorization path also lacks a registered action-bound real-record likelihood executor; the existing evaluator is a synthetic GR known-answer control only. | Correctly fail-closed without opening data. External authorization and a real-record executor are both required. |
