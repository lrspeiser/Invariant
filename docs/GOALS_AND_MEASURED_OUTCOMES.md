...
| 22 | Nonlinear second-variation coverage | Exact cross-direction replay sealed 5,082 additional off-diagonal entries per candidate. Coverage remains 5,324 of 257,499 ordered D2F entries per candidate (60,984 new candidate-bound records globally); 252,175 remain per candidate. A new audit shows that 22 formal direction records represent only 20 unique coordinate vectors. It constructs the entire 133-vector complement for all 12 candidates—1,596 exact tangent certificates in 96 chained checkpoints—so all 153 unique coordinate directions are now sealed. The arbitrary-background tensor formula projects all 99 principal second-jet directions, and the lower-coordinate gate now projects the remaining 54 q/p directions. This adds 648 candidate-bound lower projection certificates, preserves 24 inherited alias reconciliations, and establishes an alias-aware 153/153 coordinate union. | Coordinate-basis coverage and coordinate-to-covariant projection are complete, but no projection certificate is misbooked as a D2 value. D2 remains 5,324/257,499 until candidate-bound derivatives for the 31,680 reachable A/B/C component-input leaf obligations are registered and propagated through every D1 arithmetic DAG. |
...
4. Materialize the general gauge-map scalar values and a common candidate-jet domain,
   lift the 96/96 cylindrical rows to the general domain and close sourced constraint propagation, then address
   Maxwell external-current/boundary terms and a separate vortical-fluid model.
