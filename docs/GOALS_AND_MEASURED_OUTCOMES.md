...
| 22 | Nonlinear second-variation coverage | Exact cross-direction replay sealed 5,082 additional off-diagonal entries per candidate. Coverage remains 5,324 of 257,499 ordered D2F entries per candidate (60,984 new candidate-bound records globally); 252,175 remain per candidate. A new audit shows that 22 formal direction records represent only 20 unique coordinate vectors. It constructs the entire 133-vector complement for all 12 candidates—1,596 exact tangent certificates in 96 chained checkpoints—so all 153 unique coordinate directions are now sealed. The arbitrary-background tensor formula now projects all 99 principal second-jet directions: nine scalar-Hessian plus 90 metric/Einstein directions. This replays the prior 20 unique directions, registers 79 new unique projections per candidate, reconciles both duplicated formal slots, and leaves exactly 54 lower q/p directions unresolved. | Coordinate-basis coverage and the principal second-jet covariant projection are complete, but no projection certificate is misbooked as a D2 value. The next blocker is the nonlinear projection for the 54 lower directions followed by exact propagation of all 153 directions through every D1 arithmetic DAG. |
