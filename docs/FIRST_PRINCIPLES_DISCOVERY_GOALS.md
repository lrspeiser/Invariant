...
| **D4** | **Symmetry- and dimension-forced derivation.** Derive the form of a law from invariance plus dimensional analysis alone. | Declare the symmetry group and dimensions; enumerate invariants; report the forced functional form and its free parameters. | A derivation admitting a form that violates the declared invariance. | CPU | **Built for four bounded controls.** One declaration engine derives the primitive pendulum, Kepler, diffusion, and Reynolds invariant coordinates. Fraction Gaussian elimination and SymPy exact nullspaces agree; exponent-offset mutations fail, and deleting each declared nuisance symmetry increases nullity and admits a spurious coordinate. The core live path now injects the sealed briefs, typed grammar, proof mechanisms, and origin labels into every future provider request and binds their context hash per call. The preserved authenticated calls predate this path, so live prompt consumption remains explicitly unevidenced until an authorized rerun. The receipt preserves the arbitrary `F(pi_1)=0`: dimensional admissibility is not a discovered law, empirical fit, or novelty result. Receipt: `runs/math/symmetry-dimension-derivation/receipt.json` |
