...
| **I2** | **One-sided sampling error.** Sampling may only ever *overstate* the solution space, never understate it. | Sampled nullity must be at least the count of explicitly exhibited members; uniqueness claimed only when the sandwich closes on held-out samples. | A uniqueness claim where exhibited members are fewer than sampled nullity — sampling manufactured the result. | CPU | **FAILING.** Reproduced: `bank_samples=1` yields `sampled_dimension=1` against `independently_exhibited_dimension=2` — below the exhibited count, the wrong direction. Flagged by `sandwich_tight:false`, not prevented |
...
| **D5** | **Domain transfer without modification.** The same derivation engine works in a second domain with only declarations changed. | Run D1's machinery on a non-gravitational system (elasticity, fluid, gauge) with zero code changes. | Any code change required to change domain. | CPU | **Falsifier already fires.** `declared_framework` is consumed nowhere — it appears at `tensor_constraint_search.py:114` (key name) and `:1788` (verbatim receipt copy), with 0 references in the 628-line test file. A config declaring flat Euclidean 3-space and a strain tensor still returns `['g_mn', 'R_mn - (1/2) R g_mn']`. The receipt can carry an elasticity claim over a GR computation |
