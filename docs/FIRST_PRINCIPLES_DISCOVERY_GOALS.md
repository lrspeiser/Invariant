...
| **D3** | **New equation *kinds*, not new instances.** The engine extends its own grammar. | Declare a grammar, statement, tactic, or gate as data; the engine admits it under the same verification discipline as mathematics and searches the new space with no new hand-written module. | Any new capability still requiring a hand-written module and a manual topology pin. | LLM + CPU | **Built for a bounded declarative interpreter.** `declarative_discovery_runtime.py` admits operator, invariant, proof-tactic, and grammar declarations from strict JSON after two independently identified schema/replay checks, then executes them without a per-extension Python module. The operational receipt runs all four extension kinds, all ten creativity families, feature-level reachability, proof-plan search, the sealed dataset stages, and an evidence-backed blind ladder through a prospective decidable world. The opcode set remains deliberately closed, and neither the control campaign nor its metrics establish autonomous novelty or a solved open problem. Receipt: `runs/math/declarative-discovery-platform/operational-v2.json` |
| **D4** | **Symmetry- and dimension-forced derivation.** Derive the form of a law from invariance plus dimensional analysis alone. | Declare the symmetry group and dimensions; enumerate invariants; report the forced functional form and its free parameters. | A derivation admitting a form that violates the declared invariance. | CPU | **Built for five bounded controls.** One declaration engine derives primitive pendulum, Kepler, diffusion, Reynolds, and drag invariant coordinates. Fraction Gaussian elimination and SymPy exact nullspaces agree; six exponent-offset mutations fail, five collapsed bases fail, and deleting each declared nuisance symmetry admits a spurious coordinate. Drag supplies a two-coordinate quotient while the other four controls supply one coordinate. All eight authenticated core calls bind and consume the sealed briefs, typed grammar, proof mechanisms, and origin labels through the same context hash. The receipts preserve arbitrary `F(pi)=0` and `F(pi_1, pi_2)=0` families: dimensional admissibility is not a discovered law, empirical fit, or novelty result. Receipt: `runs/math/symmetry-dimension-derivation/receipt.json` |
...
| # | Goal | Test | Falsifier | Lane | Current |
|---|---|---|---|---|---|
| **C1** | **Reachability certificates.** Before a search runs, prove the target class is *inside* the declared space. A null result must distinguish "no such object exists" from "our grammar could not express it". | For a declared space and a declared target class, emit a certificate: either an explicit embedding of the class into the space, or a proof that the class is outside it. Control: plant a known answer in the space and confirm the certificate finds it reachable. | Any exhaustive search reported as a negative result without a reachability certificate for what it was looking for. | CPU | **Wired.** `certified_null_search.py` is the consumer: it runs a real exhaustive CPU search over a declared fragment at the real mode C program length (262,144 sequences, 7,168 valid programs, coverage argument checked against the closed-form tree count) and adjudicates its null to REAL_NEGATIVE, UNINFORMATIVE_NULL, or INCONSISTENT from a *verified* certificate. `run_campaign` now attaches a 24-cell block (12 targets x 2 modes) to every receipt it seals, and `validate_receipt` **refuses** a receipt reporting a zero headline without one. Mode C reachability proved for 9/12 targets, mode F for 3/12 via a new `summation`/`limit` lane; `zeta3`, `catalan`, `euler_gamma` stay UNRESOLVED, so their nulls are reported as uninformative rather than as results. 12 forged adjudications and 2 new in-campaign tamper probes all rejected |
...
| **C6** | **The search learns.** A 10^12 sweep must be steered by what survived the last one. | Compare the proposal distribution at the start and end of a campaign; survivors must shift it measurably. | Enumeration whose distribution is identical at both ends — that is a lottery with extra steps, not a search. | GPU + LLM | **Measured, and the answer is no. The first measurement of it was wrong.** `funsearch_learning.py` maps every proposal to integer counts over a closed 68-symbol alphabet and projects the start-to-end drift onto the axis separating the top-scoring from the bottom-scoring pole of the mid-campaign population, in exact rationals. The null shipped on `agent/c6-learning` permuted *generations* while holding that axis fixed, so it tested whether the distribution drifted and never put the score axis at risk: handed a score-blind axis on a drifting stream it fires 29 times in 60 at alpha 1/20, against 1 and 0 for the two pole-randomised nulls that replace it. Under those, over 12 seeded campaigns of the deterministic-mutator loop, the live arm clears the pole-randomised null on 1 and the generation-stratified one on 0, while the old null clears on 6; the `selection_pressure=False` ablation clears none of the three on any campaign. Five of the twelve live campaigns carry a measured detection floor and are real negatives. The other seven have no floor and are sealed `uninformative_null` rather than counted as evidence, because in an improving campaign the elite pole *is* the late pole and the score axis is collinear with the clock. Receipt `runs/math/funsearch/learning-v2.json` |
...
The detailed successor plan is
[`NO_DARK_MATTER_GRAVITY_DISCOVERY_GOALS.md`](NO_DARK_MATTER_GRAVITY_DISCOVERY_GOALS.md).
It keeps R1/R2 as the measured starting point, but expands reconciliation into an ordered
galaxy-atlas, formula-equivalence, meta-law, universal-galaxy, external-replication, cluster,
lensing, inverse-action, local-gravity, wave, and cosmology ladder. That document is
authoritative where this older five-row summary is less specific.
...
| # | Goal | Test | Falsifier | Lane | Current |
|---|---|---|---|---|---|
| **R1** | **Per-object decomposition.** Every object is fitted independently and the whole population of solutions is kept, not just the best. | Fit each galaxy/cluster with its own parameters over the declared law space; seal the population as an exploratory receipt with the exploration/confirmation split recorded. | Any per-object fit that touched a confirmation-set object, or a population reported without its split. | GPU + CPU | **Built, and widened to the whole published sample.** `per_object_law_decomposition.py` fits each object alone over 14 declared law spaces, keeping the whole population and ranking none; the split is a salted hash of galaxy *names*, sealed with its own digest before pass 1, and every fitting entry point raises `ConfirmationSetTouched` rather than declining. `sparc_full_sample.py` then runs the same interface over `configs/sparc_rotation_curves_full_v1.json` -- **175 galaxies and 3391 published points**, up from 6 and 214, each galaxy carrying its own source-file and row digests. 14 law spaces x 139 exploration galaxies = **1946 independent single-object fits** on 2720 points. 35 galaxies are withheld and only 2 of them had ever been read by an earlier receipt, so unlike the six-galaxy predecessor the confirmation set is very largely virgin. The widened rows are authenticated by exact agreement with the six galaxies the repository retrieved independently from VizieR: 214 rows, 1284 published fields, byte for byte |
| **R2** | **Parameter-variation diagnosis.** Report which per-object parameters are constant within uncertainty and which are not. | For each parameter, an exact interval per object; test whether a single value lies in every interval. A constant parameter means the universal law already exists. | Declaring a parameter constant when no single value lies in all intervals, or variable when one does. | CPU | **Built, and the six-galaxy answer does not survive the widening.** Exact rational interval per object per parameter, and an exhibited witness or an exhibited disjoint pair with its exact gap. On 4 exploration galaxies the 12 screened families were all **CONSTANT** at price exactly 1. On **139** exploration galaxies every one of the 14 law spaces is **VARIES** -- no shared value reaches every galaxy, and each verdict carries two named galaxies and an exact positive gap. The measurement that separates law from sample size: cut the same 139 galaxies into 34 disjoint blocks of 4 -- the predecessor's population size -- and **135 of 476** law-space-blocks call their parameter CONSTANT, the deliberately wrong law in 12 of its 34. So CONSTANT drawn from four objects is a statement about the sample and not about the law, and the price of universality remains a diagnosis and never a score. What does still separate on the full sample is `k_pop`: the wrong law needs coverage 165.7 where every screened family needs at most 95.0, and the Newtonian baryonic rescale needs 66.0 but prices universality at 1.856 against the families' 1.039-1.122 |
| **R3** | **Channel discovery.** Ask whether the variation is tracked by a measured but *unlabelled* channel. | Give the reconciliation search extra unlabelled channels plus an MDL cost for using one. A channel earns its place only by beating every law that ignores it. | A channel declared explanatory without paying the MDL cost, or a named channel - naming it before it is measured leaks the concept. | GPU + CPU | **Built on synthetic tables; no real data.** `rule(u)` is widened to `rule(u, w, x)`: `ProblemSpec.channel_points` carries a complete factorial design and the entry point receives positional channels with allowlisted placeholder names. Every law pays a decodable exact two-part MDL code plus a declared fee for each channel it actually uses. The sealed controls distinguish a load-bearing channel, an inert channel, and signal too weak to repay the fee; the FunSearch receipt reports the exact obstruction floor and adoption decision separately. |
| **R4** | **Reconciliation search.** Find one law that reproduces the entire population with zero per-object freedom. | Canonicalize the full formula atlas; learn a host-disjoint meta-law from measured baryonic features; then search for a universal formula whose per-object residuals are inside the R2 intervals. Report the Pareto population and the exact obstruction if none exists. | A reconciled law that quietly retains a per-object parameter, uses a galaxy identifier or target-derived discrepancy, or is only a cosmetic rewrite counted as a new solution. | GPU + CPU + LLM | **Not built on real data.** G1-G3 of the successor plan specify the formula atlas, equivalence classes, and meta-law tests. |
| **R5** | **Held-out confirmation.** The reconciled law is tried once, on objects that took no part in R1-R4. | Freeze one zero-object-parameter law and evaluate it once on the 35 SPARC confirmation galaxies, followed without refit by independent galaxy, cluster, and lensing tests. PASS and REJECT are both publishable under I6. | Any tuning after confirmation is opened, or a different gravity/lensing law for another phenomenon. That spends the trial and voids the result. | CPU | **Not built.** G4-G12 of the successor plan define the full empirical and theory gates. |
