...
| **G2** | Collapse cosmetic formulas into real solution classes | Canonicalize and compare every G1 survivor across all 139 real galaxies. | Algebraic rewrites, unit reparameterizations, parameter renamings, and behaviorally indistinguishable formulas are merged or explicitly marked unresolved; mutation controls prove the equivalence detector can separate near misses. | **AUTHORIZED; IN PROGRESS.** |
...
G3 writes `galaxy-formula-meta-law-v1.json`. It treats the G2 atlas as a population-level
