...
| **G4** | Freeze one universal galaxy law | Refit G3 into one formula with universal gravitational constants and zero per-galaxy gravitational parameters, then open the 35 SPARC confirmation galaxies once. | The frozen law beats Newtonian baryons, meets the preregistered comparison to RAR/MOND and the flexible halo performance ceiling, is calibrated across morphology/surface-brightness strata, and receives no post-open adjustment. | **TERMINAL BLOCK AFTER FOUR DISTINCT CONSTRUCTIONS** — conditional-generator v4 represented 261,348 cells across all 8,609 G2 structural classes plus 80 focusing/synchronization operators. A user-proposed baryonic-focusing combination won at **106,187.797**, 18.76% better than RAR and materially better than v3's 120,016.785. It uses two universal generator constants, zero local fitted constants, and acceptable strata, but still exceeds the unchanged NFW-plus-slack ceiling 33,458.807 by **72,728.990**. Confirmation remains locked. |
...
This run has reached its stopping condition at **G4**. G0-G3 passed. The fourth G4 construction
showed a material new result: baryonic-occupancy focusing improved RAR by 18.76% and substantially
outperformed both the conditional formula atlas and the synchronization lane. It nevertheless
remains far from the frozen flexible-halo ceiling, so it cannot authorize confirmation. G5-G12
remain deliberately unopened. The next scientifically distinct branch should derive the focusing
operator from a field/action or transport principle and expose predictions beyond rotation curves;
