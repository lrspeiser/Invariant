...
| **G4** | Freeze one universal galaxy law | Refit G3 into one formula with universal gravitational constants and zero per-galaxy gravitational parameters, then open the 35 SPARC confirmation galaxies once. | The frozen law beats Newtonian baryons, meets the preregistered comparison to RAR/MOND and the flexible halo performance ceiling, is calibrated across morphology/surface-brightness strata, and receives no post-open adjustment. | **TERMINAL BLOCK AFTER FIVE DISTINCT CONSTRUCTIONS PLUS V6 AND AN EXPLORATORY V7 CROSS-SCALE DIAGNOSTIC** — v6 puts the fold-stable cross-scale parent inside one effective radial action. Exact variation produces its screened and directed auxiliary equations and integrated flux law. A post-v5 disk-dimension closure predicts `beta=1/2` and `ell=1/4`; `D=2` reproduces the parent exactly and beats frozen `D=1,3` counterfactuals. The action remains **6.06% worse than RAR** on galaxies. User-authorized v7 then transferred the grammar to 20 CLASH clusters/84 lensing-derived acceleration points: fixed `D=3` fails, while every whole-cluster fold selects the same parent structure at `beta=2`, four times the galaxy coefficient. This is a model-dependent structural lead, not a universal law or direct-lensing pass. Confirmation and sequential G6-G8 remain locked. |
| **G5** | Replicate through independent measurement pipelines | Without refitting the G4 law, predict the 19 high-resolution THINGS rotation curves and 26 LITTLE THINGS dwarf-galaxy rotation curves; separate objects overlapping SPARC from genuinely new objects. | The frozen law passes the same direct-observable score on both the overlap-consistency and new-object subsets; instrument/pipeline changes do not create a systematic residual. | **NOT AUTHORIZED: G4 produced no frozen law.** |
| **G6** | Build and reconcile a cluster formula atlas | On the 12 X-COP clusters, infer local baryon-to-dynamics relations from XMM-Newton density/temperature data and Planck SZ pressure data, with member-galaxy baryons and nonthermal pressure treated as declared nuisances rather than dark matter. | Each cluster first has a radial-holdout local solution; those solutions then compress to the same G4 law with zero cluster-specific gravitational parameter and predict held-out X-ray/SZ observables, not a GR-derived mass map. | **NOT AUTHORIZED IN THIS SEQUENTIAL RUN: G4 blocked; evaluator schema only.** |
| **G7** | Find one cross-scale weak-field operator | Compare G2 galaxy classes with G6 cluster classes and search for a shared local operator, environment law, or nonlocal kernel that generates both. | A single dimensionally valid operator predicts held-out galaxies and held-out clusters, carries no object label, and forward-generates the accepted local formula classes within their uncertainty. | **NOT AUTHORIZED: no G4/G6 survivor.** |
| **G8** | Make the same law bend light | Freeze the G7 operator and test a covariant completion on the 25 CLASH clusters using calibrated HST images, multiple-image positions/parities, and audited time delays where available. | With no lensing-only gravity parameter or GR-derived mass map as target, the same law beats baryon-only GR and is non-inferior to the preregistered standard lens model on direct lensing observables. | **NOT AUTHORIZED: no G7 operator; schema controls only.** |
| **G9** | Invert the atlas into root-cause theories | Build a verified graph from local formula classes to shared weak-field operators to covariant actions/field equations. Run blinded Newton/Poisson, Yukawa/Klein-Gordon, and AQUAL-like controls before applying it to G7. | Every edge is checked by forward derivation; the candidate action reproduces G7, supplies G8 lensing from the same fields, obeys dimensions and conservation identities, and reports all action classes still observationally indistinguishable. | **NOT AUTHORIZED: no data-driven G7 operator; formal pieces only.** |
| **G10** | Survive local and strong-field gravity | Test the unchanged G9 action against Cassini solar-conjunction tracking, the Double Pulsar, and GW170817/GRB 170817A propagation constraints. | One parameter set passes Solar-System PPN, strong-field timing/radiation, gravitational-wave speed/dispersion/polarization, stability, hyperbolicity, and positive-energy gates. | **NOT AUTHORIZED: no G9 action; formal controls remain supporting infrastructure only.** |
| **G11** | Survive cosmology without cold dark matter | Evaluate the unchanged theory with the public Planck 2018 CMB likelihoods and DESI DR2 BAO measurements, then structure-growth and cosmological-lensing data. | A declared no-non-baryonic-dark-matter cosmology gives an acceptable preregistered joint likelihood and does not introduce an undeclared effective matter component or a new set of phenomenon-specific constants. | **NOT AUTHORIZED: no unchanged G9/G10 survivor.** |
| **G12** | Independent replication and novelty adjudication | Freeze code, theory, data transformations, exclusions, and prior-art corpus; have an independent analyst replay a new-object test and inspect the claimed construction. | Independent replay passes; known rewrites and known-family instances are removed from the novelty claim; human domain review supports the exact bounded claim. | **NOT AUTHORIZED: no candidate claim to replicate or adjudicate.** |

This run has reached its stopping condition at **G4**. G0-G3 passed. The fourth construction found
an in-development baryonic-focusing improvement, while the fifth construction's genuine outer
whole-galaxy experiment showed that unconstrained selection among ten weak-field mechanism lanes is
unstable. V6 then achieved a narrower result: one radial action derives both terms of the stable
cross-scale parent, and a disk-dimension closure reproduces its coefficient and scale. The inherited
transition and every complete-theory obligation remain unresolved, and the prediction is still
worse than RAR. G5-G12 remain deliberately unopened. The next branch must derive the transition and
a covariant same-field lensing limit; additional SPARC-only parameter tuning is not sufficient.

A separate, explicitly exploratory v7 branch subsequently tested real CLASH cluster-lensing-derived
acceleration profiles without advancing the sequential gates. The fixed spherical v6 projection
failed, but a whole-cluster-held-out transfer selected the same structural parent in all five folds
at `beta=2`, rather than the galaxy value `beta=0.5`. This preserves a precise structural lead while
falsifying current universality. See `GRAVITY_G4_CLUSTER_LENSING_EXPLORATION_RESULT.md`.
