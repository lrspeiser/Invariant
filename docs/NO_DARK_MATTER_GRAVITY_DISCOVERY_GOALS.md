...
| **G4** | Freeze one universal galaxy law | Refit G3 into one formula with universal gravitational constants and zero per-galaxy gravitational parameters, then open the 35 SPARC confirmation galaxies once. | The frozen law beats Newtonian baryons, meets the preregistered comparison to RAR/MOND and the flexible halo performance ceiling, is calibrated across morphology/surface-brightness strata, and receives no post-open adjustment. | **V1 CONSTRUCTION BLOCK** — an exhaustive 513-cell compact family produced a zero-local-constant acceleration law that improves RAR by 4.26%, but its chi-square 125,143 exceeds the NFW-plus-slack ceiling 33,459 by 91,684. It is a `KNOWN_FAMILY` recalibration, not a new theory. Confirmation remains locked. |
