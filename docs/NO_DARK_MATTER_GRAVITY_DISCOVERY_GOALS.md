...
| **G4** | Freeze one universal galaxy law | Refit G3 into one formula with universal gravitational constants and zero per-galaxy gravitational parameters, then open the 35 SPARC confirmation galaxies once. | The frozen law beats Newtonian baryons, meets the preregistered comparison to RAR/MOND and the flexible halo performance ceiling, is calibrated across morphology/surface-brightness strata, and receives no post-open adjustment. | **TERMINAL BLOCK AFTER FIVE DISTINCT CONSTRUCTIONS** — v5 implemented all ten proposed first-principles mechanism lanes: 280 mechanisms plus an exact known RAR control, 3,653 cells per selection, and 49,680,800 scoring-point evaluations. Genuine outer whole-galaxy selection was unstable and reached **193,904.989**, 48.34% worse than RAR. The best lane was a fold-stable cross-scale permittivity-plus-auxiliary parent at **138,636.609**, 6.06% worse than RAR. The known control was recovered exactly and kept separate from creativity. No covariant completion or downstream gate passed; confirmation remains locked. |
...
an in-development baryonic-focusing improvement, but the fifth construction's genuine outer
whole-galaxy experiment showed that free selection among ten weak-field mechanism lanes is unstable
and substantially worse than RAR. One cross-scale permittivity-plus-auxiliary structure was selected
identically in all folds and remains a useful derivation parent, but it does not pass G4. G5-G12
remain deliberately unopened. The next scientifically distinct branch must derive that stable
parent's coefficient and scale from an action and discharge conservation/stability obligations;
additional SPARC-only parameter tuning is not sufficient.
